...
WHERE pagos_facturas_proveedores.idFacturacion = {$_GET['view']} AND idTipo=4
";
//Consulta
$resultado = mysqli_query ($dbConn, $query);
//Si ejecuto correctamente la consulta
...
WHERE pagos_facturas_clientes.idFacturacion = {$_GET['view']} AND idTipo=4
";
//Consulta
$resultado = mysqli_query ($dbConn, $query);
//Si ejecuto correctamente la consulta
...
WHERE bodegas_arriendos_facturacion_historial.idFacturacion = {$_GET['view']} 
...
WHERE bodegas_arriendos_facturacion.idFacturacion = {$_GET['view']} 
...
LEFT JOIN `core_documentos_mercantiles`  ON core_documentos_mercantiles.idDocumentos     = bodegas_arriendos_facturacion.idDocumentos
LEFT JOIN `usuarios_listado`             ON usuarios_listado.idUsuario                   = bodegas_arriendos_facturacion.idUsuario
WHERE bodegas_arriendos_facturacion.idFacturacionRelacionado = {$_GET['view']}";
//Consulta
$resultado = mysqli_query ($dbConn, $query);
//Si ejecuto correctamente la consulta
if(!$resultado){
			...
			case 11:
				...
				<div class="col-sm-4 invoice-col">
					...
					if(isset($row_data['Devolucion_fecha'])&&$row_data['Devolucion_fecha']!=''&&$row_data['Devolucion_fecha']!='0000-00-00'){ 
						echo '<b>Fecha Devolucion : </b>'.Fecha_estandar($row_data['Devolucion_fecha']).'<br>';
					}
					...
					if(isset($row_data['idEstadoDevolucion'])&&$row_data['idEstadoDevolucion']==1){ 
						echo '<b>Estado Devolucion : </b>'.$row_data['Devolucion_Estado'].'<br>';
					}
					if(isset($row_data['idEstadoDevolucion'])&&$row_data['idEstadoDevolucion']==2){ 
						echo '<b>Estado Devolucion : </b>'.$row_data['Devolucion_Estado'].'<br>';
						echo '<b>Usuario Devolucion : </b>'.$row_data['Devolucion_Usuario'].'<br>';
						echo '<b>Fecha Real Devolucion : </b>'.Fecha_estandar($row_data['Devolucion_FechaReal']).'<br>';
					} 
