...
$location = $original;
$new_location = "prospectos_listado_observaciones.php";
...
if (!empty($_POST['submit'])){
	...
	$form_trabajo= 'insert';
	require_once 'A1XRXS_sys/xrxs_form/prospectos_observaciones.php';
}
...
FROM `prospectos_observaciones`
LEFT JOIN `prospectos_listado`   ON prospectos_listado.idProspecto   = prospectos_observaciones.idProspecto
LEFT JOIN `usuarios_listado`     ON usuarios_listado.idUsuario       = prospectos_observaciones.idUsuario
WHERE prospectos_observaciones.idObservacion = ".$_GET['view']."
...
$query = "SELECT 
prospectos_observaciones.idObservacion,
prospectos_listado.Nombre AS nombre_prospecto,
usuarios_listado.Nombre AS nombre_usuario,
prospectos_observaciones.Fecha,
prospectos_observaciones.Observacion
...
LEFT JOIN `prospectos_listado`   ON prospectos_listado.idProspecto   = prospectos_observaciones.idProspecto
LEFT JOIN `usuarios_listado`     ON usuarios_listado.idUsuario       = prospectos_observaciones.idUsuario
WHERE prospectos_observaciones.idProspecto = ".$_GET['id']."
ORDER BY prospectos_observaciones.idObservacion ASC ";
//Consulta
$resultado = mysqli_query ($dbConn, $query);
//Si ejecuto correctamente la consulta
			...
			<ul class="nav nav-tabs pull-right">
				...
				<li class="dropdown">
					...
					<ul class="dropdown-menu" role="menu">
						...
						<li class="active"><a href="<?php echo 'prospectos_listado_observaciones.php?pagina='.$_GET['pagina'].'&id='.$_GET['id']?>" ><i class="fa fa-tasks" aria-hidden="true"></i> Observaciones</a></li>
