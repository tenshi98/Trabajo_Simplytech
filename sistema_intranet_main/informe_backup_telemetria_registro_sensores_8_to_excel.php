...
$arrUni = array();
$arrGru = array();
foreach ($arrUnimed as $uni) { $arrUni[$uni['idUniMed']] = $uni['Nombre']; }
foreach ($arrGrupo as $uni) {  $arrGru[$uni['idGrupo']]  = $uni['Nombre']; }
...
if(isset($_GET['idTelemetria'])&&$_GET['idTelemetria']!=''){
	...
	foreach ($arrTemporal as $rutas) {
		...
		$spreadsheet->setActiveSheetIndex(0)
					->setCellValue('A'.$nn, $rutas['idTabla'].' - '.$rutas['NombreEquipo'])
