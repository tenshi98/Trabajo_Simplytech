...
//se hace consulta						
$rowdata = db_select_data (false, 'Nombre, cantSensores, CrossCrane_tiempo_revision, CrossCrane_grupo_amperaje, CrossCrane_grupo_elevacion, CrossCrane_grupo_giro, CrossCrane_grupo_carro, CrossCrane_grupo_voltaje '.$subquery.', (SELECT COUNT(idErrores) FROM `telemetria_listado_errores` WHERE idTelemetria='.$X_Puntero.' AND idLeido=0 AND Fecha BETWEEN "'.$Fecha_inicio.'" AND "'.$Fecha_fin.'" AND idTipo!="999" AND Valor<"99900" AND idSistema='.$_SESSION['usuario']['basic_data']['idSistema'].') AS Alertas', 'telemetria_listado', '', 'idTelemetria ='.$X_Puntero, $dbConn, $_SESSION['usuario']['basic_data']['Nombre'], basename($_SERVER["REQUEST_URI"], ".php"), 'n_permisos');
