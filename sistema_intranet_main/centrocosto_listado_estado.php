...
?>
<div class="col-sm-12">
	<?php echo widget_title('bg-aqua', 'fa-cog', 100, 'Centro de Costo', $rowdata['Nombre'], 'Editar Estado');?>
</div>
	...
	<div class="box">
		<header>
			<ul class="nav nav-tabs pull-right">
				...
				<li class="active"><a href="<?php echo 'centrocosto_listado_estado.php?pagina='.$_GET['pagina'].'&id='.$_GET['id']?>" ><i class="fa fa-power-off" aria-hidden="true"></i> Estado</a></li>
				<li class="dropdown">
							...
							<div class="btn-group" style="width: 100px;" id="toggle_event_editing">	 
								<?php if ($rowlevel['level']>=2){?> 
								   <?php if ( $rowdata['estado']=='Activo' ) {?>   
										<a class="btn btn-sm btn-default unlocked_inactive" href="<?php echo $new_location.'&id='.$rowdata['idCentroCosto'].'&estado='.simpleEncode(2, fecha_actual()) ; ?>">OFF</a>
										<a class="btn btn-sm btn-info locked_active" href="#">ON</a>
								   <?php } else {?>
										<a class="btn btn-sm btn-info locked_active" href="#">OFF</a>
										<a class="btn btn-sm btn-default unlocked_inactive" href="<?php echo $new_location.'&id='.$rowdata['idCentroCosto'].'&estado='.simpleEncode(1, fecha_actual()) ; ?>" >ON</a>
									<?php }?>    
								<?php }?>  
							</div>     
						</td>	
