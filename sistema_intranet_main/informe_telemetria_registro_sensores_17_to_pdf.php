...
$rowEquipo = db_select_data (false, 'cantSensores', 'telemetria_listado', '', 'idTelemetria='.$idTelemetria, $dbConn, $_SESSION['usuario']['basic_data']['Nombre'], basename($_SERVER["REQUEST_URI"], ".php"), 'rowEquipo');
				...
				if($rutas['SensoresGrupo_'.$i]==$idGrupo){
					if(isset($rutas['SensorValue_'.$i])&&$rutas['SensorValue_'.$i]<99900){
						...
						if(isset($RangoMinimo)&&$RangoMinimo!=''&&isset($RangoMaximo)&&$RangoMaximo!=''){
							if($rutas['SensorValue_'.$i]<$RangoMinimo OR $rutas['SensorValue_'.$i]>$RangoMaximo){
								$x_counter++;
							}
						}
