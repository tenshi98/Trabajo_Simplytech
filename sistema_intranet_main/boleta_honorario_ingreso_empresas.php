...
$original = "boleta_honorario_ingreso_empresas.php";
$location = $original;
...
if ( !empty($_POST['submit_servicios']) )  { 
	//Llamamos al formulario
	$form_trabajo= 'new_servicio_ing_emp';
	require_once 'A1XRXS_sys/xrxs_form/z_boleta_honorario.php';
}
//formulario para crear
if ( !empty($_POST['submit_edit_servicios']) )  { 
	//Llamamos al formulario
	$form_trabajo= 'edit_servicio_ing_emp';
	require_once 'A1XRXS_sys/xrxs_form/z_boleta_honorario.php';
}
//se borra un dato
if ( !empty($_GET['del_servicios']) )     {
	//Llamamos al formulario
	$form_trabajo= 'del_servicio_ing_emp';
	require_once 'A1XRXS_sys/xrxs_form/z_boleta_honorario.php';	
}
/**********************************************/
//se borra un dato
if ( !empty($_GET['add_obs']) )     {
	//Llamamos al formulario
	$form_trabajo= 'add_obs_ing_nd_emp';
	require_once 'A1XRXS_sys/xrxs_form/z_boleta_honorario.php';	
}
//se borra un dato
if ( !empty($_GET['del_obs']) )     {
	//Llamamos al formulario
	$form_trabajo= 'del_obs_ing_nd_emp';
	require_once 'A1XRXS_sys/xrxs_form/z_boleta_honorario.php';	
}
/**********************************************/
//formulario para crear
if ( !empty($_POST['submit_file']) )  { 
	//Llamamos al formulario
	$form_trabajo= 'new_file_ing_emp';
	require_once 'A1XRXS_sys/xrxs_form/z_boleta_honorario.php';
}
//se borra un dato
if ( !empty($_GET['del_file']) )     {
	//Llamamos al formulario
	$form_trabajo= 'del_file_ing_emp';
	require_once 'A1XRXS_sys/xrxs_form/z_boleta_honorario.php';	
}
/**********************************************/
if ( !empty($_GET['ing_bodega']) )     {
	//Llamamos al formulario
	$form_trabajo= 'ing_bodega_emp';
	require_once 'A1XRXS_sys/xrxs_form/z_boleta_honorario.php';	
}
//se borra un dato
if ( !empty($_GET['del']) )     {
	...
	<div class="box dark">
		...
		<div id="div-1" class="body">
			<form class="form-horizontal" method="post" id="form1" name="form1" novalidate>
				...
				if(isset($Nombre)) {  $x1  = $Nombre;  }else{$x1  = $_SESSION['boleta_ing_prov_servicios'][$_GET['editOtros']]['Nombre'];}
				if(isset($vTotal)) {  $x2  = $vTotal;  }else{$x2  = Cantidades_decimales_justos($_SESSION['boleta_ing_prov_servicios'][$_GET['editOtros']]['vTotal']);}
				
				//se dibujan los inputs
				$Form_Imputs = new Form_Inputs();
				$Form_Imputs->form_textarea('Descripcion','Nombre', $x1, 2, 160);
				$Form_Imputs->form_input_number('Valor Total Neto', 'vTotal', $x2, 2);
				
				
				$Form_Imputs->form_input_hidden('oldidProducto', $_SESSION['boleta_ing_prov_servicios'][$_GET['editOtros']]['idServicio'], 2);
				if(isset($_SESSION['boleta_ing_prov_servicios'][$_GET['editOtros']]['idExistencia'])&&$_SESSION['boleta_ing_prov_servicios'][$_GET['editOtros']]['idExistencia']!=''){
					$Form_Imputs->form_input_hidden('idExistencia', $_SESSION['boleta_ing_prov_servicios'][$_GET['editOtros']]['idExistencia'], 2);
				}
				?>
	...
	<div class="box dark">
		...
		<div id="div-1" class="body">
			<form class="form-horizontal" method="post" id="form1" name="form1" novalidate>
				...
				if(isset($N_Doc)) {             $x2  = $N_Doc;           }else{$x2  = $_SESSION['boleta_ing_prov_basicos']['N_Doc'];}
				if(isset($Creacion_fecha)) {    $x3  = $Creacion_fecha;  }else{$x3  = $_SESSION['boleta_ing_prov_basicos']['Creacion_fecha'];}
				if(isset($idOcompra)) {         $x4  = $idOcompra;       }else{$x4  = $_SESSION['boleta_ing_prov_basicos']['idOcompra'];}
				
				//se dibujan los inputs
				$Form_Imputs = new Form_Inputs();
				$Form_Imputs->form_select_filter('Proveedor','idProveedor', $x1, 2, 'idProveedor', 'Rut,Nombre', 'proveedor_listado', $w, '', $dbConn);
				$Form_Imputs->form_input_number('Numero de Documento', 'N_Doc', $x2, 2);
				$Form_Imputs->form_date('Fecha Documento','Creacion_fecha', $x3, 2);
				$Form_Imputs->form_input_number('Numero Orden Compra', 'idOcompra', $x4, 1);
...
<div class="col-sm-12 fcenter">
	...
	<div id="page-wrap">
		<div id="header"> <?php echo $_SESSION['boleta_ing_prov_basicos']['TipoDocumento']; ?></div>
			...
			<table id="meta" class="fleft otdata">
				<tbody>
					...
					<tr>
						...
						<td><?php echo $_SESSION['boleta_ing_prov_basicos']['Proveedor']; ?></td>
					</tr>
					<tr>
						<td class="meta-head">Boleta N°</td>
						<td><?php echo n_doc($_SESSION['boleta_ing_prov_basicos']['N_Doc'], 5) ?></td>
					</tr>
					<?php if(isset($_SESSION['boleta_ing_prov_basicos']['idOcompra'])&&$_SESSION['boleta_ing_prov_basicos']['idOcompra']!=''){ ?>
						<tr>
							<td class="meta-head">Orden Compra N°</td>
							<td><?php echo n_doc($_SESSION['boleta_ing_prov_basicos']['idOcompra'], 5) ?></td>
						</tr>
					<?php } ?>
				</tbody>
			</table>
			<table id="meta" class="otdata2">
				<tbody>
					<tr>
						...
						<td colspan="2"><?php echo Fecha_estandar($_SESSION['boleta_ing_prov_basicos']['Creacion_fecha'])?></td>
					</tr>
				</tbody>
			</table>
		</div>
		<table id="items">
			<tbody>
						...
						<tr class="item-row linea_punteada">
							...
							<td>
								<div class="btn-group" style="width: 70px;" >
									<a href="<?php echo $location.'&editOtros='.$producto['idServicio']; ?>" title="Editar Servicio" class="btn btn-success btn-sm tooltip"><i class="fa fa-pencil-square-o"></i></a>
									<?php 
				...
				<?php
				//Calculo totales
				$_SESSION['boleta_ing_prov_basicos']['valor_neto']   = $vtotal_neto;
				$_SESSION['boleta_ing_prov_basicos']['valor_imp']    = $vtotal_neto * 0.10;
				$_SESSION['boleta_ing_prov_basicos']['valor_total']  = $vtotal_neto - ($vtotal_neto * 0.10);
				?>
					<tr class="invoice-total" bgcolor="#f1f1f1">
						<td colspan="4" align="right"><strong>Total Honorarios</strong></td> 
						<td align="right"><?php echo Valores($_SESSION['boleta_ing_prov_basicos']['valor_neto'], 0);?></td>
						<td></td>
					</tr>
					<tr class="invoice-total" bgcolor="#f1f1f1">
						<td colspan="4" align="right"><strong>10% Impuesto Retenido</strong></td> 
						<td align="right"><?php echo Valores($_SESSION['boleta_ing_prov_basicos']['valor_imp'], 0);?></td>
						<td></td>
					</tr>
					<tr class="invoice-total" bgcolor="#f1f1f1">
						<td colspan="4" align="right"> <strong>Total</strong></td>    
						<td align="right"><?php echo Valores($_SESSION['boleta_ing_prov_basicos']['valor_total'], 0);?></td>
						<td></td>
					</tr>
				...
				<tr>
					<?php if(isset($_SESSION['boleta_ing_prov_basicos']['Observaciones'])&&$_SESSION['boleta_ing_prov_basicos']['Observaciones']!=''){ ?>
					
						<td colspan="5" class="blank word_break"> 
							<?php echo $_SESSION['boleta_ing_prov_basicos']['Observaciones'];?>
						</td>
						<td class="blank">
					...
					<?php }else{?>
						<td colspan="5" class="blank"> 
							<?php 
							$non = '';
							if(isset($_SESSION['boleta_ing_prov_temporal'])&&$_SESSION['boleta_ing_prov_temporal']!=''){
								$non = $_SESSION['boleta_ing_prov_temporal'];
							}	
	...
	<div class="box dark">
		...
		<div id="div-1" class="body">
			<form class="form-horizontal" method="post" id="form1" name="form1" novalidate>
				...
				$Form_Imputs->form_input_number('Numero Orden Compra', 'idOcompra', $x5, 1);
...
$z="WHERE boleta_honorarios_facturacion.idTipo=3";//Solo empresas
//Verifico el tipo de usuario que esta ingresando
...
if(isset($_GET['idProveedor']) && $_GET['idProveedor'] != ''){        $z .= " AND boleta_honorarios_facturacion.idProveedor=".$_GET['idProveedor'];}
	...
	<div class="well">
		<div class="col-sm-8 fcenter">
			<form class="form-horizontal" id="form1" name="form1" action="<?php echo $location; ?>" novalidate>
				...
				//se dibujan los inputs
				$Form_Imputs = new Form_Inputs();
				$Form_Imputs->form_select_filter('Proveedor','idProveedor', $x1, 1, 'idProveedor', 'Rut,Nombre', 'proveedor_listado', $w, '', $dbConn);
...
<div class="col-sm-12">
	<div class="box">
		<header>
			<div class="icons"><i class="fa fa-table"></i></div><h5>Listado de Boletas de Honorarios</h5>
			<div class="toolbar">
				<?php 
		...
		<div class="table-responsive"> 
			<table id="dataTable" class="table table-bordered table-condensed table-hover table-striped dataTable">
				...
				<tbody role="alert" aria-live="polite" aria-relevant="all">
					...
					<tr class="odd">
						<td><?php echo $tipo['ProveedorNombre']; ?></td>
						<td><?php echo 'Boleta N° '.$tipo['N_Doc']; ?></td>
						<td><?php echo Fecha_estandar($tipo['Creacion_fecha']); ?></td>
						<?php if($_SESSION['usuario']['basic_data']['idTipoUsuario']==1){ ?><td><?php echo $tipo['Sistema']; ?></td><?php } ?>
						<td>
							<div class="btn-group" style="width: 70px;" >
								<?php if ($rowlevel['level']>=1){?><a href="<?php echo 'view_boleta_honorarios.php?view='.$tipo['idFacturacion']; ?>" title="Ver Informacion" class="iframe btn btn-primary btn-sm tooltip"><i class="fa fa-list"></i></a><?php } ?>
								<?php if ($rowlevel['level']>=4){
									$ubicacion = $location.'&del='.$tipo['idFacturacion'];
