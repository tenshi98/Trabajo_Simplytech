...
//sololas del mismo sistema
$SIS_where_1.=" GROUP BY bodegas_arriendos_facturacion.idProveedor";
$SIS_where_2.=" GROUP BY bodegas_insumos_facturacion.idProveedor";
$SIS_where_3.=" GROUP BY bodegas_productos_facturacion.idProveedor";
$SIS_where_4.=" GROUP BY bodegas_servicios_facturacion.idProveedor";
...
foreach ($arrCreativo as $prod) {
	...
	$spreadsheet->setActiveSheetIndex(0)
				->setCellValue('A'.$nn, $prod['Proveedor'])
...
$spreadsheet->setActiveSheetIndex(0)
			...
			->setCellValue('F'.$nn, cantidades_excel($Total_1))
			->setCellValue('G'.$nn, cantidades_excel($Total_2))
			->setCellValue('H'.$nn, cantidades_excel($Total_3))
			->setCellValue('I'.$nn, cantidades_excel($Total_4))
			->setCellValue('K'.$nn, cantidades_excel($Total_1+$Total_2+$Total_3+$Total_4));
		
							

// Rename worksheet
$spreadsheet->getActiveSheet()->setTitle('Compras por proveedor');
