...
if(isset($arrConsulta[0]['EquipoActivacionValor'])&&$arrConsulta[0]['EquipoActivacionValor']==0){
	...
	<div class="col-sm-12" >
		<div class="alert alert-danger" role="alert">
			No tiene configurado el interruptor de encendido
		</div>
	</div>';
	$unk_var++;
}
if(isset($arrConsulta[0]['EquipoColacion_termino'])&&$arrConsulta[0]['EquipoColacion_termino']=='00:00:00'){
	...
	<div class="col-sm-12" >
		<div class="alert alert-danger" role="alert">
			No tiene configurada la hora de termino de la colacion
		</div>
	</div>';
	$unk_var++;
}
if(isset($arrConsulta[0]['EquipoColacion_inicio'])&&$arrConsulta[0]['EquipoColacion_inicio']=='00:00:00'){
	...
	<div class="col-sm-12" >
		<div class="alert alert-danger" role="alert">
			No tiene configurada la hora de inicio de la colacion
		</div>
	</div>';
	$unk_var++;
}
if(isset($arrConsulta[0]['EquipoJornada_inicio'])&&$arrConsulta[0]['EquipoJornada_inicio']=='00:00:00'){
	...
	<div class="col-sm-12" >
		<div class="alert alert-danger" role="alert">
			No tiene configurada la hora de inicio de la jornada
		</div>
	</div>';
	$unk_var++;
}
if(isset($arrConsulta[0]['EquipoJornada_termino'])&&$arrConsulta[0]['EquipoJornada_termino']=='00:00:00'){
	...
	<div class="col-sm-12" >
		<div class="alert alert-danger" role="alert">
			No tiene configurada la hora de termino de la jornada
		</div>
	</div>';
	$unk_var++;
}
...
<div class="col-sm-12">
	<a target="new" href="<?php echo 'informe_telemetria_activaciones_03_to_excel.php?d=d'.$search.'&idTipoUsuario='.$_SESSION['usuario']['basic_data']['idTipoUsuario'].'&idSistema='.$_SESSION['usuario']['basic_data']['idSistema'] ; ?>" class="btn btn-sm btn-metis-2 fright margin_width"><i class="fa fa-file-excel-o"></i> Exportar a Excel</a>
</div>
...
<?php filtrar($arrConsulta, 'EquipoNombre');
	...
	foreach($arrConsulta as $categoria=>$permisos){ ?>
		...
		<div class="col-sm-12">
			<div class="box">
				<header>
					<div class="icons"><i class="fa fa-table"></i></div><h5>Actividad del equipo <?php echo $categoria.' ('.$permisos[0]['EquipoJornada_inicio'].' a '.$permisos[0]['EquipoJornada_termino'].')'; ?></h5>
				</header>
					...
					<table id="dataTable" class="table table-bordered table-condensed table-hover table-striped dataTable">
						<thead>
							<tr role="row">
								...
								<th>Fecha</th>
								<th>Contrato</th>
								<th>Hora<br/>Inicio     <a title="Hora de inicio real del equipo (Hora Programada:<?php echo $permisos[0]['EquipoJornada_inicio']; ?> hrs)" class="tooltip" style="display: inline-block;"><i class="fa fa-info-circle"></i></a></th>
								<th>Hora<br/>Termino    <a title="Hora de termino real del equipo (Hora Programada:<?php echo $permisos[0]['EquipoJornada_termino']; ?> hrs)" class="tooltip" style="display: inline-block;"><i class="fa fa-info-circle"></i></a></th>
								<th>Tiempo<br/>Colacion <a title="Detencion para la hora de colacion programada entre las <?php echo $permisos[0]['EquipoColacion_inicio']; ?> y las <?php echo $permisos[0]['EquipoColacion_termino']; ?>" class="tooltip" style="display: inline-block;"><i class="fa fa-info-circle"></i></a></th>
								<th>Tiempo<br/>Muerto   <a title="Todas las detenciones superiores a <?php echo $permisos[0]['EquipoMicroparada']; ?> entre la hora de inicio y termino real del equipo" class="tooltip" style="display: inline-block;"><i class="fa fa-info-circle"></i></a></th>
									...
									<tr class="odd">
										...
										<td>
											<div class="btn-group" style="width: 105px;" >
												<?php if ($rowlevel['level']>=1&&$fecha==''){?><a href="<?php echo 'view_telemetria_historial.php?view='.$con['idTelemetria'].'&dia='.$fecha; ?>" title="Ver Informacion" class="iframe btn btn-primary btn-sm tooltip"><i class="fa fa-list"></i></a><?php } ?>
												<?php if ($rowlevel['level']>=1){?><a href="<?php echo 'view_telemetria_historial_activaciones.php?view='.$con['idTelemetria'].'&dia='.$fecha.'&cantSensores='.$con['EquipoN_Sensores'].$Ampx; ?>" title="Ver Amperes" class="iframe btn btn-primary btn-sm tooltip"><i class="fa fa-power-off"></i></a><?php } ?>
												<?php if ($rowlevel['level']>=1&&$FueraHorario!=0&&$fecha==''){?><a href="<?php echo 'view_telemetria_historial.php?view='.$con['idTelemetria'].'&dia='.$fecha.'&fueraHorario=true'; ?>" title="Ver Apagado por Amp." class="iframe btn btn-danger btn-sm tooltip"><i class="fa fa-power-off"></i></a><?php } ?>
											</div>
										</td>
									</tr>
								<?php
