...
WHERE comunicaciones_streaming_listado.idStreaming = ".$_GET['id'];
//Consulta
...
$query = "SELECT 
usuarios_listado.Nombre AS Usuario
FROM `comunicaciones_streaming_listado_usuarios`
LEFT JOIN `usuarios_listado`   ON usuarios_listado.idUsuario   = comunicaciones_streaming_listado_usuarios.idUsuario

WHERE comunicaciones_streaming_listado_usuarios.idStreaming = ".$_GET['id']."
	...
	<div class="box">
		<header>
			<ul class="nav nav-tabs pull-right">
				...
				<li class=""><a href="<?php echo 'comunicaciones_streaming_listado_datos.php?pagina='.$_GET['pagina'].'&id='.$_GET['id']?>" ><i class="fa fa-list-alt" aria-hidden="true"></i> Datos Basicos</a></li>
...
$z = "WHERE comunicaciones_streaming_listado.idEstado!=0";
//Verifico el tipo de usuario que esta ingresando
$z.=" AND comunicaciones_streaming_listado.idSistema=".$_SESSION['usuario']['basic_data']['idSistema'];
/**********************************************************/
//Se aplican los filtros
if(isset($_GET['Nombre']) && $_GET['Nombre']!=''){ $z .= " AND comunicaciones_streaming_listado.Nombre LIKE '%".EstandarizarInput($_GET['Nombre'])."%'";}
if(isset($_GET['Fecha']) && $_GET['Fecha']!=''){   $z .= " AND comunicaciones_streaming_listado.Fecha='".$_GET['Fecha']."'";}
if(isset($_GET['idEstado']) && $_GET['idEstado']!=''){    $z .= " AND comunicaciones_streaming_listado.idEstado='".$_GET['idEstado']."'";}
if(isset($_GET['idTipo']) && $_GET['idTipo']!=''){ $z .= " AND comunicaciones_streaming_listado.idTipo='".$_GET['idTipo']."'";}
if(isset($_GET['idUsuario']) && $_GET['idUsuario']!=''){  $z .= " AND comunicaciones_streaming_listado.idUsuario='".$_GET['idUsuario']."'";}
/**********************************************************/
...
".$z."
ORDER BY comunicaciones_streaming_listado.idEstado ASC, comunicaciones_streaming_listado.Fecha ASC
";
//Consulta
$resultado = mysqli_query ($dbConn, $query);
//Si ejecuto correctamente la consulta
						...
						<td>
							<div class="btn-group" style="width: 105px;" >
								...
								<?php if(($Conferencia[0]['idUsuario']==$_SESSION['usuario']['basic_data']['idUsuario']) OR ($_SESSION['usuario']['basic_data']['idTipoUsuario']==1)){ ?>
									<?php if ($rowlevel['level']>=2){ ?><a href="<?php echo $location.'&id='.$Streaming; ?>" title="Editar Informacion" class="btn btn-success btn-sm tooltip"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></a><?php } ?>
