...
$original = "bodegas_servicios_ingreso_nota_credito.php";
$location = $original;
	...
	<div class="box dark">
		...
		<div id="div-1" class="body">
			<form class="form-horizontal" method="post" id="form1" name="form1" novalidate>
				...
				if(isset($idServicio)) {     $x1  = $idServicio;    }else{$x1  = $_SESSION['servicios_ing_nc_productos'][$_GET['editProd']]['idServicio'];}
				if(isset($Cantidad_eg)) {    $x2  = $Cantidad_eg;   }else{$x2  = Cantidades_decimales_justos($_SESSION['servicios_ing_nc_productos'][$_GET['editProd']]['Cantidad_eg']);}
				if(isset($idFrecuencia)) {   $x3  = $idFrecuencia;  }else{$x3  = $_SESSION['servicios_ing_nc_productos'][$_GET['editProd']]['idFrecuencia'];}
				if(isset($ValorTotal)) {     $x4  = $ValorTotal;    }else{$x4  = Cantidades_decimales_justos($_SESSION['servicios_ing_nc_productos'][$_GET['editProd']]['ValorTotal']);}
				
				//se dibujan los inputs
				$Form_Imputs = new Form_Inputs();
				$Form_Imputs->form_select_filter('Servicios','idServicio', $x1, 2, 'idServicio', 'Nombre', 'servicios_listado', 'idEstado=1', '', $dbConn);
				...
				$Form_Imputs->form_input_disabled('Valor Unitario Neto','Unitario', Cantidades_decimales_justos($_SESSION['servicios_ing_nc_productos'][$_GET['editProd']]['ValorIngreso']), 1);
				$Form_Imputs->form_input_number('Valor Total Neto', 'ValorTotal', $x4, 2);
				$Form_Imputs->form_input_hidden('vUnitario', Cantidades_decimales_justos($_SESSION['servicios_ing_nc_productos'][$_GET['editProd']]['ValorIngreso']), 2);
	...
	<div class="box dark">
		...
		<div id="div-1" class="body">
			<form class="form-horizontal" method="post" id="form1" name="form1" novalidate>
				...
				<?php 
				//Se verifican si existen los datos
				if(isset($Nombre)) {  $x1  = $Nombre;  }else{$x1  = $_SESSION['servicios_ing_nc_otros'][$_GET['editOtros']]['Nombre'];}
				if(isset($vTotal)) {  $x2  = $vTotal;  }else{$x2  = Cantidades_decimales_justos($_SESSION['servicios_ing_nc_otros'][$_GET['editOtros']]['vTotal']);}
				...
				$Form_Imputs->form_input_hidden('oldidProducto', $_SESSION['servicios_ing_nc_otros'][$_GET['editOtros']]['idOtros'], 2);
	...
	<div class="box dark">
		...
		<div id="div-1" class="body">
			<form class="form-horizontal" method="post" id="form1" name="form1" novalidate>
				...
				<?php 
				//Se verifican si existen los datos
				if(isset($idCentroCosto)) {  $x1  = $idCentroCosto;  }else{$x1  = $_SESSION['servicios_ing_nc_basicos']['idCentroCosto'];}
				if(isset($idLevel_1)) {      $x2  = $idLevel_1;      }else{$x2  = $_SESSION['servicios_ing_nc_basicos']['idLevel_1'];}
				if(isset($idLevel_2)) {      $x3  = $idLevel_2;      }else{$x3  = $_SESSION['servicios_ing_nc_basicos']['idLevel_2'];}
				if(isset($idLevel_3)) {      $x4  = $idLevel_3;      }else{$x4  = $_SESSION['servicios_ing_nc_basicos']['idLevel_3'];}
				if(isset($idLevel_4)) {      $x5  = $idLevel_4;      }else{$x5  = $_SESSION['servicios_ing_nc_basicos']['idLevel_4'];}
				if(isset($idLevel_5)) {      $x6  = $idLevel_5;      }else{$x6  = $_SESSION['servicios_ing_nc_basicos']['idLevel_5'];}
	...
	<div class="box dark">
		...
		<div id="div-1" class="body">
			<form class="form-horizontal" method="post" id="form1" name="form1" novalidate>
				...
				<?php 
				//Se verifican si existen los datos
				if(isset($idProveedor)) {        $x1  = $idProveedor;      }else{$x1  = $_SESSION['servicios_ing_nc_basicos']['idProveedor'];}
				if(isset($N_Doc)) {              $x2  = $N_Doc;            }else{$x2  = $_SESSION['servicios_ing_nc_basicos']['N_Doc'];}
				if(isset($Creacion_fecha)) {     $x3  = $Creacion_fecha;   }else{$x3  = $_SESSION['servicios_ing_nc_basicos']['Creacion_fecha'];}
				if(isset($Observaciones)) {      $x4  = $Observaciones;    }else{$x4  = $_SESSION['servicios_ing_nc_basicos']['Observaciones'];}
...
<div class="col-sm-12 fcenter">
	...
	<div id="page-wrap">
		<div id="header"> <?php echo $_SESSION['servicios_ing_nc_basicos']['TipoDocumento']; ?></div>
			...
			<table id="meta" class="fleft otdata">
				<tbody>
					...
					<tr>
						...
						<td><?php echo $_SESSION['servicios_ing_nc_basicos']['Proveedor']; ?></td>
					</tr>
					<tr>
						<td class="meta-head">Documento</td>
						<td><?php echo $_SESSION['servicios_ing_nc_basicos']['Documento'].' N°'.$_SESSION['servicios_ing_nc_basicos']['N_Doc']?></td>
					</tr>
					<tr>
					...
					<tr>
						...
						<td><?php echo $_SESSION['servicios_ing_nc_basicos']['CentroCosto']; ?></td>
					</tr>
				</tbody>
			</table>
		...
		<table id="items">
			<tbody>
				...
				<?php  //Guardo el neto
				$_SESSION['servicios_ing_nc_basicos']['valor_neto_fact'] = $vtotal_neto;
				?>
					...
					$_SESSION['servicios_ing_nc_basicos']['valor_neto_imp'] = $vtotal_neto;
					?>
					<tr class="invoice-total" bgcolor="#f1f1f1">
					...
					if (isset($_SESSION['servicios_ing_nc_impuestos'])){
						...
						$tempa = $vtotal_neto;
						//recorro el lsiatdo entregado por la base de datos
						foreach ($_SESSION['servicios_ing_nc_impuestos'] as $key => $producto){
							...
							$_SESSION['servicios_ing_nc_impuestos'][$producto['idImpuesto']]['valor'] = $vtotal_IVA;
					...
					//guardo el total
					$_SESSION['servicios_ing_nc_basicos']['valor_total_fact'] = $vtotal_neto;
				...
				<tr>
					<?php if(isset($_SESSION['servicios_ing_nc_basicos']['Observaciones'])&&$_SESSION['servicios_ing_nc_basicos']['Observaciones']!=''){ ?>
					
						<td colspan="5" class="blank word_break"> 
							<?php echo $_SESSION['servicios_ing_nc_basicos']['Observaciones'];?>
						</td>
						<td class="blank">
							<div class="btn-group" style="width: 35px;" >
								<?php 
					...
					<?php }else{?>
						<td colspan="5" class="blank"> 
							<?php 
							$non = '';
							if(isset($_SESSION['servicios_ing_nc_temporal'])&&$_SESSION['servicios_ing_nc_temporal']!=''){
								$non = $_SESSION['servicios_ing_nc_temporal'];
							}	
...
$z="WHERE bodegas_servicios_facturacion.idTipo=11";//Solo egresos
//Verifico el tipo de usuario que esta ingresando
...
<div class="col-sm-12 breadcrumb-bar">
	...
	<?php if ($rowlevel['level']>=3){ ?>
		<?php if (isset($_SESSION['servicios_ing_nc_basicos']['idProveedor'])&&$_SESSION['servicios_ing_nc_basicos']['idProveedor']!=''){?>
