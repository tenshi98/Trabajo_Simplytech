...
$original = "informe_telemetria_registro_promedios_6.php";
$location = $original;
...
function crear_data($cantsens, $filtro, $idTelemetria, $f_inicio, $f_termino, $desde, $hasta, $dbConn ) {
	...
	telemetria_listado_tablarelacionada_".$idTelemetria.".FechaSistema AS FechaControl
	".$consql." 
	...
	if($Temperatura_N!=0 OR $Humedad_N!=0){
		if(isset($_GET['idOpciones'])&&$_GET['idOpciones']!=''&&$_GET['idOpciones']!=0){ 
			switch ($_GET['idOpciones']) {
				case 1:
					$chain   .= "['".Fecha_estandar($fac['FechaSistema'])."', ".$New_Temperatura."],";
					$m_table .= '
					<tr class="odd">
						<td>'.fecha_estandar($fac['FechaSistema']).'</td>
						<td>'.cantidades($New_Temperatura, 2).' °C</td>
					</tr>';
					break;
				case 2:
					$chain   .= "['".Fecha_estandar($fac['FechaSistema'])."', ".$New_Humedad."],";
					$m_table .= '
					<tr class="odd">
						<td>'.fecha_estandar($fac['FechaSistema']).'</td>
						<td>'.cantidades($New_Humedad, 2).' %</td>
					</tr>';
					break;
			}
		}else{
			$chain   .= "['".Fecha_estandar($fac['FechaSistema'])."', ".$New_Temperatura.", ".$New_Humedad."],";
			$m_table .= '
			<tr class="odd">
				<td>'.fecha_estandar($fac['FechaSistema']).'</td>
				<td>'.cantidades($New_Temperatura, 2).' °C</td>
...
<div class="col-sm-12">
	...
	<div class="col-md-6 col-sm-6 col-xs-12 clearfix">
		<a target="new" href="<?php echo 'informe_telemetria_registro_promedios_6_to_excel.php?bla=bla'.$search ; ?>" class="btn btn-sm btn-metis-2 pull-right margin_width"><i class="fa fa-file-excel-o" aria-hidden="true"></i> Exportar a Excel</a>
		...
		<?php }else{ ?>
			<a target="new" href="<?php echo 'informe_telemetria_registro_promedios_6_to_pdf.php?bla=bla'.$search ; ?>" class="btn btn-sm btn-metis-3 pull-right margin_width"><i class="fa fa-file-pdf-o" aria-hidden="true"></i> Exportar a PDF</a>
		<?php } ?>
...
if(isset($_GET['idGrafico'])&&$_GET['idGrafico']==1){ ?>
	<div class="col-sm-12">
		<div class="box">
			<header>
				...
				<h5>Graficos del equipo <?php echo $rowEquipo['Nombre']; ?></h5>
			</header>
				...
				<script>
					...
					function drawChart() {
						...
						if(isset($_GET['idOpciones'])&&$_GET['idOpciones']!=''&&$_GET['idOpciones']!=0){ 
							switch ($_GET['idOpciones']) {
								case 1:
									echo 'data.addColumn(\'number\', "Temperatura (°C)");';
									break;
								case 2:
									echo 'data.addColumn(\'number\', "Humedad (%)");';
									break;
							}
						}else{
							echo 'data.addColumn(\'number\', "Temperatura");';
							echo 'data.addColumn(\'number\', "Humedad");';
						} ?>
	...
	<div class="col-sm-12" style="display: none;">
		...
		<form method="post" id="make_pdf" action="informe_telemetria_registro_promedios_6_to_pdf.php">
	...
	<div class="box dark">	
		...
		<div id="div-1" class="body">	
			<form class="form-horizontal" action="<?php echo $location ?>" id="form1" name="form1" novalidate>
				...
				if(isset($idGrafico)) {     $x7  = $idGrafico;    }else{$x7  = '';}
				...
				$input = '<div class="form-group" id="div_sensorn" >
								<label for="text2" class="control-label col-sm-4">Seleccionar columna</label>
								<div class="col-sm-8 field">
									<select name="idOpciones" id="idOpciones" class="form-control">
										<option value="" selected>Seleccione una Opcion</option>
