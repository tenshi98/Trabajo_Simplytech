...
if(!empty($_GET['editProd'])){
	...
	(SELECT SUM(Cantidad_ing) FROM `bodegas_productos_facturacion_existencias` WHERE idProducto = productos_listado.idProducto AND idBodega='.$_SESSION['productos_traspasomanualempresa_basicos']['idBodegaOrigen'].' LIMIT 1) AS ingreso,
	(SELECT SUM(Cantidad_eg)  FROM `bodegas_productos_facturacion_existencias` WHERE idProducto = productos_listado.idProducto AND idBodega='.$_SESSION['productos_traspasomanualempresa_basicos']['idBodegaOrigen'].' LIMIT 1) AS egreso';
	$SIS_join  = 'LEFT JOIN `sistema_productos_uml` ON sistema_productos_uml.idUml = productos_listado.idUml';
	$SIS_where = 'productos_listado.idProducto='.$_SESSION['productos_traspasomanualempresa_productos'][$_GET['editProd']]['idProducto'];
		...
		<div class="box">
			...
			<div class="body">
				<form class="form-horizontal" method="post" id="form1" name="form1" autocomplete="off" novalidate>
					...
					$Form_Inputs->form_input_disabled('Valor Unitario','Unitario', Cantidades_decimales_justos($_SESSION['productos_traspasomanualempresa_productos'][$_GET['editProd']]['ValorEgreso']));
...
<?php //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	if(isset($_GET['order_by'])&&$_GET['order_by']!=''){
		switch ($_GET['order_by']) {
			...
			case 'destino_asc':        $order_by = 'sisdestino.Nombre ASC ';                             $bread_order = '<i class="fa fa-sort-alpha-asc" aria-hidden="true"></i> Destino Ascendente';break;
			case 'destino_desc':       $order_by = 'sisdestino.Nombre DESC ';                            $bread_order = '<i class="fa fa-sort-alpha-desc" aria-hidden="true"></i> Destino Descendente';break;
	...
	$SIS_where = "bodegas_productos_facturacion.idTipo=8";//Solo traspaso manual a otra empresa
