...
function crear_data($limite, $idTelemetria, $f_inicio, $f_termino, $dbConn ) {
	...
	$SIS_order = 0;
	$arrRutas = array();
	$arrRutas = db_select_array (false, $SIS_query, 'telemetria_listado_tablarelacionada_'.$idTelemetria, $SIS_join, $SIS_where, $SIS_order, $dbConn, $_SESSION['usuario']['basic_data']['Nombre'], $original, 'arrRutas');
...
if(isset($_GET['idTelemetria'])&&$_GET['idTelemetria']!=''){
	...
	$s4 = $nn + 3;
	$spreadsheet->setActiveSheetIndex(0)
				->setCellValue('C'.$s1, 'Minimo')
				->setCellValue('C'.$s2, 'Maximo')
				->setCellValue('C'.$s3, 'AVG')
	...
	for ($i = 1; $i <= $arrTemporal[0]['cantSensores']; $i++) { 
		...
		$count_val = 0 ;
		$sum_val   = 0 ;      
		//Verifico los datos internamente
		foreach ($arrTemporal as $rutas) {
			if(isset($rutas['SensorValue_'.$i])&&$rutas['SensorValue_'.$i]<99900&&$rutas['SensorValue_'.$i]!=0){
				//Variable
				$weight = Cantidades_decimales_justos($rutas['SensorValue_'.$i]);
				//Minimo
				if($weight <= $min_val ) {
					$min_val =  $weight ;
				}
		...
		/******************************************************/
		$spreadsheet->setActiveSheetIndex(0)
					->setCellValue($arrData[$i].$s1, $min_val)
					->setCellValue($arrData[$i].$s2, $max_val)
					->setCellValue($arrData[$i].$s3, $avg_val)
					->setCellValue($arrData[$i].$s4, $std_val);
