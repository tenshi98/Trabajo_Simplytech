	...
	<div class="box dark">
		...
		<div id="div-1" class="body">
			<form class="form-horizontal" method="post" id="form1" name="form1" novalidate>
				...
				//se dibujan los inputs
				$Form_Imputs = new Form_Inputs();
				$Form_Imputs->form_select_join_filter('Caja destino','idCajaChica', $x1, 2, 'idCajaChica', 'Nombre', 'caja_chica_listado', 'usuarios_cajas_chicas', $z, $dbConn);
				$Form_Imputs->form_date('Fecha Ingreso','Creacion_fecha', $x2, 2);
	...
	<div id="page-wrap">
		...
		<table id="items">
			<tbody>
						...
						<tr class="item-row linea_punteada">
							...
							<td>
								<div class="btn-group" style="width: 70px;" >
									<a href="<?php echo $location.'&editMonto='.$producto['bvar']; ?>" title="Editar Ingreso" class="btn btn-success btn-sm tooltip"><i class="fa fa-pencil-square-o"></i></a>
									<?php 
									$ubicacion = $location.'&del_monto='.$producto['bvar'];
									$dialogo   = '¿Realmente deseas eliminar el ingreso '.str_replace('"','',$producto['DocPago']).'?';?>
									<a onClick="dialogBox('<?php echo $ubicacion ?>', '<?php echo $dialogo ?>')" title="Borrar Ingreso" class="btn btn-metis-1 btn-sm tooltip"><i class="fa fa-trash-o"></i></a>								
								</div>
							</td>
						</tr> 
					<?php 
				...
				<tr>
					<?php if(isset($_SESSION['caja_ing_basicos']['Observaciones'])&&$_SESSION['caja_ing_basicos']['Observaciones']!=''){ ?>
					
						<td colspan="5" class="blank word_break"> 
							<?php echo $_SESSION['caja_ing_basicos']['Observaciones'];?>
						</td>
						<td class="blank">
					...
					<?php }else{?>
						<td colspan="5" class="blank"> 
							<?php 
							$non = '';
							if(isset($_SESSION['caja_ing_temporal'])&&$_SESSION['caja_ing_temporal']!=''){
								$non = $_SESSION['caja_ing_temporal'];
							}	
	...
	<div class="well">
		<div class="col-sm-8 fcenter">
			<form class="form-horizontal" id="form1" name="form1" action="<?php echo $location; ?>" novalidate>
				...
				$Form_Imputs->form_date('Fecha Ingreso','Creacion_fecha', $x2, 1);
