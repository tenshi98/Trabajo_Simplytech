...
$original = "caja_chica_ingreso.php";
$location = $original;
...
if (!empty($_POST['submit_monto'])){
	//Llamamos al formulario
	$form_trabajo= 'new_monto_ing';
	require_once 'A1XRXS_sys/xrxs_form/z_caja_chica.php';
}
//formulario para crear
if (!empty($_POST['submit_edit_monto'])){
	//Llamamos al formulario
	$form_trabajo= 'edit_monto_ing';
	require_once 'A1XRXS_sys/xrxs_form/z_caja_chica.php';
}
//se borra un dato
if (!empty($_GET['del_monto'])){
	//Llamamos al formulario
	$form_trabajo= 'del_monto_ing';
	require_once 'A1XRXS_sys/xrxs_form/z_caja_chica.php';
}
/**********************************************/
//formulario para crear
...
/**********************************************/
if (!empty($_GET['ing_Caja'])){
	//Llamamos al formulario
	$form_trabajo= 'ing_Caja';
	require_once 'A1XRXS_sys/xrxs_form/z_caja_chica.php';
}
/**********************************************************************************************************************************/
	...
	<div class="box dark">
		...
		<div class="body">
			<form class="form-horizontal" method="post" id="form1" name="form1" novalidate>
				...
				if(isset($idDocPago)){  $x1  = $idDocPago;  }else{$x1  = $_SESSION['caja_ing_documentos'][$_GET['editMonto']]['idDocPago'];}
				if(isset($N_Doc)){      $x2  = $N_Doc;      }else{$x2  = $_SESSION['caja_ing_documentos'][$_GET['editMonto']]['N_Doc'];}
				if(isset($Valor)){      $x3  = $Valor;     }else{$x3  = $_SESSION['caja_ing_documentos'][$_GET['editMonto']]['Valor'];}
	...
	<div class="box dark">
		...
		<div class="body">
			<form class="form-horizontal" method="post" id="form1" name="form1" novalidate>
				...
				<?php 
				//Se verifican si existen los datos
				if(isset($idCajaChica)){      $x1 = $idCajaChica;    }else{$x1 = $_SESSION['caja_ing_basicos']['idCajaChica'];}
				if(isset($Creacion_fecha)){   $x2 = $Creacion_fecha; }else{$x2 = $_SESSION['caja_ing_basicos']['Creacion_fecha'];}
				if(isset($Observaciones)){    $x3 = $Observaciones;  }else{$x3 = $_SESSION['caja_ing_basicos']['Observaciones'];}

				//se dibujan los inputs
				$Form_Inputs = new Form_Inputs();
				if($_SESSION['usuario']['basic_data']['idTipoUsuario']==1){
					$Form_Inputs->form_select_filter('Caja destino','idCajaChica', $x1, 2, 'idCajaChica', 'Nombre', 'caja_chica_listado', $z, '', $dbConn);
				}else{
...
<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12" style="margin-bottom:30px">
	<div class="btn-group pull-right" role="group" aria-label="...">
		...
		<?php 		
		$ubicacion = $location.'&view=true&ing_Caja=true';
...
<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
	...
	<div id="page-wrap">
		<div id="header"> <?php echo $_SESSION['caja_ing_basicos']['TipoDocumento']; ?></div>
			...
			<table id="meta" class="pull-left otdata">
				<tbody>
					...
					<tr>
						...
						<td><?php echo $_SESSION['caja_ing_basicos']['Caja']; ?></td>
					</tr>
				</tbody>
			</table>
			<table id="meta" class="otdata2">
				<tbody>
					<tr>
						...
						<td colspan="2"><?php echo Fecha_estandar($_SESSION['caja_ing_basicos']['Creacion_fecha'])?></td>
					</tr>
				</tbody>
			</table>
		</div>
		<table id="items">
			<tbody>
				...
				<tr class="item-row fact_tittle">
					...
					<td>
						<a href="<?php echo $location.'&addMonto=true' ?>" title="Agregar Ingreso" class="btn btn-xs btn-primary tooltip" style="position: initial;"><i class="fa fa-plus" aria-hidden="true"></i> Agregar Ingreso</a>
					</td>
				</tr>
				<?php 
				$vtotal = 0;
						...
						<tr class="item-row linea_punteada">
							...
							<td>
								<div class="btn-group" style="width: 70px;" >
									<a href="<?php echo $location.'&editMonto='.$producto['bvar']; ?>" title="Editar Ingreso" class="btn btn-success btn-sm tooltip"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></a>
									<?php 
									$ubicacion = $location.'&del_monto='.$producto['bvar'];
									$dialogo   = '¿Realmente deseas eliminar el ingreso '.str_replace('"','',$producto['DocPago']).'?';?>
									<a onClick="dialogBox('<?php echo $ubicacion ?>', '<?php echo $dialogo ?>')" title="Borrar Ingreso" class="btn btn-metis-1 btn-sm tooltip"><i class="fa fa-trash-o" aria-hidden="true"></i></a>								
								</div>
							</td>
						</tr>
					<?php 
				...
				<tr class="invoice-total" bgcolor="#f1f1f1">
					...
					<td align="right"><?php echo Valores($vtotal, 0);$_SESSION['caja_ing_basicos']['Valor'] = $vtotal;?></td>
					<td></td>
				</tr>
    ...
    <div class="col-xs-12">
		<div class="row">
			...
			<p class="text-muted well well-sm no-shadow" ><?php echo $_SESSION['caja_ing_basicos']['Observaciones'];?></p>
		</div>
	</div>
...
$SIS_where = "caja_chica_facturacion.idTipo=1";//Solo ingresos
	...
	<div class="well">
		<div class="col-xs-12 col-sm-10 col-md-9 col-lg-8 fcenter">
			<form class="form-horizontal" id="form1" name="form1" action="<?php echo $location; ?>" novalidate>
				...
				$Form_Inputs = new Form_Inputs();
				if($_SESSION['usuario']['basic_data']['idTipoUsuario']==1){
					$Form_Inputs->form_select_filter('Caja destino','idCajaChica', $x1, 1, 'idCajaChica', 'Nombre', 'caja_chica_listado', $y, '', $dbConn);
				}else{
					$Form_Inputs->form_select_join_filter('Caja destino','idCajaChica', $x1, 1, 'idCajaChica', 'Nombre', 'caja_chica_listado', 'usuarios_cajas_chicas', $y, $dbConn);
				}
				$Form_Inputs->form_date('Fecha Ingreso','Creacion_fecha', $x2, 1);
