...
$original = "comunicaciones_correos_filtro.php";
$location = $original;
...
if (!empty($_POST['submit'])){
	...
	$form_trabajo= 'email_masivo';
	require_once 'A1XRXS_sys/xrxs_form/z_comunicaciones_internas_email.php';
}
//se borra un dato
if (!empty($_GET['del'])){
	//Llamamos al formulario
...
if(!empty($_GET['submit_filter'])){
	...
	$SIS_join  = 'LEFT JOIN `usuarios_sistemas` ON usuarios_sistemas.idUsuario = usuarios_listado.idUsuario';
	$SIS_where = 'usuarios_listado.idEstado = 1 AND usuarios_listado.email!= ""';
	...
	$SIS_where .= ' GROUP BY usuarios_listado.idUsuario';
	$SIS_order = 0;
	$arrNotificaciones = array();
	$arrNotificaciones = db_select_array (false, $SIS_query, 'usuarios_listado', $SIS_join, $SIS_where, $SIS_order, $dbConn, $_SESSION['usuario']['basic_data']['Nombre'], $original, 'arrNotificaciones');
	...
	<div class="col-xs-12 col-sm-10 col-md-9 col-lg-8 fcenter">
		<div class="box">
			<header>
				...
				<h5>Crear Email Masivo para <?php echo $total_usr ?> usuarios</h5>
			</header>
			<div class="body">
				<form class="form-horizontal" method="post" id="form1" name="form1" autocomplete="off" novalidate>
					...
					if(isset($Asunto)){        $x1  = $Asunto;        }else{$x1  = '';}
					if(isset($Cuerpo)){        $x2  = $Cuerpo;        }else{$x2  = '';}

					//se dibujan los inputs
					$Form_Inputs = new Form_Inputs();
					$Form_Inputs->form_input_text('Asunto', 'Asunto', $x1, 2);
					$Form_Inputs->form_textarea('Mensaje','Cuerpo', $x2, 2);
...
<?php //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	if(isset($_GET['order_by'])&&$_GET['order_by']!=''){
		switch ($_GET['order_by']) {
			...
			case 'asunto_asc':    $order_by = 'Asunto ASC ';  $bread_order = '<i class="fa fa-sort-alpha-asc" aria-hidden="true"></i> Asunto Ascendente';break;
			case 'asunto_desc':   $order_by = 'Asunto DESC '; $bread_order = '<i class="fa fa-sort-alpha-desc" aria-hidden="true"></i> Asunto Descendente';break;
	...
	$SIS_where = "comunicaciones_internas_email.idEmail!=0";
	$SIS_where.= " AND comunicaciones_internas_email.idSistema=".$_SESSION['usuario']['basic_data']['idSistema'];//Verifico el tipo de usuario que esta ingresando

	/**********************************************************/
	//Realizo una consulta para saber el total de elementos existentes
	$cuenta_registros = db_select_nrows (false, 'idEmail', 'comunicaciones_internas_email', '', $SIS_where, $dbConn, $_SESSION['usuario']['basic_data']['Nombre'], $original, 'cuenta_registros');
	//Realizo la operacion para saber la cantidad de paginas que hay
	$total_paginas = ceil($cuenta_registros / $cant_reg);
	// Se trae un listado con todos los elementos
	$SIS_query = 'idEmail,Asunto, Fecha';
	$SIS_join  = '';
	$SIS_order = $order_by.' LIMIT '.$comienzo.', '.$cant_reg;
	$arrNotificaciones = array();
	$arrNotificaciones = db_select_array (false, $SIS_query, 'comunicaciones_internas_email', $SIS_join, $SIS_where, $SIS_order, $dbConn, $_SESSION['usuario']['basic_data']['Nombre'], $original, 'arrNotificaciones');
	...
	<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
		<div class="box">
			<header>
				<div class="icons"><i class="fa fa-table" aria-hidden="true"></i></div><h5>Listado de Email Enviados</h5>
				<div class="toolbar">
					<?php
				...
				<table id="dataTable" class="table table-bordered table-condensed table-hover table-striped dataTable">
					<thead>
						<tr role="row">
							...
							<th>
								<div class="pull-left">Asunto</div>
								<div class="btn-group pull-right" style="width: 50px;" >
									<a href="<?php echo $location.'&order_by=asunto_asc'; ?>" title="Ascendente" class="btn btn-default btn-xs tooltip"><i class="fa fa-sort-alpha-asc" aria-hidden="true"></i></a>
									<a href="<?php echo $location.'&order_by=asunto_desc'; ?>" title="Descendente" class="btn btn-default btn-xs tooltip"><i class="fa fa-sort-alpha-desc" aria-hidden="true"></i></a>
								</div>
							</th>
