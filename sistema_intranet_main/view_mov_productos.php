...
bodegas_productos_facturacion.idDocPago,
...
LEFT JOIN `usuarios_listado`     usuario_pago       ON usuario_pago.idUsuario                       = bodegas_productos_facturacion.idUsuarioPago
...
$rowData = db_select_data (false, $SIS_query, 'bodegas_productos_facturacion', $SIS_join, $SIS_where, $dbConn, $_SESSION['usuario']['basic_data']['Nombre'], basename($_SERVER["REQUEST_URI"], ".php"), 'rowData');
...
$SIS_where = 'pagos_facturas_proveedores.idFacturacion ='.$X_Puntero.' AND pagos_facturas_proveedores.idTipo=2';
...
$SIS_where = 'pagos_facturas_clientes.idFacturacion ='.$X_Puntero.' AND pagos_facturas_clientes.idTipo=2';
$SIS_order = 'sistema_documentos_pago.Nombre ASC';
$arrPagosClien = array();
...
$SIS_order = 'Nombre ASC';
$arrArchivo = array();
$arrArchivo = db_select_array (false, $SIS_query, 'bodegas_productos_facturacion_archivos', $SIS_join, $SIS_where, $SIS_order, $dbConn, $_SESSION['usuario']['basic_data']['Nombre'], basename($_SERVER["REQUEST_URI"], ".php"), 'arrArchivo');

/*****************************************/
// Se trae un listado con el historial
$SIS_query = '
bodegas_productos_facturacion_historial.Creacion_fecha, 
bodegas_productos_facturacion_historial.Observacion,
core_historial_tipos.Nombre,
core_historial_tipos.FonAwesome,
usuarios_listado.Nombre AS Usuario';
$SIS_join  = '
LEFT JOIN `core_historial_tipos`     ON core_historial_tipos.idTipo   = bodegas_productos_facturacion_historial.idTipo
LEFT JOIN `usuarios_listado`         ON usuarios_listado.idUsuario    = bodegas_productos_facturacion_historial.idUsuario';
$SIS_where = 'bodegas_productos_facturacion_historial.idFacturacion ='.$X_Puntero;
$SIS_order = 'bodegas_productos_facturacion_historial.idHistorial ASC';
$arrHistorial = array();
$arrHistorial = db_select_array (false, $SIS_query, 'bodegas_productos_facturacion_historial', $SIS_join, $SIS_where, $SIS_order, $dbConn, $_SESSION['usuario']['basic_data']['Nombre'], basename($_SERVER["REQUEST_URI"], ".php"), 'arrHistorial');
...
$SIS_join  = '
LEFT JOIN `ocompra_listado_documentos`  ON ocompra_listado_documentos.idOcompra   = bodegas_productos_facturacion.idOcompra
...
$arrDocRel = array();
$arrDocRel = db_select_array (false, $SIS_query, 'bodegas_productos_facturacion', $SIS_join, $SIS_where, $SIS_order, $dbConn, $_SESSION['usuario']['basic_data']['Nombre'], basename($_SERVER["REQUEST_URI"], ".php"), 'arrDocRel');
...
LEFT JOIN `core_documentos_mercantiles`  ON core_documentos_mercantiles.idDocumentos     = bodegas_productos_facturacion.idDocumentos
LEFT JOIN `usuarios_listado`             ON usuarios_listado.idUsuario                   = bodegas_productos_facturacion.idUsuario';
$SIS_where = 'bodegas_productos_facturacion.idFacturacionRelacionado ='.$X_Puntero;
$SIS_order = 'core_documentos_mercantiles.Nombre ASC';
$arrNotasCredito = array();
$arrNotasCredito = db_select_array (false, $SIS_query, 'bodegas_productos_facturacion', $SIS_join, $SIS_where, $SIS_order, $dbConn, $_SESSION['usuario']['basic_data']['Nombre'], basename($_SERVER["REQUEST_URI"], ".php"), 'arrNotasCredito');
							...
							<tbody role="alert" aria-live="polite" aria-relevant="all">
								<?php foreach ($arrDocRel as $doc) { ?>
									<tr class="odd">
										...
										<td align="right"><?php echo Valores($doc['Documento_valor'], 0); ?></td>
									</tr>
								<?php } ?>
							</tbody>
						</table>
...
<section class="invoice">
		...
		switch ($rowData['idTipo']) {
			...
			case 11:
				...
				<div class="col-xs-12 col-sm-4 col-md-4 col-lg-4 invoice-col">
					...
					<strong>Bodega Destino: </strong>'.$rowData['BodegaHacia'].'<br/>';
			...
			case 13:
				...
				<div class="col-xs-12 col-sm-4 col-md-4 col-lg-4 invoice-col">
					<strong>'.$rowData['Documento'].' N°'.$rowData['N_Doc'].'</strong><br/>
					<strong>Doc N°'.N_doc($rowData['idFacturacion'], 5).'</strong><br/>
					<strong>Bodega Origen: </strong>'.$rowData['BodegaDesde'].'<br/>
			...
			case 6:
				...
				<div class="col-xs-12 col-sm-4 col-md-4 col-lg-4 invoice-col">
					Empresa Destino
					<address>
						<strong>'.$rowData['SistemaDestino'].'</strong><br/>
						'.$rowData['SistemaDestinoCiudad'].' '.$rowData['SistemaDestinoComuna'].'<br/>
						'.$rowData['SistemaDestinoDireccion'].'<br/>
						Fono: '.formatPhone($rowData['SistemaDestinoFono']).'<br/>
						Fax: '.$rowData['SistemaDestinoFax'].'<br/>
						Rut: '.$rowData['SistemaDestinoRut'].'<br/>
						Email: '.$rowData['SistemaDestinoEmail'].'
					</address>
				</div>
			 
				<div class="col-xs-12 col-sm-4 col-md-4 col-lg-4 invoice-col">
					<strong>Doc N°'.N_doc($rowData['idFacturacion'], 5).'</strong><br/>
					<strong>Bodega de destino:</strong> '.$rowData['BodegaHacia'].'<br/>';
					if(isset($rowData['CentroCosto_Nombre'])&&$rowData['CentroCosto_Nombre']!=''){
						echo '<strong>Centro de Costo : </strong>'.$rowData['CentroCosto_Nombre'];
						if(isset($rowData['CentroCosto_Level_1'])&&$rowData['CentroCosto_Level_1']!=''){echo ' - '.$rowData['CentroCosto_Level_1'];}
						if(isset($rowData['CentroCosto_Level_2'])&&$rowData['CentroCosto_Level_2']!=''){echo ' - '.$rowData['CentroCosto_Level_2'];}
			...
			case 7:
				...
				<div class="col-xs-12 col-sm-4 col-md-4 col-lg-4 invoice-col">
					<strong>Doc N°'.N_doc($rowData['idFacturacion'], 5).'</strong><br/>
					<strong>Bodega utilizada:</strong> '.$rowData['BodegaDesde'].'<br/>
					<strong>Orden de Trabajo N°:</strong> '.N_doc($rowData['idOT'], 5).'<br/>';
					if(isset($rowData['CentroCosto_Nombre'])&&$rowData['CentroCosto_Nombre']!=''){
						echo '<strong>Centro de Costo : </strong>'.$rowData['CentroCosto_Nombre'];
						if(isset($rowData['CentroCosto_Level_1'])&&$rowData['CentroCosto_Level_1']!=''){echo ' - '.$rowData['CentroCosto_Level_1'];}
						if(isset($rowData['CentroCosto_Level_2'])&&$rowData['CentroCosto_Level_2']!=''){echo ' - '.$rowData['CentroCosto_Level_2'];}
	...
	<div class="row no-print">
		<div class="col-xs-12">
			<a target="new" href="view_mov_productos_to_print.php<?php echo $zz ?>" class="btn btn-default">
				<i class="fa fa-print" aria-hidden="true"></i> Imprimir
			</a>

			<a target="new" href="view_mov_productos_to_pdf.php<?php echo $zz ?>" class="btn btn-primary pull-right" style="margin-right: 5px;">
