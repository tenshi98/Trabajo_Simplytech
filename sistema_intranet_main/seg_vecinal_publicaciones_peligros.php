...
if(!empty($_GET['id'])){
	...
	/*******************************************************/
	// consulto los datos
	$SIS_query = 'idCliente, idTipo, idCiudad, idComuna, Direccion, Fecha, Hora, Descripcion, idEstado, idValidado';
	$SIS_join  = '';
	$SIS_where = 'idPeligro = '.$_GET['id'];
	$rowData = db_select_data (false, $SIS_query, 'seg_vecinal_peligros_listado', $SIS_join, $SIS_where, $dbConn, $_SESSION['usuario']['basic_data']['Nombre'], $original, 'rowData');
	/*******************************************************/
							...
							<tr class="odd">
								...
								<td>
									<div class="btn-group" style="width: 105px;" >
										<?php if ($rowlevel['level']>=1){ ?><a href="<?php echo 'view_seg_vecinal_peligro.php?view='.simpleEncode($eve['idPeligro'], fecha_actual()); ?>" title="Ver Información" class="iframe btn btn-primary btn-sm tooltip"><i class="fa fa-list" aria-hidden="true"></i></a><?php } ?>
										<?php if ($rowlevel['level']>=2){ ?><a href="<?php echo $location.'&id='.$eve['idPeligro']; ?>" title="Editar Información" class="btn btn-success btn-sm tooltip"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></a><?php } ?>
