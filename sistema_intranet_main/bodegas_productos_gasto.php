...
<?php //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	(SELECT SUM(Cantidad_ing) FROM `bodegas_productos_facturacion_existencias` WHERE idProducto = productos_listado.idProducto AND idBodega='.$_SESSION['productos_gasto_basicos']['idBodega'].' LIMIT 1) AS ingreso,
	(SELECT SUM(Cantidad_eg)  FROM `bodegas_productos_facturacion_existencias` WHERE idProducto = productos_listado.idProducto AND idBodega='.$_SESSION['productos_gasto_basicos']['idBodega'].' LIMIT 1) AS egreso';
	$SIS_join  = 'LEFT JOIN `sistema_productos_uml` ON sistema_productos_uml.idUml = productos_listado.idUml';
	$SIS_where = 'productos_listado.idProducto='.$_SESSION['productos_gasto_productos'][$_GET['editProd']]['idProducto'];
