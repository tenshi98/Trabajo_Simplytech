...
$original = "bodegas_insumos_ventas.php";
$location = $original;
...
if ( !empty($_POST['submit_prod']) )  { 
	//Llamamos al formulario
	$form_trabajo= 'new_prod_Ventas';
	require_once 'A1XRXS_sys/xrxs_form/z_bodega_insumos.php';
}
//formulario para crear
if ( !empty($_POST['submit_edit_prod']) )  { 
	//Llamamos al formulario
	$form_trabajo= 'edit_prod_Ventas';
	require_once 'A1XRXS_sys/xrxs_form/z_bodega_insumos.php';
}
//se borra un dato
if ( !empty($_GET['del_prod']) )     {
	//Llamamos al formulario
	$form_trabajo= 'del_prod_Ventas';
	require_once 'A1XRXS_sys/xrxs_form/z_bodega_insumos.php';	
}
/**********************************************/
//se borra un dato
if ( !empty($_GET['add_obs']) )     {
	//Llamamos al formulario
	$form_trabajo= 'add_obs_Ventas';
	require_once 'A1XRXS_sys/xrxs_form/z_bodega_insumos.php';	
}
//se borra un dato
if ( !empty($_GET['del_obs']) )     {
	//Llamamos al formulario
	$form_trabajo= 'del_obs_Ventas';
	require_once 'A1XRXS_sys/xrxs_form/z_bodega_insumos.php';	
}
/**********************************************/
//formulario para crear
if ( !empty($_POST['submit_guia']) )  { 
	//Llamamos al formulario
	$form_trabajo= 'new_guia_ventas';
	require_once 'A1XRXS_sys/xrxs_form/z_bodega_insumos.php';
}
//se borra un dato
if ( !empty($_GET['del_guia']) )     {
	//Llamamos al formulario
	$form_trabajo= 'del_guia_ventas';
	require_once 'A1XRXS_sys/xrxs_form/z_bodega_insumos.php';	
}
/**********************************************/
//formulario para crear
if ( !empty($_POST['submit_impuesto']) )  { 
	//Llamamos al formulario
	$form_trabajo= 'new_impuesto_ventas';
	require_once 'A1XRXS_sys/xrxs_form/z_bodega_insumos.php';
}
//se borra un dato
if ( !empty($_GET['del_impuesto']) )     {
	//Llamamos al formulario
	$form_trabajo= 'del_impuesto_ventas';
	require_once 'A1XRXS_sys/xrxs_form/z_bodega_insumos.php';	
}
/**********************************************/
//formulario para crear
if ( !empty($_POST['submit_file']) )  { 
	//Llamamos al formulario
	$form_trabajo= 'new_file_Ventas';
	require_once 'A1XRXS_sys/xrxs_form/z_bodega_insumos.php';
}
//se borra un dato
if ( !empty($_GET['del_file']) )     {
	//Llamamos al formulario
	$form_trabajo= 'del_file_Ventas';
	require_once 'A1XRXS_sys/xrxs_form/z_bodega_insumos.php';	
}
/**********************************************/
//formulario para crear
if ( !empty($_POST['submit_descuento']) )  { 
	//Llamamos al formulario
	$form_trabajo= 'new_desc_Ventas';
	require_once 'A1XRXS_sys/xrxs_form/z_bodega_insumos.php';
}
//formulario para editar
if ( !empty($_POST['submit_edit_descuento']) )  { 
	//Llamamos al formulario
	$form_trabajo= 'edit_desc_Ventas';
	require_once 'A1XRXS_sys/xrxs_form/z_bodega_insumos.php';
}
//se borra un dato
if ( !empty($_GET['del_descuento']) )     {
	//Llamamos al formulario
	$form_trabajo= 'del_desc_Ventas';
	require_once 'A1XRXS_sys/xrxs_form/z_bodega_insumos.php';	
}
/**********************************************/
//se borra un dato
if ( !empty($_GET['addfpago']) )     {
	//Llamamos al formulario
	$form_trabajo= 'addfpagoVentas';
	require_once 'A1XRXS_sys/xrxs_form/z_bodega_insumos.php';	
}
//se borra un dato
if ( !empty($_GET['delfpago']) )     {
	//Llamamos al formulario
	$form_trabajo= 'delfpagoVentas';
	require_once 'A1XRXS_sys/xrxs_form/z_bodega_insumos.php';	
}
/**********************************************/
if ( !empty($_GET['Venta_bodega']) )     {
	//Llamamos al formulario
	$form_trabajo= 'Venta_bodega';
	require_once 'A1XRXS_sys/xrxs_form/z_bodega_insumos.php';	
}
/**********************************************************************************************************************************/
	...
	<div class="box dark">
		...
		<div id="div-1" class="body">
			<form class="form-horizontal" method="post" id="form1" name="form1" novalidate>
				...
				if(isset($vTotal)) {      $x2  = $vTotal;     }else{$x2  = $_SESSION['insumos_vent_descuentos'][$_GET['editDescuentos']]['vTotal'];}
				...
				$Form_Imputs->form_input_hidden('oldidProducto', $_SESSION['insumos_vent_descuentos'][$_GET['editDescuentos']]['idDescuento'], 2);
				?>
...
sistema_productos_uml.Nombre AS Unimed,
	...
	AND idBodega=".$_SESSION['insumos_vent_basicos']['idBodega']." LIMIT 1) AS ingreso,
				
	(SELECT 
	SUM(Cantidad_eg) AS egreso
	FROM `bodegas_insumos_facturacion_existencias`
	WHERE idProducto = insumos_listado.idProducto 
	AND idBodega={$bodega} LIMIT 1) AS egreso
FROM `insumos_listado`
LEFT JOIN `sistema_productos_uml` ON sistema_productos_uml.idUml = insumos_listado.idUml
WHERE insumos_listado.idProducto='{$_SESSION['insumos_vent_productos'][$_GET['editProd']]['idProducto']}'
	...
	<div class="box dark">
		...
		<div id="div-1" class="body">
			<form class="form-horizontal" method="post" id="form1" name="form1" novalidate>
				...
				if(isset($Number)) {           $x2  = $Number;          }else{$x2  = $_SESSION['insumos_vent_productos'][$_GET['editProd']]['Number'];}
				if(isset($ValorTotal)) {       $x3  = $ValorTotal;      }else{$x3  = $_SESSION['insumos_vent_productos'][$_GET['editProd']]['ValorTotal'];}
				...
				$Form_Imputs->form_input_disabled('Valor Unitario','Unitario', Cantidades_decimales_justos($_SESSION['insumos_vent_productos'][$_GET['editProd']]['ValorEgreso']), 1);
				$Form_Imputs->form_input_number('Valor Total Neto', 'ValorTotal', $x3, 2);
				$Form_Imputs->form_input_hidden('ValorEgreso', Cantidades_decimales_justos($_SESSION['insumos_vent_productos'][$_GET['editProd']]['ValorEgreso']), 2);
				
				echo prod_print_venta($bodega, 'ValorEgreso', 'insumos_listado','bodegas_insumos_facturacion_existencias', 
...
<script>
	...
	function OnSelectionChange (select) {
		...
		FROM `bodegas_insumos_facturacion_existencias`
		WHERE idProducto = insumos_listado.idProducto 
		AND idBodega=".$_SESSION['insumos_vent_basicos']['idBodega'].") AS ingreso,
					
		(SELECT 
		SUM(Cantidad_eg) AS egreso
		FROM `bodegas_insumos_facturacion_existencias`
		WHERE idProducto = insumos_listado.idProducto 
		AND idBodega=".$_SESSION['insumos_vent_basicos']['idBodega'].") AS egreso
...
$z=" idDocumentos = 1 AND idEstado = 1 AND idCliente = {$_SESSION['insumos_vent_basicos']['idCliente']}";		 
?>
	...
	<div class="box dark">
		...
		<div id="div-1" class="body">
			<form class="form-horizontal" method="post" id="form1" name="form1" novalidate>
				...
				if(isset($idLevel_1)) {      $x2  = $idLevel_1;      }else{$x2  = $_SESSION['insumos_vent_basicos']['idLevel_1'];}
				if(isset($idLevel_2)) {      $x3  = $idLevel_2;      }else{$x3  = $_SESSION['insumos_vent_basicos']['idLevel_2'];}
				if(isset($idLevel_3)) {      $x4  = $idLevel_3;      }else{$x4  = $_SESSION['insumos_vent_basicos']['idLevel_3'];}
				if(isset($idLevel_4)) {      $x5  = $idLevel_4;      }else{$x5  = $_SESSION['insumos_vent_basicos']['idLevel_4'];}
				if(isset($idLevel_5)) {      $x6  = $idLevel_5;      }else{$x6  = $_SESSION['insumos_vent_basicos']['idLevel_5'];}
	...
	<div class="box dark">
		...
		<div id="div-1" class="body">
			<form class="form-horizontal" method="post" id="form1" name="form1" novalidate>
				...
				if(isset($idDocumentos)) {     $x2  = $idDocumentos;   }else{$x2  = $_SESSION['insumos_vent_basicos']['idDocumentos'];}
				if(isset($N_Doc)) {            $x3  = $N_Doc;          }else{$x3  = $_SESSION['insumos_vent_basicos']['N_Doc'];}
				if(isset($Creacion_fecha)) {   $x4  = $Creacion_fecha; }else{$x4  = $_SESSION['insumos_vent_basicos']['Creacion_fecha'];}
				if(isset($idBodega)) {         $x5  = $idBodega;       }else{$x5  = $_SESSION['insumos_vent_basicos']['idBodega'];}
				if(isset($idTrabajador)) {     $x6  = $idTrabajador;   }else{$x6  = $_SESSION['insumos_vent_basicos']['idTrabajador'];}
				if(isset($OC_Ventas)) {        $x7  = $OC_Ventas;      }else{$x7  = $_SESSION['insumos_vent_basicos']['OC_Ventas'];}
				...
				$Form_Imputs->form_select_join_filter('Bodega Origen','idBodega', $x5, 2, 'idBodega', 'Nombre', 'bodegas_insumos_listado', 'usuarios_bodegas_insumos', $z, $dbConn);
...
<div class="col-sm-12 fcenter" style="margin-bottom:30px">
	...
	<?php 
	$ubicacion = $location.'&view=true&Venta_bodega=true';
...
<div class="col-sm-12 fcenter">
	...
	<div id="page-wrap">
		<div id="header"> <?php echo $_SESSION['insumos_vent_basicos']['TipoDocumento']; ?></div>
			...
			<table id="meta" class="fleft otdata">
				<tbody>
					...
					<tr>
						...
						<td><?php echo $_SESSION['insumos_vent_basicos']['Cliente']; ?></td>
					</tr>
					<tr>
						<td class="meta-head">Documento</td>
						<td><?php echo $_SESSION['insumos_vent_basicos']['Documento'].' N°'.$_SESSION['insumos_vent_basicos']['N_Doc']?></td>
					</tr>
					<tr>
						<td class="meta-head">Bodega Origen</td>
						<td><?php echo $_SESSION['insumos_vent_basicos']['Bodega']; ?></td>
					</tr>
					<tr>
					...
					<tr>
						...
						<td><?php echo $_SESSION['insumos_vent_basicos']['CentroCosto']; ?></td>
					</tr>
				</tbody>
			</table>
		...
		<table id="items">
			<tbody>
				...
				<tr class="item-row fact_tittle">
					<td colspan="5">Insumos a Vender</td>
					<td><a href="<?php echo $location.'&addProd=true' ?>" title="Agregar Insumo" class="btn btn-xs btn-primary tooltip" style="position: initial;"><i class="fa fa-plus" aria-hidden="true"></i> Agregar Insumo</a></td>
				</tr>
				<?php 
				$vtotal_neto = 0;
				...
				?>
					...
					$_SESSION['insumos_vent_basicos']['valor_neto_imp'] = $vtotal_neto;
					?>
					<tr class="invoice-total" bgcolor="#f1f1f1">
					...
					if (isset($_SESSION['insumos_vent_impuestos'])){
						...
						$tempa = $vtotal_neto;
						//recorro el lsiatdo entregado por la base de datos
						foreach ($_SESSION['insumos_vent_impuestos'] as $key => $producto){
							...
							$_SESSION['insumos_vent_impuestos'][$producto['idImpuesto']]['valor'] = $vtotal_IVA;
					...
					//guardo el total
					$_SESSION['insumos_vent_basicos']['valor_total_fact'] = $vtotal_neto;
				...
				<tr>
					<?php if(isset($_SESSION['insumos_vent_basicos']['Observaciones'])&&$_SESSION['insumos_vent_basicos']['Observaciones']!=''){ ?>
					
						<td colspan="5" class="blank word_break"> 
							<?php echo $_SESSION['insumos_vent_basicos']['Observaciones'];?>
						</td>
						<td class="blank">
							<div class="btn-group" style="width: 35px;" >
								<?php 
					...
					<?php }else{?>
						<td colspan="5" class="blank"> 
							<?php 
							$non = '';
							if(isset($_SESSION['insumos_vent_temporal'])&&$_SESSION['insumos_vent_temporal']!=''){
								$non = $_SESSION['insumos_vent_temporal'];
							}	
...
//Verifico el tipo de usuario que esta ingresando
if($_SESSION['usuario']['basic_data']['idTipoUsuario']==1){
	$v = "bodegas_insumos_listado.idSistema>=0";
	$y = "idTipo=3 AND idSistema>=0 AND idEstado=1";
	$w="idSistema>=0 AND idEstado=1";
}else{
	$v = "bodegas_insumos_listado.idSistema={$_SESSION['usuario']['basic_data']['idSistema']} AND usuarios_bodegas_insumos.idUsuario = {$_SESSION['usuario']['basic_data']['idUsuario']}";	
	$y = "idTipo=3 AND idSistema={$_SESSION['usuario']['basic_data']['idSistema']} AND idEstado=1";
	$w="idSistema={$_SESSION['usuario']['basic_data']['idSistema']} AND idEstado=1";
}
/**********************************************************/
//Variable con la ubicacion
$z="WHERE bodegas_insumos_facturacion.idTipo=2";//Solo egresos
//Verifico el tipo de usuario que esta ingresando
...
if(isset($_GET['N_Doc']) && $_GET['N_Doc'] != ''){                    $z .= " AND bodegas_insumos_facturacion.N_Doc LIKE '%".$_GET['N_Doc']."%'";}
if(isset($_GET['Creacion_fecha']) && $_GET['Creacion_fecha'] != ''){  $z .= " AND bodegas_insumos_facturacion.Creacion_fecha='".$_GET['Creacion_fecha']."'";}
if(isset($_GET['idBodega']) && $_GET['idBodega'] != ''){              $z .= " AND bodegas_insumos_facturacion.idBodegaDestino=".$_GET['idBodega'];}
	...
	<div class="well">
		<div class="col-sm-8 fcenter">
			<form class="form-horizontal" id="form1" name="form1" action="<?php echo $location; ?>" novalidate>
				...
				$Form_Imputs->form_select_join_filter('Bodega Origen','idBodega', $x5, 1, 'idBodega', 'Nombre', 'bodegas_insumos_listado', 'usuarios_bodegas_insumos', $v, $dbConn);
