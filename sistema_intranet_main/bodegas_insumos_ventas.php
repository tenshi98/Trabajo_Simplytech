	...
	<div class="box dark">
		...
		<div id="div-1" class="body">
			<form class="form-horizontal" method="post" id="form1" name="form1" novalidate>
				...
				$Form_Inputs->form_input_disabled('Valor Unitario','Unitario', Cantidades_decimales_justos($_SESSION['insumos_vent_productos'][$_GET['editProd']]['ValorEgreso']), 1);
				$Form_Inputs->form_input_number('Valor Total Neto', 'ValorTotal', $x3, 2);
				$Form_Inputs->form_input_hidden('ValorEgreso', Cantidades_decimales_justos($_SESSION['insumos_vent_productos'][$_GET['editProd']]['ValorEgreso']), 2);
	...
	<div class="box dark">
		...
		<div id="div-1" class="body">
			<form class="form-horizontal" method="post" id="form1" name="form1" novalidate>
				...
				//se dibujan los inputs
				$Form_Inputs = new Form_Inputs();
				$Form_Inputs->form_select_filter('Guias disponibles','idGuia', $x1, 2, 'idFacturacion', 'N_Doc', 'bodegas_insumos_facturacion', $z, 'ORDER BY N_Doc ASC', $dbConn);
	...
	<div class="box dark">
		...
		<div id="div-1" class="body">
			<form class="form-horizontal" method="post" id="form1" name="form1" novalidate>
				...
				if(isset($fecha_fact_desde)) { $x5  = $fecha_fact_desde; }else{$x5  = $_SESSION['insumos_vent_basicos']['fecha_fact_desde'];}
				if(isset($fecha_fact_hasta)) { $x6  = $fecha_fact_hasta; }else{$x6  = $_SESSION['insumos_vent_basicos']['fecha_fact_hasta'];}
				if(isset($idBodega)) {         $x7  = $idBodega;         }else{$x7  = $_SESSION['insumos_vent_basicos']['idBodega'];}
				if(isset($idTrabajador)) {     $x8  = $idTrabajador;     }else{$x8  = $_SESSION['insumos_vent_basicos']['idTrabajador'];}
				if(isset($Observaciones)) {    $x9  = $Observaciones;    }else{$x9  = $_SESSION['insumos_vent_basicos']['Observaciones'];}
				if(isset($OC_Ventas)) {        $x10 = $OC_Ventas;        }else{$x10 = $_SESSION['insumos_vent_basicos']['OC_Ventas'];}
				if(isset($idUsoIVA)) {         $x11 = $idUsoIVA;         }else{$x11 = $_SESSION['insumos_vent_basicos']['idUsoIVA'];}
				...
				$Form_Inputs->form_select_join_filter('Bodega Origen','idBodega', $x7, 2, 'idBodega', 'Nombre', 'bodegas_insumos_listado', 'usuarios_bodegas_insumos', $z, $dbConn);
...
<div class="col-sm-12">
		...
		<div id="customer">
			...
			<table id="meta" class="fleft otdata">
				<tbody>
					...
					<?php if(isset($_SESSION['insumos_vent_basicos']['idUsoIVA'])&&$_SESSION['insumos_vent_basicos']['idUsoIVA']==1){ ?>
						<tr>
    ...
    <div class="row">
		<div class="col-xs-12">
			...
			<p class="text-muted well well-sm no-shadow" ><?php echo $_SESSION['insumos_vent_basicos']['Observaciones'];?></p>
		</div>
	</div>
...
/**********************************************************/
//Verifico el tipo de usuario que esta ingresando
$v = "bodegas_insumos_listado.idSistema=".$_SESSION['usuario']['basic_data']['idSistema'];
$y = "idTipo=3 AND idSistema=".$_SESSION['usuario']['basic_data']['idSistema']." AND idEstado=1";
$w = "idSistema=".$_SESSION['usuario']['basic_data']['idSistema']." AND idEstado=1";
if($_SESSION['usuario']['basic_data']['idTipoUsuario']!=1){
	$v .= " AND usuarios_bodegas_insumos.idUsuario = ".$_SESSION['usuario']['basic_data']['idUsuario'];	
}
/**********************************************************/
...
if(isset($_GET['idUsoIVA']) && $_GET['idUsoIVA'] != ''){               $z .= " AND bodegas_insumos_facturacion.idUsoIVA=".$_GET['idUsoIVA'];}
/**********************************************************/
	...
	<div class="well">
		<div class="col-sm-8 fcenter">
			<form class="form-horizontal" id="form1" name="form1" action="<?php echo $location; ?>" novalidate>
				...
				$Form_Inputs->form_select_join_filter('Bodega Origen','idBodega', $x7, 1, 'idBodega', 'Nombre', 'bodegas_insumos_listado', 'usuarios_bodegas_insumos', $v, $dbConn);
