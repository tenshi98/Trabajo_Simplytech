...
if (isset($_GET['created'])) {$error['usuario'] 	  = 'sucess/Geocerca creada correctamente';}
if (isset($_GET['edited']))  {$error['usuario'] 	  = 'sucess/Geocerca editada correctamente';}
if (isset($_GET['deleted'])) {$error['usuario'] 	  = 'sucess/Geocerca borrada correctamente';}
...
WHERE telemetria_geocercas.idZona = ".$_GET['edit_puntos'];
//Consulta
...
if(isset($_GET['order_by'])&&$_GET['order_by']!=''){
	switch ($_GET['order_by']) {
		case 'nombre_asc':    $order_by = 'ORDER BY telemetria_geocercas.Nombre ASC ';    $bread_order = '<i class="fa fa-sort-alpha-asc" aria-hidden="true"></i> Nombre Ascendente';break;
		case 'nombre_desc':   $order_by = 'ORDER BY telemetria_geocercas.Nombre DESC ';   $bread_order = '<i class="fa fa-sort-alpha-desc" aria-hidden="true"></i> Nombre Descendente';break;
		case 'estado_asc':    $order_by = 'ORDER BY telemetria_geocercas.idEstado ASC ';  $bread_order = '<i class="fa fa-sort-alpha-asc" aria-hidden="true"></i> Estado Ascendente';break;
		case 'estado_desc':   $order_by = 'ORDER BY telemetria_geocercas.idEstado DESC '; $bread_order = '<i class="fa fa-sort-alpha-desc" aria-hidden="true"></i> Estado Descendente';break;
		
		default: $order_by = 'ORDER BY telemetria_geocercas.Nombre ASC '; $bread_order = '<i class="fa fa-sort-alpha-asc" aria-hidden="true"></i> Nombre Ascendente';
	}
}else{
	$order_by = 'ORDER BY telemetria_geocercas.Nombre ASC '; $bread_order = '<i class="fa fa-sort-alpha-asc" aria-hidden="true"></i> Nombre Ascendente';
}
/**********************************************************/
//Variable de busqueda
$z = "WHERE telemetria_geocercas.idZona!=0";
//verifico que sea un administrador
$z.=" AND telemetria_geocercas.idSistema=".$_SESSION['usuario']['basic_data']['idSistema'];	

/**********************************************************/
//Se aplican los filtros
if(isset($_GET['Nombre']) && $_GET['Nombre'] != ''){             $z .= " AND telemetria_geocercas.Nombre LIKE '%".$_GET['Nombre']."%'";}
/**********************************************************/
//Realizo una consulta para saber el total de elementos existentes
$query = "SELECT telemetria_geocercas.idZona FROM `telemetria_geocercas` ".$z;
//Consulta
$resultado = mysqli_query ($dbConn, $query);
//Si ejecuto correctamente la consulta
if(!$resultado){
...
telemetria_geocercas.idEstado

FROM `telemetria_geocercas`
LEFT JOIN `core_estados`   ON core_estados.idEstado       = telemetria_geocercas.idEstado
LEFT JOIN `core_sistemas`  ON core_sistemas.idSistema     = telemetria_geocercas.idSistema
".$z."
".$order_by."
					...
					<tr class="odd">		
						...
						<td>
							<div class="btn-group" style="width: 140px;" >
								<?php if ($rowlevel['level']>=1){?><a href="<?php echo 'view_geocerca_telemetria.php?view='.simpleEncode($cerca['idZona'], fecha_actual()); ?>" title="Ver Informacion" class="iframe btn btn-primary btn-sm tooltip"><i class="fa fa-list" aria-hidden="true"></i></a><?php } ?>
