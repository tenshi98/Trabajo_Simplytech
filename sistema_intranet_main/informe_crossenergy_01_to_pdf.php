...
if(isset($f_inicio)&&$f_inicio!=''&&isset($f_termino)&&$f_termino!=''&&isset($h_inicio)&&$h_inicio!=''&&isset($h_termino)&&$h_termino!=''){
	$z.=" WHERE (telemetria_listado_crossenergy_dia.TimeStamp BETWEEN '".$f_inicio." ".$h_inicio."' AND '".$f_termino." ".$h_termino."')";
}elseif(isset($f_inicio)&&$f_inicio!=''&&isset($f_termino)&&$f_termino!=''){
	$z.=" WHERE (telemetria_listado_crossenergy_dia.FechaSistema BETWEEN '".$f_inicio."' AND '".$f_termino."')";
}
$z.=" AND telemetria_listado_crossenergy_dia.idTelemetria=".$idTelemetria;
...
$pdf_file       = 'Informe Resumen Dia grupo '.$rowGrupo['Nombre'].' del equipo '.$arrRutas[0]['NombreEquipo'].'.pdf';
$OpcDom         = "'A4', 'landscape'";
