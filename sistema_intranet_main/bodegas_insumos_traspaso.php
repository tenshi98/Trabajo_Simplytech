...
if(!empty($_GET['editProd'])){
	...
	(SELECT SUM(Cantidad_ing) FROM `bodegas_insumos_facturacion_existencias` WHERE idProducto = insumos_listado.idProducto AND idBodega='.$_SESSION['insumos_traspaso_basicos']['idBodegaOrigen'].' LIMIT 1) AS ingreso,
	(SELECT SUM(Cantidad_eg)  FROM `bodegas_insumos_facturacion_existencias` WHERE idProducto = insumos_listado.idProducto AND idBodega='.$_SESSION['insumos_traspaso_basicos']['idBodegaOrigen'].' LIMIT 1) AS egreso';
	$SIS_join  = 'LEFT JOIN `sistema_productos_uml` ON sistema_productos_uml.idUml = insumos_listado.idUml';
	$SIS_where = 'insumos_listado.idProducto='.$_SESSION['insumos_traspaso_productos'][$_GET['editProd']]['idProducto'];
		...
		<div class="box">
			...
			<div class="body">
				<form class="form-horizontal" method="post" id="form1" name="form1" autocomplete="off" novalidate>
					...
					if(isset($Observaciones)){    $x4  = $Observaciones;     }else{$x4  = $_SESSION['insumos_traspaso_basicos']['Observaciones'];}
	...
	<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
		...
		<div class="col-xs-12">
			<div class="row">
				...
				<p class="text-muted well well-sm no-shadow" ><?php echo $_SESSION['insumos_traspaso_basicos']['Observaciones']; ?></p>
			</div>
		</div>
