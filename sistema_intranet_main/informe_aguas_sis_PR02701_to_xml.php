...
<Sinar codigoArchivo="9" codigoProceso="3">
    <Empresa periodo="'.$periodo.'" rut="'.$rut.'">
        <Limite codigoLimite="7">
            <Localidad codigoComuna="13115" codigoLocalidad="393">';
				$total1   = 0;
				$total2   = 0;
				$clientes = 0;
				for ($i = 1; $i <= 5; $i++) {
					...
					//conpruebo
					if($comprobar!=0){
						$xmlstr .= '<Cliente tipoCliente="'.$i.'">';
						$xmlstr .= '<Servicio tipoServicio="3">';
						for ($x = 1; $x <= 18; $x++) {
							if(isset($informepr[$i][$x]['Cantidad'])&&$informepr[$i][$x]['Cantidad']!=''){
								...
								$xmlstr .= '
								<Rango codigoRango="'.$x.'">
									<MetrosCubicosAP>'.$informepr[$i][$x]['Medicion'].'</MetrosCubicosAP>
									<MetrosCubicosAS>'.$informepr[$i][$x]['Medicion'].'</MetrosCubicosAS>
									<CantidadClientes>'.$informepr[$i][$x]['Cantidad'].'</CantidadClientes>
								</Rango>';
							}
						}
						$xmlstr .= '</Servicio>';
