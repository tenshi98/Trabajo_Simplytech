...
$original = "bodegas_productos_egreso_nota_debito.php";
$location = $original;
...
if ( !empty($_GET['add_obs']) )     {
	//Llamamos al formulario
	$form_trabajo= 'add_obs_egr_nd';
	require_once 'A1XRXS_sys/xrxs_form/z_bodega_productos.php';	
}
//se borra un dato
if ( !empty($_GET['del_obs']) )     {
	//Llamamos al formulario
	$form_trabajo= 'del_obs_egr_nd';
	require_once 'A1XRXS_sys/xrxs_form/z_bodega_productos.php';	
}
/**********************************************/
//formulario para crear
...
WHERE productos_listado.idProducto='{$_SESSION['productos_egr_nd_productos'][$_GET['editProd']]['idProducto']}'
	...
	<div class="box dark">
		...
		<div id="div-1" class="body">
			<form class="form-horizontal" method="post" id="form1" name="form1" novalidate>
				...
				if(isset($idProducto)) {       $x1  = $idProducto;      }else{$x1  = $_SESSION['productos_egr_nd_productos'][$_GET['editProd']]['idProducto'];}
				if(isset($Cantidad_eg)) {           $x2  = $Cantidad_eg;          }else{$x2  = $_SESSION['productos_egr_nd_productos'][$_GET['editProd']]['Cantidad_eg'];}
				if(isset($ValorTotal)) {       $x3  = $ValorTotal;      }else{$x3  = $_SESSION['productos_egr_nd_productos'][$_GET['editProd']]['ValorTotal'];}
				...
				$Form_Imputs->form_input_disabled('Cliente Actual','cliente', $prov, 1);
				$Form_Imputs->form_input_disabled('Valor Unitario Neto','Unitario', Cantidades_decimales_justos($_SESSION['productos_egr_nd_productos'][$_GET['editProd']]['ValorIngreso']), 1);
				$Form_Imputs->form_input_number('Valor Total Neto', 'ValorTotal', $x3, 2);
				$Form_Imputs->form_input_hidden('ValorIngreso', Cantidades_decimales_justos($_SESSION['productos_egr_nd_productos'][$_GET['editProd']]['ValorIngreso']), 2);
...
<div style="display: none;"> 
	<div id="clone_producto" class="prod_container"> 	
		...
		<div class="col-sm-2 nopadding">
			<div class="form-group">
				<?php $Form_Imputs->input_disabled('text', 'Cliente Actual', 'escribeme2', 0, 1);?>
			</div>
		</div>
	...
	<div class="box dark">
		...
		<div id="div-1" class="body">
			<form class="form-horizontal" method="post" id="form1" name="form1" novalidate>
				...
				<?php 
				//Se verifican si existen los datos
				if(isset($Nombre)) {  $x1  = $Nombre;  }else{$x1  = $_SESSION['productos_egr_nd_otros'][$_GET['editOtros']]['Nombre'];}
				if(isset($vTotal)) {  $x2  = $vTotal;  }else{$x2  = Cantidades_decimales_justos($_SESSION['productos_egr_nd_otros'][$_GET['editOtros']]['vTotal']);}
				...
				$Form_Imputs->form_input_hidden('oldidProducto', $_SESSION['productos_egr_nd_otros'][$_GET['editOtros']]['idOtros'], 2);
	...
	<div class="box dark">
		...
		<div id="div-1" class="body">
			<form class="form-horizontal" method="post" id="form1" name="form1" novalidate>
				...
				<?php 
				//Se verifican si existen los datos
				if(isset($idCentroCosto)) {  $x1  = $idCentroCosto;  }else{$x1  = $_SESSION['productos_egr_nd_basicos']['idCentroCosto'];}
				if(isset($idLevel_1)) {      $x2  = $idLevel_1;      }else{$x2  = $_SESSION['productos_egr_nd_basicos']['idLevel_1'];}
				if(isset($idLevel_2)) {      $x3  = $idLevel_2;      }else{$x3  = $_SESSION['productos_egr_nd_basicos']['idLevel_2'];}
				if(isset($idLevel_3)) {      $x4  = $idLevel_3;      }else{$x4  = $_SESSION['productos_egr_nd_basicos']['idLevel_3'];}
				if(isset($idLevel_4)) {      $x5  = $idLevel_4;      }else{$x5  = $_SESSION['productos_egr_nd_basicos']['idLevel_4'];}
				if(isset($idLevel_5)) {      $x6  = $idLevel_5;      }else{$x6  = $_SESSION['productos_egr_nd_basicos']['idLevel_5'];}
	...
	<div class="box dark">
		...
		<div id="div-1" class="body">
			<form class="form-horizontal" method="post" id="form1" name="form1" novalidate>
				...
				<?php 
				//Se verifican si existen los datos
				if(isset($idCliente)) {          $x1  = $idCliente;        }else{$x1  = $_SESSION['productos_egr_nd_basicos']['idCliente'];}
				if(isset($N_Doc)) {              $x2  = $N_Doc;            }else{$x2  = $_SESSION['productos_egr_nd_basicos']['N_Doc'];}
				if(isset($Creacion_fecha)) {     $x3  = $Creacion_fecha;   }else{$x3  = $_SESSION['productos_egr_nd_basicos']['Creacion_fecha'];}
				if(isset($idBodega)) {           $x4  = $idBodega;         }else{$x4  = $_SESSION['productos_egr_nd_basicos']['idBodega'];}
				if(isset($Observaciones)) {      $x5  = $Observaciones;    }else{$x5  = $_SESSION['productos_egr_nd_basicos']['Observaciones'];}
				...
				$Form_Imputs->form_input_hidden('idTipo', 12, 2);
				$Form_Imputs->form_input_hidden('fecha_auto', fecha_actual(), 2);
				$Form_Imputs->form_input_hidden('idDocumentos', 4, 2);
...
<div class="col-sm-12 fcenter">
	...
	<div id="page-wrap">
		<div id="header"> <?php echo $_SESSION['productos_egr_nd_basicos']['TipoDocumento']; ?></div>
			...
			<table id="meta" class="fleft otdata">
				<tbody>
					...
					<tr>
						...
						<td><?php echo $_SESSION['productos_egr_nd_basicos']['Cliente']; ?></td>
					</tr>
					<tr>
						<td class="meta-head">Documento</td>
						<td><?php echo $_SESSION['productos_egr_nd_basicos']['Documento'].' N°'.$_SESSION['productos_egr_nd_basicos']['N_Doc']?></td>
					</tr>
					<tr>
						<td class="meta-head">Bodega</td>
						<td><?php echo $_SESSION['productos_egr_nd_basicos']['Bodega']; ?></td>
					</tr>
					<tr>
					...
					<tr>
						...
						<td><?php echo $_SESSION['productos_egr_nd_basicos']['CentroCosto']; ?></td>
					</tr>
				</tbody>
			</table>
			<table id="meta" class="otdata2">
				<tbody>
					<tr>
						...
						<td colspan="2"><?php echo Fecha_estandar($_SESSION['productos_egr_nd_basicos']['Creacion_fecha'])?></td>
					</tr>
				</tbody>
			</table>
		</div>
		<table id="items">
			<tbody>
				...
				<?php  //Guardo el neto
				$_SESSION['productos_egr_nd_basicos']['valor_neto_fact'] = $vtotal_neto;
				?>
					...
					$_SESSION['productos_egr_nd_basicos']['valor_neto_imp'] = $vtotal_neto;
					?>
					<tr class="invoice-total" bgcolor="#f1f1f1">
					...
					if (isset($_SESSION['productos_egr_nd_impuestos'])){
						...
						$tempa = $vtotal_neto;
						//recorro el lsiatdo entregado por la base de datos
						foreach ($_SESSION['productos_egr_nd_impuestos'] as $key => $producto){
							...
							$_SESSION['productos_egr_nd_impuestos'][$producto['idImpuesto']]['valor'] = $vtotal_IVA;
					...
					//guardo el total
					$_SESSION['productos_egr_nd_basicos']['valor_total_fact'] = $vtotal_neto;?>
					<tr class="invoice-total" bgcolor="#f1f1f1">
				...
				<tr>
					<?php if(isset($_SESSION['productos_egr_nd_basicos']['Observaciones'])&&$_SESSION['productos_egr_nd_basicos']['Observaciones']!=''){ ?>
					
						<td colspan="5" class="blank word_break"> 
							<?php echo $_SESSION['productos_egr_nd_basicos']['Observaciones'];?>
						</td>
						<td class="blank">
							<div class="btn-group" style="width: 35px;" >
								<?php 
					...
					<?php }else{?>
						<td colspan="5" class="blank"> 
							<?php 
							$non = '';
							if(isset($_SESSION['productos_egr_nd_temporal'])&&$_SESSION['productos_egr_nd_temporal']!=''){
								$non = $_SESSION['productos_egr_nd_temporal'];
							}	
...
$z="WHERE bodegas_productos_facturacion.idTipo=12";//Solo ingresos
//Verifico el tipo de usuario que esta ingresando
...
<div class="col-sm-12 breadcrumb-bar">
	...
	<?php if ($rowlevel['level']>=3){ ?>
		<?php if (isset($_SESSION['productos_egr_nd_basicos']['idCliente'])&&$_SESSION['productos_egr_nd_basicos']['idCliente']!=''){?>
	...
	<div class="well">
		<div class="col-sm-8 fcenter">
			<form class="form-horizontal" id="form1" name="form1" action="<?php echo $location; ?>" novalidate>
				...
				$Form_Imputs->form_select_join_filter('Bodega origen','idBodega', $x4, 1, 'idBodega', 'Nombre', 'bodegas_productos_listado', 'usuarios_bodegas_productos', $y, $dbConn);
				$Form_Imputs->form_textarea('Observaciones','Observaciones', $x5, 1, 160);
...
<div class="col-sm-12">
	<div class="box">
		<header>
			<div class="icons"><i class="fa fa-table"></i></div><h5>Listado de Notas de Debito</h5>
			<div class="toolbar">
				<?php 
