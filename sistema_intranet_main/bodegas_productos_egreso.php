...
$original = "bodegas_productos_egreso.php";
$location = $original;
...
if ( !empty($_POST['submit_modCentroCosto']) )  { 
	//Llamamos al formulario
	$form_trabajo= 'modCentroCosto_egr';
	require_once 'A1XRXS_sys/xrxs_form/z_bodega_productos.php';
}
/**********************************************/
//formulario para crear
	...
	<div class="box dark">
		...
		<div id="div-1" class="body">
			<form class="form-horizontal" method="post" id="form1" name="form1" novalidate>
				...
				if(isset($Nombre)) {      $x1  = $Nombre;     }else{$x1  = $_SESSION['productos_egr_descuentos'][$_GET['editDescuentos']]['Nombre'];}
				if(isset($vTotal)) {      $x2  = $vTotal;     }else{$x2  = $_SESSION['productos_egr_descuentos'][$_GET['editDescuentos']]['vTotal'];}
				...
				$Form_Imputs->form_input_hidden('oldidProducto', $_SESSION['productos_egr_descuentos'][$_GET['editDescuentos']]['idDescuento'], 2);
				?>
...
sistema_productos_uml.Nombre AS Unimed,
	...
	AND idBodega=".$_SESSION['productos_egr_basicos']['idBodega']." LIMIT 1) AS ingreso,
				
	(SELECT 
	SUM(Cantidad_eg) AS egreso
	FROM `bodegas_productos_facturacion_existencias`
	WHERE idProducto = productos_listado.idProducto 
	AND idBodega=".$_SESSION['productos_egr_basicos']['idBodega']." LIMIT 1) AS egreso
FROM `productos_listado`
LEFT JOIN `sistema_productos_uml` ON sistema_productos_uml.idUml = productos_listado.idUml
WHERE productos_listado.idProducto='{$_SESSION['productos_egr_productos'][$_GET['editProd']]['idProducto']}'
	...
	<div class="box dark">
		...
		<div id="div-1" class="body">
			<form class="form-horizontal" method="post" id="form1" name="form1" novalidate>
				...
				<?php 
				//Se verifican si existen los datos
				if(isset($idProducto)) {       $x1  = $idProducto;      }else{$x1  = $_SESSION['productos_egr_productos'][$_GET['editProd']]['idProducto'];}
				if(isset($Number)) {           $x2  = $Number;          }else{$x2  = $_SESSION['productos_egr_productos'][$_GET['editProd']]['Number'];}
				if(isset($ValorTotal)) {       $x3  = $ValorTotal;      }else{$x3  = $_SESSION['productos_egr_productos'][$_GET['editProd']]['ValorTotal'];}
				...
				$Form_Imputs->form_input_disabled('Valor Unitario','Unitario', Cantidades_decimales_justos($_SESSION['productos_egr_productos'][$_GET['editProd']]['ValorEgreso']), 1);
				$Form_Imputs->form_input_number('Valor Total Neto', 'ValorTotal', $x3, 2);
				$Form_Imputs->form_input_hidden('ValorEgreso', Cantidades_decimales_justos($_SESSION['productos_egr_productos'][$_GET['editProd']]['ValorEgreso']), 2);
...
<div style="display: none;"> 
	<div id="clone_producto" class="prod_container"> 	
		<div class="col-sm-2 nopadding">
			<div class="form-group">
				<?php $Form_Imputs->select_change('Producto','idProducto[]', 2, 'idProducto', 'Nombre', 'productos_listado', $zx1,'', 'OnSelectionChange',$dbConn); ?>	
			</div>
		</div>
		<div class="col-sm-2 nopadding">
			<div class="form-group">
		...
		<div class="col-sm-3 nopadding">
			<div class="form-group">
				<div class="input-group">
					<?php $Form_Imputs->input_number('Valor Total Neto','ValorTotal[]', '', 2);?>
...
<script>
	...
	function OnSelectionChange (select) {
		...
		FROM `bodegas_productos_facturacion_existencias`
		WHERE idProducto = productos_listado.idProducto 
		AND idBodega=".$_SESSION['productos_egr_basicos']['idBodega'].") AS ingreso,
					
		(SELECT 
		SUM(Cantidad_eg) AS egreso
		FROM `bodegas_productos_facturacion_existencias`
		WHERE idProducto = productos_listado.idProducto 
		AND idBodega=".$_SESSION['productos_egr_basicos']['idBodega'].") AS egreso
...
$z=" idDocumentos = 1 AND idEstado = 1 AND idCliente = {$_SESSION['productos_egr_basicos']['idCliente']}";		 
?>
	...
	<div class="box dark">
		...
		<div id="div-1" class="body">
			<form class="form-horizontal" method="post" id="form1" name="form1" novalidate>
				...
				//se dibujan los inputs
				$Form_Imputs = new Form_Inputs();
				$Form_Imputs->form_select_filter('Guias disponibles','idGuia', $x1, 2, 'idFacturacion', 'N_Doc', 'bodegas_productos_facturacion', $z, 'ORDER BY N_Doc ASC', $dbConn);
	...
	<div class="box dark">
		...
		<div id="div-1" class="body">
			<form class="form-horizontal" method="post" id="form1" name="form1" novalidate>
				...
				<?php 
				//Se verifican si existen los datos
				if(isset($idCentroCosto)) {  $x1  = $idCentroCosto;  }else{$x1  = $_SESSION['productos_egr_basicos']['idCentroCosto'];}
				if(isset($idLevel_1)) {      $x2  = $idLevel_1;      }else{$x2  = $_SESSION['productos_egr_basicos']['idLevel_1'];}
				if(isset($idLevel_2)) {      $x3  = $idLevel_2;      }else{$x3  = $_SESSION['productos_egr_basicos']['idLevel_2'];}
				if(isset($idLevel_3)) {      $x4  = $idLevel_3;      }else{$x4  = $_SESSION['productos_egr_basicos']['idLevel_3'];}
				if(isset($idLevel_4)) {      $x5  = $idLevel_4;      }else{$x5  = $_SESSION['productos_egr_basicos']['idLevel_4'];}
				if(isset($idLevel_5)) {      $x6  = $idLevel_5;      }else{$x6  = $_SESSION['productos_egr_basicos']['idLevel_5'];}
	...
	<div class="box dark">
		...
		<div id="div-1" class="body">
			<form class="form-horizontal" method="post" id="form1" name="form1" novalidate>
				...
				<?php 
				//Se verifican si existen los datos
				if(isset($idCliente)) {        $x1  = $idCliente;      }else{$x1  = $_SESSION['productos_egr_basicos']['idCliente'];}
				if(isset($idDocumentos)) {     $x2  = $idDocumentos;   }else{$x2  = $_SESSION['productos_egr_basicos']['idDocumentos'];}
				if(isset($N_Doc)) {            $x3  = $N_Doc;          }else{$x3  = $_SESSION['productos_egr_basicos']['N_Doc'];}
				if(isset($Creacion_fecha)) {   $x4  = $Creacion_fecha; }else{$x4  = $_SESSION['productos_egr_basicos']['Creacion_fecha'];}
				if(isset($idBodega)) {         $x5  = $idBodega;       }else{$x5  = $_SESSION['productos_egr_basicos']['idBodega'];}
				if(isset($idTrabajador)) {     $x6  = $idTrabajador;   }else{$x6  = $_SESSION['productos_egr_basicos']['idTrabajador'];}
				if(isset($OC_Ventas)) {        $x7  = $OC_Ventas;      }else{$x7  = $_SESSION['productos_egr_basicos']['OC_Ventas'];}
				...
				$Form_Imputs->form_select_join_filter('Bodega Origen','idBodega', $x5, 2, 'idBodega', 'Nombre', 'bodegas_productos_listado', 'usuarios_bodegas_productos', $z, $dbConn);
				$Form_Imputs->form_select('Vendedor','idTrabajador', $x6, 2, 'idTrabajador', 'Rut,Nombre,ApellidoPat,ApellidoMat', 'trabajadores_listado', $y, '', $dbConn);
				$Form_Imputs->form_input_text('OC Relacionada','OC_Ventas', $x7, 1);
				
				$Form_Imputs->form_input_disabled('Empresa Relacionada','fake_emp', $_SESSION['usuario']['basic_data']['RazonSocial'], 1);
				$Form_Imputs->form_input_hidden('idSistema', $_SESSION['usuario']['basic_data']['idSistema'], 2);
				$Form_Imputs->form_input_hidden('idUsuario', $_SESSION['usuario']['basic_data']['idUsuario'], 2);
				$Form_Imputs->form_input_hidden('idTipo', 2, 2);	
...
<div class="col-sm-12 fcenter">
	...
	<div id="page-wrap">
		<div id="header"> <?php echo $_SESSION['productos_egr_basicos']['TipoDocumento']; ?></div>
			...
			<table id="meta" class="fleft otdata">
				<tbody>
					...
					<tr>
						...
						<td><?php echo $_SESSION['productos_egr_basicos']['Cliente']; ?></td>
					</tr>
					<tr>
						<td class="meta-head">Documento</td>
						<td><?php echo $_SESSION['productos_egr_basicos']['Documento'].' N°'.$_SESSION['productos_egr_basicos']['N_Doc']?></td>
					</tr>
					<tr>
						<td class="meta-head">Bodega Origen</td>
						<td><?php echo $_SESSION['productos_egr_basicos']['Bodega']; ?></td>
					</tr>
					<tr>
						<td class="meta-head">Vendedor</td>
						<td><?php echo $_SESSION['productos_egr_basicos']['Vendedor']; ?></td>
					</tr>
					<?php if(isset($_SESSION['productos_egr_basicos']['OC_Ventas'])&&$_SESSION['productos_egr_basicos']['OC_Ventas']!=''){ ?>
						<tr>
							<td class="meta-head">Orden Compra Relacionada</td>
							<td><?php echo 'OC N°'.$_SESSION['productos_egr_basicos']['OC_Ventas']?></td>
						</tr>
					<?php } ?>
					<tr>
					...
					<tr>
						...
						<td><?php echo $_SESSION['productos_egr_basicos']['CentroCosto']; ?></td>
					</tr>
				</tbody>
			</table>
			<table id="meta" class="otdata2">
				<tbody>
					<tr>
						...
						<td colspan="2"><?php echo Fecha_estandar($_SESSION['productos_egr_basicos']['Creacion_fecha'])?></td>
					</tr>
					<?php if($_SESSION['productos_egr_basicos']['idDocumentos']==2){?>
						<tr>
							<td class="meta-head">Fecha Vencimiento</td>
							<?php if($_SESSION['productos_egr_basicos']['Pago_fecha']!='0000-00-00'){?>
								<td><?php echo Fecha_estandar($_SESSION['productos_egr_basicos']['Pago_fecha']);?></td>
								<td>
							...
							<?php }else{?>
								...
								<td>
									<div class="btn-group" style="width: 35px;" >
										...
										<a onclick="addfpago('<?php echo $ubicacion ?>')"  title="Asignar fecha de Vencimiento" class="btn btn-primary btn-sm tooltip"><i class="fa fa-check-square-o"></i></a>
									</div>	
								</td>
							<?php }?>
						</tr>
		...
		<table id="items">
			<tbody>
				...
				<tr class="item-row fact_tittle">
					<td colspan="5">Productos a Vender</td>
					...
					foreach ($_SESSION['productos_egr_productos'] as $key => $producto){ ?>
						<tr class="item-row linea_punteada">
							...
							<td class="item-name">
								...
								echo 'Total '.Valores(Cantidades_decimales_justos($producto['ValorTotal']), 0);?>
							</td>
							<td>
								<div class="btn-group" style="width: 70px;" >
									<a href="<?php echo $location.'&editProd='.$producto['idProducto']; ?>" title="Editar Producto" class="btn btn-success btn-sm tooltip"><i class="fa fa-pencil-square-o"></i></a>
									<?php 
									$ubicacion = $location.'&del_prod='.$producto['idProducto'];
									$dialogo   = '¿Realmente deseas eliminar el registro '.str_replace('"','',$producto['Nombre']).'?';?>
				...
				<?php  //Guardo el neto
				$_SESSION['productos_egr_basicos']['valor_neto_fact'] = $vtotal_neto;
				?>
					...
					$_SESSION['productos_egr_basicos']['valor_neto_imp'] = $vtotal_neto;
					?>
					<tr class="invoice-total" bgcolor="#f1f1f1">
					...
					if (isset($_SESSION['productos_egr_impuestos'])){
						...
						$tempa = $vtotal_neto;
						//recorro el lsiatdo entregado por la base de datos
						foreach ($_SESSION['productos_egr_impuestos'] as $key => $producto){
							...
							$_SESSION['productos_egr_impuestos'][$producto['idImpuesto']]['valor'] = $vtotal_IVA;
					...
					//guardo el total
					$_SESSION['productos_egr_basicos']['valor_total_fact'] = $vtotal_neto;
				...
				<tr>
					<?php if(isset($_SESSION['productos_egr_basicos']['Observaciones'])&&$_SESSION['productos_egr_basicos']['Observaciones']!=''){ ?>
					
						<td colspan="5" class="blank word_break"> 
							<?php echo $_SESSION['productos_egr_basicos']['Observaciones'];?>
						</td>
						<td class="blank">
							<div class="btn-group" style="width: 35px;" >
								<?php 
					...
					<?php }else{?>
						<td colspan="5" class="blank"> 
							<?php 
							$non = '';
							if(isset($_SESSION['productos_egr_temporal'])&&$_SESSION['productos_egr_temporal']!=''){
								$non = $_SESSION['productos_egr_temporal'];
							}	
	...
	<div class="box dark">
		...
		<div id="div-1" class="body">
			<form class="form-horizontal" method="post" id="form1" name="form1" novalidate>
				...
				if(isset($OC_Ventas)) {        $x8  = $OC_Ventas;      }else{$x8  = '';}
				...
				$Form_Imputs->form_input_text('OC Relacionada','OC_Ventas', $x8, 1);
...
if(isset($_GET['order_by'])&&$_GET['order_by']!=''){
	switch ($_GET['order_by']) {
		case 'bodega_asc':       $order_by = 'ORDER BY bodegas_productos_listado.Nombre ASC ';                $bread_order = '<i class="fa fa-sort-alpha-asc" aria-hidden="true"></i> Bodega Ascendente'; break;
		case 'bodega_desc':      $order_by = 'ORDER BY bodegas_productos_listado.Nombre DESC ';               $bread_order = '<i class="fa fa-sort-alpha-desc" aria-hidden="true"></i> Bodega Descendente';break;
		...
		case 'documento_asc':    $order_by = 'ORDER BY core_documentos_mercantiles.Nombre ASC ';              $bread_order = '<i class="fa fa-sort-alpha-asc" aria-hidden="true"></i> Documentos Ascendente';break;
		case 'documento_desc':   $order_by = 'ORDER BY core_documentos_mercantiles.Nombre DESC ';             $bread_order = '<i class="fa fa-sort-alpha-desc" aria-hidden="true"></i> Documentos Descendente';break;
...
//Verifico el tipo de usuario que esta ingresando
if($_SESSION['usuario']['basic_data']['idTipoUsuario']==1){
	$v = "bodegas_productos_listado.idSistema>=0";
	$y = "idTipo=3 AND idSistema>=0 AND idEstado=1";
	$w = "idSistema>=0 AND idEstado=1";
}else{
	$v = "bodegas_productos_listado.idSistema={$_SESSION['usuario']['basic_data']['idSistema']} AND usuarios_bodegas_productos.idUsuario = {$_SESSION['usuario']['basic_data']['idUsuario']}";	
	$y = "idTipo=3 AND idSistema={$_SESSION['usuario']['basic_data']['idSistema']} AND idEstado=1";
	$w = "idSistema={$_SESSION['usuario']['basic_data']['idSistema']} AND idEstado=1";
}
/**********************************************************/
//Variable con la ubicacion
$z="WHERE bodegas_productos_facturacion.idTipo=2";//Solo egresos
//Verifico el tipo de usuario que esta ingresando
...
if(isset($_GET['idBodega']) && $_GET['idBodega'] != ''){              $z .= " AND bodegas_productos_facturacion.idBodegaDestino=".$_GET['idBodega'];}
...
<div class="col-sm-12 breadcrumb-bar">
	...
	if (isset($_SESSION['productos_egr_basicos']['idCliente'])&&$_SESSION['productos_egr_basicos']['idCliente']!=''){?>
	...
	<div class="well">
		<div class="col-sm-8 fcenter">
			<form class="form-horizontal" id="form1" name="form1" action="<?php echo $location; ?>" novalidate>
				...
				$Form_Imputs->form_select_join_filter('Bodega Origen','idBodega', $x5, 1, 'idBodega', 'Nombre', 'bodegas_productos_listado', 'usuarios_bodegas_productos', $v, $dbConn);
			...
			<table id="dataTable" class="table table-bordered table-condensed table-hover table-striped dataTable">
				...
				<tbody role="alert" aria-live="polite" aria-relevant="all">
					...
					<tr class="odd">
						...
						<td>
							<div class="btn-group" style="width: 35px;" >
								<?php if ($rowlevel['level']>=1){?><a href="<?php echo 'view_mov_productos.php?view='.$tipo['idFacturacion']; ?>" title="Ver Informacion" class="iframe btn btn-primary btn-sm tooltip"><i class="fa fa-list"></i></a><?php } ?>
							</div>
						</td>
					</tr>
					<?php } ?>                    
