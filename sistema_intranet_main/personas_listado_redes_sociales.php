...
$location = $original;
$new_location = "personas_listado_redes_sociales.php";
...
if (!empty($_POST['submit'])){
	...
	$form_trabajo= 'insert';
	require_once 'A1XRXS_sys/xrxs_form/personas_listado_redes_sociales.php';
}
	...
	<div class="box dark">
		...
		<div class="body">
			<form class="form-horizontal" method="post" id="form1" name="form1" autocomplete="off" novalidate>
				...
				if(isset($URL)){          $x1 = $URL;         }else{$x1 = $rowdata['URL'];}
				if(isset($Comentario)){   $x2 = $Comentario;  }else{$x2 = $rowdata['Comentario'];}

				//se dibujan los inputs
				$Form_Inputs = new Form_Inputs();
				$Form_Inputs->form_input_text('URL', 'URL', $x1, 2);
				$Form_Inputs->form_textarea('Comentario', 'Comentario', $x2, 1);

				$Form_Inputs->form_input_hidden('idRedes', $_GET['edit'], 2);
...
// consulto los datos
$SIS_query = 'idRedes,URL,Comentario';
$SIS_join  = '';
$SIS_where = 'idPersona = '.$_GET['id'];
$SIS_order = 'URL ASC';
$arrURL = array();
	...
	<div class="box">
			...
			<ul class="nav nav-tabs pull-right">
				...
				<li class="dropdown">
					...
					<ul class="dropdown-menu" role="menu">
						...
						<li class="active"><a href="<?php echo 'personas_listado_redes_sociales.php?pagina='.$_GET['pagina'].'&id='.$_GET['id']?>" ><i class="fa fa-share-alt" aria-hidden="true"></i> Redes Sociales</a></li>
        ...
        <div class="table-responsive">
			<table id="dataTable" class="table table-bordered table-condensed table-hover table-striped dataTable">
				...
				<?php foreach ($arrURL as $fonos) { ?>
					<tr class="odd">
						<td><?php echo $fonos['URL']; ?></td>
						<td><?php echo cortar($fonos['Comentario'], 70); ?></td>
						<td>
							<div class="btn-group" style="width: 70px;" >
								<?php if ($rowlevel['level']>=2){ ?><a href="<?php echo $new_location.'&id='.$_GET['id'].'&edit='.$fonos['idRedes']; ?>" title="Editar Información" class="btn btn-success btn-sm tooltip"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></a><?php } ?>
