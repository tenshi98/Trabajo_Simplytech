...
$original = "pago_masivo_proveedor.php";
$location = $original;   
...
if (!empty($_GET['del_insumo_ex'])){
	...
	if(isset($_GET['idProveedor'])&&$_GET['idProveedor']!=''){  $location .= '&idProveedor='.$_GET['idProveedor'];}
	//Llamamos al formulario
	$form_trabajo= 'del_insumo_ex';
	require_once 'A1XRXS_sys/xrxs_form/z_pagos_proveedores.php';
}
//formulario para borrar
				...
				<div class="table-responsive">
					<table id="dataTable" class="table table-bordered table-condensed table-hover table-striped dataTable">
						...
						if(isset($_SESSION['pago_proveedor_insumos'])){ ?>
							...
							<?php foreach ($_SESSION['pago_proveedor_insumos'] as $key => $tipo){
								$Registro_total++; ?>
								<tr class="odd">
									<td>
...
<script>
	...
	function saveData(level, number, pago) {
		$.get("pago_masivo_proveedor_ok_save.php?type="+level+"&idFacturacion="+number+"&pago="+pago);
		return false;
	}
	//////////////////////////////////////////////////////////
	function delData(level, number) {
		$.get("pago_masivo_proveedor_ok_delete.php?type="+level+"&idFacturacion="+number);
		return false;
	}
	//////////////////////////////////////////////////////////
	function saveFact(level, number, value, text, valor) {
		$.get("pago_masivo_proveedor_fact_save.php?type="+level+"&idFacturacion="+number+"&value="+value+"&text="+text+"&valor="+valor);
		return false;
	}
	//////////////////////////////////////////////////////////
	function delFact(level, number, value, valor) {
		$.get("pago_masivo_proveedor_fac_delete.php?type="+level+"&idFacturacion="+number+"&value="+value+"&valor="+valor);
		return false;
	}
	//////////////////////////////////////////////////////////
...
if(isset($_GET['idProveedor'])&&$_GET['idProveedor']!=''){				
	$query = "SELECT Nombre
	FROM `proveedor_listado`
	WHERE idProveedor=".$_GET['idProveedor'];
	//Consulta
	$resultado = mysqli_query ($dbConn, $query);
	//Si ejecuto correctamente la consulta
	if(!$resultado){
...
$location .= '?submit_filter=true';
if(isset($_GET['idProveedor'])&&$_GET['idProveedor']!=''){
	$NDocsubmit_filter1 .= ' AND bodegas_insumos_facturacion.idProveedor='.$_GET['idProveedor'];
	$NDocsubmit_filter2 .= ' AND bodegas_productos_facturacion.idProveedor='.$_GET['idProveedor'];
	$NDocsubmit_filter3 .= ' AND bodegas_arriendos_facturacion.idProveedor='.$_GET['idProveedor'];
	$NDocsubmit_filter4 .= ' AND bodegas_servicios_facturacion.idProveedor='.$_GET['idProveedor'];
...
OR (bodegas_insumos_facturacion.idFacturacion>0
".$NDocsubmit_filter1."
AND bodegas_insumos_facturacion.idTipo=10
...
OR (bodegas_insumos_facturacion.idFacturacion>0
".$NDocsubmit_filter1."
AND bodegas_insumos_facturacion.idTipo=11
...
OR (bodegas_productos_facturacion.idFacturacion>0
".$NDocsubmit_filter2."
AND bodegas_productos_facturacion.idTipo=10
...
OR (bodegas_productos_facturacion.idFacturacion>0
".$NDocsubmit_filter2."
AND bodegas_productos_facturacion.idTipo=11
...
core_documentos_mercantiles.Nombre AS Documento,
(SELECT SUM(MontoPagado) FROM `pagos_facturas_proveedores` WHERE idFacturacion= bodegas_arriendos_facturacion.idFacturacion AND idTipo=4 LIMIT 1) AS MontoPagado
...
OR (bodegas_arriendos_facturacion.idFacturacion>0
".$NDocsubmit_filter3."
AND bodegas_arriendos_facturacion.idTipo=10
...
OR (bodegas_arriendos_facturacion.idFacturacion>0
".$NDocsubmit_filter3."
AND bodegas_arriendos_facturacion.idTipo=11
...
core_documentos_mercantiles.Nombre AS Documento,
(SELECT SUM(MontoPagado) FROM `pagos_facturas_proveedores` WHERE idFacturacion= bodegas_servicios_facturacion.idFacturacion AND idTipo=3 LIMIT 1) AS MontoPagado
...
OR (bodegas_servicios_facturacion.idFacturacion>0
".$NDocsubmit_filter4."
AND bodegas_servicios_facturacion.idTipo=10
...
OR (bodegas_servicios_facturacion.idFacturacion>0
".$NDocsubmit_filter4."
AND bodegas_servicios_facturacion.idTipo=11
...
<script>
	...
	function doSomething(level, number) {
		$.get("pago_masivo_proveedor_execute.php?type="+level+"&idFacturacion="+number);
		return false;
	}
