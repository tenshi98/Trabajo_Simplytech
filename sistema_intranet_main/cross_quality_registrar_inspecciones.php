...
WHERE idSistema={$_SESSION['cross_quality_reg_insp_basicos']['idSistema']}
AND idProceso = {$_SESSION['cross_quality_reg_insp_basicos']['idTipo']}
AND idCategoria = {$_SESSION['cross_quality_reg_insp_basicos']['idCategoria']}";
//Consulta
$resultado = mysqli_query ($dbConn, $query);
//Si ejecuto correctamente la consulta
if(!$resultado){
	...
	<div class="box dark">
		...
		<div id="div-1" class="body">
			<form class="form-horizontal" method="post" id="form1" name="form1" novalidate>
				...
				$Form_Imputs->form_select_filter('Productor','idProductor', $x1, 2, 'idProductor', 'Codigo,Nombre', 'productores_listado', $w, '', $dbConn);
				$Form_Imputs->form_input_text( 'N° Folio / Pallet', 'n_folio_pallet', $x2, 2);
				$Form_Imputs->form_select_filter('Tipo Embalaje','idTipo', $x3, 2, 'idTipo', 'Codigo,Nombre', 'sistema_cross_analisis_embalaje', $zx1, '', $dbConn);	
				$Form_Imputs->form_input_text( 'Lote', 'lote', $x4, 2);
				$Form_Imputs->form_date('Fecha Embalaje','f_embalaje', $x5, 2);
				$Form_Imputs->form_date('Fecha Cosecha','f_cosecha', $x6, 2);
				$Form_Imputs->form_time_popover('Hora Inspeccion','H_inspeccion', $x7, 1, 1, 24);
				...
				$Form_Imputs->form_input_hidden('oldidProducto', $_SESSION['cross_quality_reg_insp_muestras'][$_GET['editMuestra']]['idMuestra'], 2);
						...
						for ($i = 1; $i <= $_GET['cantPuntos']; $i++) {
							if($grupo['idGrupo']==$rowdata['PuntoidGrupo_'.$i]){
								...
								switch ($rowdata['PuntoidTipo_'.$i]) {
									//Medicion (Decimal) con parametros limitantes
									case 1:
										$Form_Imputs->form_input_number($rowdata['PuntoNombre_'.$i], 'Medida_'.$i, $sx_val, 1);
										break;
									//Medicion (Decimal) sin parametros limitantes
									case 2:
										$Form_Imputs->form_input_number($rowdata['PuntoNombre_'.$i], 'Medida_'.$i, $sx_val, 1);
									...
									//Medicion (Enteros) con parametros limitantes
									case 3:
										$Form_Imputs->form_input_number_integer($rowdata['PuntoNombre_'.$i], 'Medida_'.$i, $sx_val, 1);
										break;
									//Medicion (Enteros) sin parametros limitantes
									case 4:
										$Form_Imputs->form_input_number_integer($rowdata['PuntoNombre_'.$i], 'Medida_'.$i, $sx_val, 1);
									...
									//Fecha
									case 5:
										$Form_Imputs->form_date($rowdata['PuntoNombre_'.$i],'Medida_'.$i, $sx_val, 1);
										break;
									//Hora
									case 6:
										$Form_Imputs->form_time_popover($rowdata['PuntoNombre_'.$i],'Medida_'.$i, $sx_val, 1, 1, 24);
										break;
									//Texto Libre
									case 7:
										$Form_Imputs->form_textarea($rowdata['PuntoNombre_'.$i],'Medida_'.$i, $sx_val, 1, 160);
										break;
									//Seleccion 1 a 3
									case 8:
										$Form_Imputs->form_select_n_auto($rowdata['PuntoNombre_'.$i],'Medida_'.$i, $sx_val, 1, 1, 3);
										break;
									//Seleccion 1 a 5
									case 9:
										$Form_Imputs->form_select_n_auto($rowdata['PuntoNombre_'.$i],'Medida_'.$i, $sx_val, 1, 1, 5);
										break;
									//Seleccion 1 a 10
									case 10:
										$Form_Imputs->form_select_n_auto($rowdata['PuntoNombre_'.$i],'Medida_'.$i, $sx_val, 1, 1, 10);
										break;
									//Texto Libre con Validacion
									case 11:
										$Form_Imputs->form_input_validate($rowdata['PuntoNombre_'.$i],'Medida_'.$i, $sx_val, 1, $rowdata['Validacion_'.$i]);
										break;
	...
	<div id="page-wrap">
		...
		<table id="items">
			<tbody>
						...
						<tr class="item-row linea_punteada">
							...
							<td>
								<div class="btn-group" style="width: 35px;" >
									...
									<a onClick="dialogBox('<?php echo $ubicacion ?>', '<?php echo $dialogo ?>')" title="Borrar <?php echo $x_column_maquina_sing; ?>" class="btn btn-metis-1 btn-sm tooltip"><i class="fa fa-trash-o"></i></a>							
								</div>
							</td>
						</tr> 
					<?php }
							...
							<tr class="item-row linea_punteada">
								...
								<td>
									<div class="btn-group" style="width: 105px;" >
										<a href="<?php echo $location.'&cloneMuestra='.$producto['idMuestra'].'&cantPuntos='.$_SESSION['cross_quality_reg_insp_basicos']['cantPuntos'].'&idCalidad='.$_SESSION['cross_quality_reg_insp_basicos']['idMatriz']; ?>" title="Clonar Registro" class="btn btn-primary btn-sm tooltip"><i class="fa fa-files-o"></i></a>
										<a href="<?php echo $location.'&editMuestra='.$producto['idMuestra'].'&cantPuntos='.$_SESSION['cross_quality_reg_insp_basicos']['cantPuntos'].'&idCalidad='.$_SESSION['cross_quality_reg_insp_basicos']['idMatriz']; ?>" title="Editar Registro" class="btn btn-success btn-sm tooltip"><i class="fa fa-pencil-square-o"></i></a>
										<?php 
				...
				<tr>
					<?php if(isset($_SESSION['cross_quality_reg_insp_basicos']['Observaciones'])&&$_SESSION['cross_quality_reg_insp_basicos']['Observaciones']!=''){ ?>
					
						<td colspan="5" class="blank word_break"> 
							<?php echo $_SESSION['cross_quality_reg_insp_basicos']['Observaciones'];?>
						</td>
						<td class="blank">
							<div class="btn-group" style="width: 35px;" >
								<?php 
					...
					<?php }else{?>
						<td colspan="5" class="blank"> 
							<?php 
							$non = '';
							if(isset($_SESSION['cross_quality_reg_insp_temporal'])&&$_SESSION['cross_quality_reg_insp_temporal']!=''){
								$non = $_SESSION['cross_quality_reg_insp_temporal'];
							}	
	...
	<div class="well">
		<div class="col-sm-8 fcenter">
			<form class="form-horizontal" id="form1" name="form1" action="<?php echo $location; ?>" novalidate>
				...
				$Form_Imputs->form_select_depend1('Especie','idCategoria', $x4, 1, 'idCategoria', 'Nombre', 'sistema_variedades_categorias', $zx1, 0,
			...
			<table id="dataTable" class="table table-bordered table-condensed table-hover table-striped dataTable">
				...
				<tbody role="alert" aria-live="polite" aria-relevant="all">
					...
					<tr class="odd">
						...
						<td>
							<div class="btn-group" style="width: 70px;" >
								<?php if ($rowlevel['level']>=1){?><a href="<?php echo 'view_cross_quality_registrar_inspecciones.php?view='.$tipo['idAnalisis']; ?>" title="Ver Informacion" class="iframe btn btn-primary btn-sm tooltip"><i class="fa fa-list"></i></a><?php } ?>
								<?php if ($rowlevel['level']>=2){?><a href="<?php echo 'cross_quality_registrar_inspecciones_edit.php?edit='.$tipo['idAnalisis']; ?>" title="Editar Inspeccion" class="btn btn-success btn-sm tooltip"><i class="fa fa-pencil-square-o"></i></a><?php } ?>
							</div>
						</td>
					</tr>
					<?php } ?>                    
