...
$query = "SELECT idTrabajador,Creacion_fecha,Monto,Observacion,idSistema,idBonoTemporal
FROM `trabajadores_bonos_temporales`
WHERE idAnticipos = {$_GET['id']}";
//Consulta
...
<div class="col-sm-8 fcenter">
	<div class="box dark">
		<header>
			...
			<h5>Modificacion de la Asignacion de Bono</h5>
		</header>
		<div id="div-1" class="body">
			<form class="form-horizontal" method="post" id="form1" name="form1" novalidate>
				...
				$Form_Imputs->form_select_filter('Bono Asignado','idBonoTemporal', $x3, 2, 'idBonoTemporal', 'Nombre', 'sistema_bonos_temporales', 0, '', $dbConn);
				...
				$Form_Imputs->form_input_hidden('idAnticipos', $_GET['id'], 2);
...
if(isset($_GET['order_by'])&&$_GET['order_by']!=''){
	switch ($_GET['order_by']) {
		case 'fecha_asc':        $order_by = 'ORDER BY trabajadores_bonos_temporales.Creacion_fecha ASC ';                                                                 $bread_order = '<i class="fa fa-sort-alpha-asc" aria-hidden="true"></i> Fecha Facturacion Ascendente';break;
		case 'fecha_desc':       $order_by = 'ORDER BY trabajadores_bonos_temporales.Creacion_fecha DESC ';                                                                $bread_order = '<i class="fa fa-sort-alpha-desc" aria-hidden="true"></i> Fecha Facturacion Descendente';break;
		...
		case 'bono_asc':         $order_by = 'ORDER BY sistema_bonos_temporales.Nombre ASC ';                                                                              $bread_order = '<i class="fa fa-sort-alpha-asc" aria-hidden="true"></i> Bono Ascendente'; break;
		case 'bono_desc':        $order_by = 'ORDER BY sistema_bonos_temporales.Nombre DESC ';                                                                             $bread_order = '<i class="fa fa-sort-alpha-desc" aria-hidden="true"></i> Bono Descendente';break;
		case 'monto_asc':        $order_by = 'ORDER BY trabajadores_bonos_temporales.Monto ASC ';                                                                          $bread_order = '<i class="fa fa-sort-alpha-asc" aria-hidden="true"></i> Monto Ascendente'; break;
		case 'monto_desc':       $order_by = 'ORDER BY trabajadores_bonos_temporales.Monto DESC ';                                                                         $bread_order = '<i class="fa fa-sort-alpha-desc" aria-hidden="true"></i> Monto Descendente';break;
		
		default: $order_by = 'ORDER BY trabajadores_bonos_temporales.Creacion_fecha DESC '; $bread_order = '<i class="fa fa-sort-alpha-desc" aria-hidden="true"></i> Fecha Facturacion Descendente';
	}
}else{
	$order_by = 'ORDER BY trabajadores_bonos_temporales.Creacion_fecha DESC '; $bread_order = '<i class="fa fa-sort-alpha-desc" aria-hidden="true"></i> Fecha Facturacion Descendente';
}
/**********************************************************/
//Variable de busqueda
$z = "WHERE trabajadores_bonos_temporales.idAnticipos!=0";
//Verifico el tipo de usuario que esta ingresando
$w = "idSistema={$_SESSION['usuario']['basic_data']['idSistema']} AND idEstado=1";
...
if(isset($_GET['idTrabajador']) && $_GET['idTrabajador'] != ''){      $z .= " AND trabajadores_bonos_temporales.idTrabajador=".$_GET['idTrabajador'];}
if(isset($_GET['idUsuario']) && $_GET['idUsuario'] != ''){            $z .= " AND trabajadores_bonos_temporales.idUsuario=".$_GET['idUsuario'];}
if(isset($_GET['Creacion_fecha']) && $_GET['Creacion_fecha'] != ''){  $z .= " AND trabajadores_bonos_temporales.Creacion_fecha='".$_GET['Creacion_fecha']."'";}
if(isset($_GET['Ano']) && $_GET['Ano'] != ''){                        $z .= " AND trabajadores_bonos_temporales.Creacion_ano=".$_GET['Ano'];}
if(isset($_GET['idMes']) && $_GET['idMes'] != ''){                    $z .= " AND trabajadores_bonos_temporales.Creacion_mes=".$_GET['idMes'];}
if(isset($_GET['idBonoTemporal']) && $_GET['idBonoTemporal'] != ''){  $z .= " AND trabajadores_bonos_temporales.idBonoTemporal=".$_GET['idBonoTemporal'];}
/**********************************************************/
//Realizo una consulta para saber el total de elementos existentes
$query = "SELECT idAnticipos FROM `trabajadores_bonos_temporales` 
...
$arrInasHoras = array();
$query = "SELECT 
trabajadores_bonos_temporales.idAnticipos,
...
<div class="col-sm-12 breadcrumb-bar">
	...
	<?php if ($rowlevel['level']>=3){?><a href="<?php echo $location; ?>&new=true" class="btn btn-default fright margin_width fmrbtn" ><i class="fa fa-file-o" aria-hidden="true"></i> Crear Asignacion</a><?php } ?>
...
<div class="col-sm-12">
	<div class="box">
		<header>
			<div class="icons"><i class="fa fa-table"></i></div><h5>Listado de Bonos Temporales Asignados</h5>
			<div class="toolbar">
				<?php 
			...
			<table id="dataTable" class="table table-bordered table-condensed table-hover table-striped dataTable">
				<thead>
					<tr role="row">
						...
						<th>
							...
							<div class="btn-group pull-right" style="width: 50px;" >
								<a href="<?php echo $location.'&order_by=bono_asc'; ?>" title="Ascendente" class="btn btn-default btn-xs tooltip"><i class="fa fa-sort-alpha-asc"></i></a>
								<a href="<?php echo $location.'&order_by=bono_desc'; ?>" title="Descendente" class="btn btn-default btn-xs tooltip"><i class="fa fa-sort-alpha-desc"></i></a>
							</div>
						</th>
						<th>
				...
				<?php foreach ($arrInasHoras as $plan) { ?>
					<tr class="odd">
						...
						<td><?php echo valores($plan['Monto'], 0); ?></td>
						...
						<td>
							<div class="btn-group" style="width: 70px;" >
								...
								if(isset($plan['idUso'])&&$plan['idUso']==1){ ?>
									<?php if ($rowlevel['level']>=2){?><a href="<?php echo $location.'&id='.$plan['idAnticipos']; ?>" title="Editar Informacion" class="btn btn-success btn-sm tooltip"><i class="fa fa-pencil-square-o"></i></a><?php } ?>
									<?php if ($rowlevel['level']>=4){
										$ubicacion = $location.'&del='.$plan['idAnticipos'];
