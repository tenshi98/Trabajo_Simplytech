				...
				<tbody role="alert" aria-live="polite" aria-relevant="all">
					<?php foreach ($arrReportes as $eve) { ?>
						<tr class="odd">
							...
							<td>
								<div class="btn-group" style="width: 35px;" >
									<?php if ($rowlevel['level']>=2){ ?><a href="<?php echo $location.'&idEventoPeligro='.$eve['idEventoPeligro'].'&idComentario='.$eve['idComentario'].'&idTipofil='.$eve['idTipo']; ?>" title="Ver Información" class="btn btn-primary btn-sm tooltip"><i class="fa fa-list" aria-hidden="true"></i></a><?php } ?>
								</div>
							</td>
						</tr>
					<?php } ?>
