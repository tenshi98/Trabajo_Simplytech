...
$query = "SELECT idErrores FROM ".$tabla_1."  
ORDER BY idErrores DESC LIMIT 1 OFFSET ".$N_Reg_Errores;
//Consulta
...
$query = "SELECT idErrores FROM ".$tabla_1_backup." ORDER BY idErrores DESC LIMIT 1";
//Consulta
$resultado = mysqli_query ($dbConn, $query);
if($resultado !== FALSE){
...
foreach ($arrSeleccion as $tel) {
	...
	$query  = "INSERT INTO `".$tabla_1_backup."` (idErrores,
...
/**************************************************/
//elimino los datos
$query  = "DELETE FROM `".$tabla_1."` WHERE idErrores BETWEEN 1 AND ".$rowPuntero['idErrores'];
//Consulta
$resultado = mysqli_query ($dbConn, $query);
