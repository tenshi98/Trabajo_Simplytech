		...
		case 'del_pagos':
			...
			if ( empty($error) ) {
				...
				if(isset($_GET['del_idDocPago'])&&$_GET['del_idDocPago']!=''){
					$NDocsubmit_filter1 .= ' AND pagos_facturas_proveedores.idDocPago='.$_GET['del_idDocPago'];
				}
				if(isset($_GET['del_N_DocPago'])&&$_GET['del_N_DocPago']!=''){
					$NDocsubmit_filter1 .= ' AND pagos_facturas_proveedores.N_DocPago='.$_GET['del_N_DocPago'];
				}
					...
					foreach ($arrTipo1 as $tipo){
						...
						switch ($tipo['idTipo']) {
							...
							case 1:
								...
								$query  = "DELETE FROM `pagos_facturas_proveedores` WHERE idPago = '".$tipo['idPago']."'";
								//Consulta
								$resultado = mysqli_query ($dbConn, $query);
								//Si ejecuto correctamente la consulta
								if(!$resultado){
