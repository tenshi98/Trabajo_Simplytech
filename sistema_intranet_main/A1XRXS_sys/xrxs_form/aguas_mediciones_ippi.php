...
/*******************************************************************************************************************/		
		case 'insert':
			...
			if ( empty($error) ) {
				...
				$query  = "INSERT INTO `aguas_mediciones_ippi` (idSistema, idMes, Ano, Valor) 
				VALUES (".$a.")";
				//Consulta
...
/*******************************************************************************************************************/		
		case 'update':	
			...
			if ( empty($error) ) {
				...
				$resultado = db_update_data (false, $a, 'aguas_mediciones_ippi', 'idIPPI = "'.$idIPPI.'"', $dbConn, $_SESSION['usuario']['basic_data']['Nombre'], $original, $form_trabajo);
				//Si ejecuto correctamente la consulta
				if($resultado==true){
					//redirijo
