		...
		case 'del_pagos':
			...
			if ( empty($error) ) {
				...
				if(isset($_GET['del_idDocPago'])&&$_GET['del_idDocPago']!=''){
					$Docsubmit_filter .= ' AND pagos_boletas_proveedores.idDocPago='.$_GET['del_idDocPago'];
				}
				if(isset($_GET['del_N_DocPago'])&&$_GET['del_N_DocPago']!=''){
					$Docsubmit_filter .= ' AND pagos_boletas_proveedores.N_DocPago='.$_GET['del_N_DocPago'];
				}
				...
				$arrBoletas = array();
				$query = "SELECT 
				pagos_boletas_proveedores.idPago, 
				pagos_boletas_proveedores.idTipo, 
				pagos_boletas_proveedores.idFacturacion,
				pagos_boletas_proveedores.idFacturacion AS idddd,
				pagos_boletas_proveedores.MontoPagado,
				(SELECT SUM(MontoPagado) FROM `pagos_boletas_proveedores`WHERE idFacturacion = idddd AND idTipo=1 LIMIT 1)AS MontoTotal_1,
				(SELECT SUM(MontoPagado) FROM `pagos_boletas_proveedores`WHERE idFacturacion = idddd AND idTipo=2 LIMIT 1)AS MontoTotal_2,
				(SELECT SUM(MontoPagado) FROM `pagos_boletas_proveedores`WHERE idFacturacion = idddd AND idTipo=3 LIMIT 1)AS MontoTotal_3,
				(SELECT SUM(MontoPagado) FROM `pagos_boletas_proveedores`WHERE idFacturacion = idddd AND idTipo=4 LIMIT 1)AS MontoTotal_4
				FROM `pagos_boletas_proveedores`				
				LEFT JOIN `bodegas_arriendos_facturacion`  ON bodegas_arriendos_facturacion.idFacturacion    = pagos_boletas_proveedores.idFacturacion
				LEFT JOIN `boleta_honorarios_facturacion`    ON boleta_honorarios_facturacion.idFacturacion      = pagos_boletas_proveedores.idFacturacion
				LEFT JOIN `bodegas_productos_facturacion`  ON bodegas_productos_facturacion.idFacturacion    = pagos_boletas_proveedores.idFacturacion
				LEFT JOIN `bodegas_servicios_facturacion`  ON bodegas_servicios_facturacion.idFacturacion    = pagos_boletas_proveedores.idFacturacion
					...
					foreach ($arrBoletas as $tipo){
						...
						switch ($tipo['idTipo']) {
							...
							case 1:
								...
								$query  = "DELETE FROM `pagos_boletas_proveedores` WHERE idPago = '".$tipo['idPago']."'";
								//Consulta
								$resultado = mysqli_query ($dbConn, $query);
								//Si ejecuto correctamente la consulta
								if(!$resultado){
