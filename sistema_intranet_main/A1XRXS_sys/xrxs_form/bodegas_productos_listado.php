...
/*******************************************************************************************************************/		
		case 'insert':
			...
			if(isset($Nombre)&&isset($idSistema)){
				$ndata_1 = db_select_nrows ('Nombre', 'bodegas_productos_listado', '', "Nombre='".$Nombre."' AND idSistema='".$idSistema."'", $dbConn);
			}
...
/*******************************************************************************************************************/		
		case 'update':	
			...
			if(isset($Nombre)&&isset($idSistema)&&isset($idBodega)){
				$ndata_1 = db_select_nrows ('Nombre', 'bodegas_productos_listado', '', "Nombre='".$Nombre."' AND idSistema='".$idSistema."' AND idBodega!='".$idBodega."'", $dbConn);
			}
			//generacion de errores
			if($ndata_1 > 0) {$error['ndata_1'] = 'error/El nombre de la bodega ya existe en el sistema';}
			/*******************************************************************/
...
/*******************************************************************************************************************/
		case 'del_img':	
			...
			$rowdata = db_select_data ('Direccion_img', 'bodegas_productos_listado', '', "idBodega = ".$_GET['del_img'], $dbConn);
...
/*******************************************************************************************************************/
		case 'del_file':	
			...
			// Se obtiene el nombre del logo
			$rowdata = db_select_data ('FichaTecnica', 'bodegas_productos_listado', '', "idBodega = ".$_GET['del_file'], $dbConn);
...
/*******************************************************************************************************************/
		case 'del':	
			...
			// Se obtiene el nombre del logo
			$rowdata = db_select_data ('Direccion_img, FichaTecnica', 'bodegas_productos_listado', '', "idBodega = ".$_GET['del'], $dbConn);

			//se borra el dato de la base de datos
			$query  = "DELETE FROM `bodegas_productos_listado` WHERE idBodega = {$_GET['del']}";
			//Consulta
			$resultado = mysqli_query ($dbConn, $query);
			//Si ejecuto correctamente la consulta
			if($resultado){
