...
/*******************************************************************************************************************/		
		case 'insert':
			...
			if(isset($Nombre)){
				$ndata_1 = db_select_nrows ('Nombre', 'variedades_listado', '', "Nombre='".$Nombre."'", $dbConn);
			}
...
/*******************************************************************************************************************/		
		case 'update':	
			...
			if(isset($Nombre)&&isset($idProducto)){
				$ndata_1 = db_select_nrows ('Nombre', 'variedades_listado', '', "Nombre='".$Nombre."' AND idProducto!='".$idProducto."'", $dbConn);
			}
			//generacion de errores
			if($ndata_1 > 0) {$error['ndata_1'] = 'error/El Nombre ya existe en el sistema';}
			/*******************************************************************/
			...
			if ( empty($error) ) {
				...
				// inserto los datos de registro en la db
				$query  = "UPDATE `variedades_listado` SET ".$a." WHERE idProducto = '$idProducto'";
				//Consulta
				$resultado = mysqli_query ($dbConn, $query);
				//Si ejecuto correctamente la consulta
				if($resultado){
...
/*******************************************************************************************************************/
		case 'del_img':	
			...
			$query  = "UPDATE `variedades_listado` SET Direccion_img='', idTipoImagen=0 WHERE idProducto = '{$_GET['del_img']}'";
			//Consulta
			$resultado = mysqli_query ($dbConn, $query);
			//Si ejecuto correctamente la consulta
			if($resultado){
...
/*******************************************************************************************************************/
		case 'del_file':	
			...
			//se borra el dato de la base de datos
			$query  = "UPDATE `variedades_listado` SET FichaTecnica='' WHERE idProducto = '{$_GET['del_file']}'";
			//Consulta
			$resultado = mysqli_query ($dbConn, $query);
			//Si ejecuto correctamente la consulta
			if($resultado){
...
/*******************************************************************************************************************/
		case 'del_hds':	
			...
			//se borra el dato de la base de datos
			$query  = "UPDATE `variedades_listado` SET HDS='' WHERE idProducto = '{$_GET['del_hds']}'";
			//Consulta
			$resultado = mysqli_query ($dbConn, $query);
			//Si ejecuto correctamente la consulta
			if($resultado){
...
/*******************************************************************************************************************/
		case 'del':	
			...
			//se borra el dato de la base de datos
			$query  = "DELETE FROM `variedades_listado` WHERE idProducto = {$_GET['del']}";
			//Consulta
			$resultado = mysqli_query ($dbConn, $query);
			//Si ejecuto correctamente la consulta
			if($resultado){
...
/*******************************************************************************************************************/
		case 'estado':	
			...
			$idProducto  = $_GET['id'];
			$estado      = $_GET['estado'];
			$query  = "UPDATE variedades_listado SET idEstado = '$estado'	
