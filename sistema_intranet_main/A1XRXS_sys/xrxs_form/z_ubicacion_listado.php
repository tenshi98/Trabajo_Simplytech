...
/*******************************************************************************************************************/		
		case 'createBasicData':	
			...
			if(isset($Nombre)&&isset($idSistema)){
				$ndata_1 = db_select_nrows ('Nombre', 'ubicacion_listado', '', "Nombre='".$Nombre."' AND idSistema='".$idSistema."'", $dbConn);
			}
...
/*******************************************************************************************************************/		
		case 'updateBasicData':	
			...
			if(isset($Nombre)&&isset($idSistema)&&isset($idUbicacion)){
				$ndata_1 = db_select_nrows ('Nombre', 'ubicacion_listado', '', "Nombre='".$Nombre."' AND idSistema='".$idSistema."' AND idUbicacion!='".$idUbicacion."'", $dbConn);
			}
			//generacion de errores
			if($ndata_1 > 0) {$error['ndata_1'] = 'error/El Nombre de la licitacion ya existe en el sistema';}
			/*******************************************************************/
			...
			if ( empty($error) ) {
				...
				// inserto los datos de registro en la db
				$query  = "UPDATE `ubicacion_listado` SET ".$a." WHERE idUbicacion = '$idUbicacion'";
				//Consulta
				$resultado = mysqli_query ($dbConn, $query);
				//Si ejecuto correctamente la consulta
				if($resultado){
...
/*******************************************************************************************************************/
		case 'delBasicData':	
			...
			$query  = "DELETE FROM `ubicacion_listado` WHERE idUbicacion = {$_GET['del']}";
			//Consulta
			$resultado = mysqli_query ($dbConn, $query);
			//Si ejecuto correctamente la consulta
			if(!$resultado){
			...
			for ($i = 1; $i <= $nmax; $i++) {
				$query  = "DELETE FROM `ubicacion_listado_level_".$i."` WHERE idUbicacion = {$_GET['del']}";
				//Consulta
				$resultado = mysqli_query ($dbConn, $query);
				//Si ejecuto correctamente la consulta
				if(!$resultado){
...
/*******************************************************************************************************************/		
		case 'insert_item':
			...
			if(isset($lvl)&&isset($Nombre)&&isset($idUbicacion)&&isset($idSistema)&&isset($Codigo)){
				$ndata_1 = db_select_nrows ('Nombre', 'ubicacion_listado_level_'.$lvl, '', "Nombre='".$Nombre."' AND idUbicacion='".$idUbicacion."' AND idSistema='".$idSistema."' AND Codigo='".$Codigo."'", $dbConn);
			}
			//generacion de errores
...
/*******************************************************************************************************************/		
		case 'update_item':	
			...
			if ( empty($error) ) {
				...
				// inserto los datos de registro en la db
				$query  = "UPDATE `ubicacion_listado_level_".$lvl."` SET ".$a." WHERE idLevel_".$lvl." = '".$idLevel[$lvl]."'";
				//Consulta
				$resultado = mysqli_query ($dbConn, $query);
				//Si ejecuto correctamente la consulta
				if($resultado){
...
/*******************************************************************************************************************/
		case 'del_item':	
			...
			for ($i = $_GET['lvl']; $i <= $_GET['nmax']; $i++) {
				$query  = "DELETE FROM `ubicacion_listado_level_".$i."` WHERE idLevel_".$_GET['lvl']." = {$_GET['del_idLevel']}";
				//Consulta
				$resultado = mysqli_query ($dbConn, $query);
				//Si ejecuto correctamente la consulta
				if(!$resultado){
...
/*******************************************************************************************************************/
		case 'estado':	
			...
			$idUbicacion  = $_GET['id'];
			$estado       = $_GET['estado'];
			$query  = "UPDATE ubicacion_listado SET idEstado = '$estado'	
