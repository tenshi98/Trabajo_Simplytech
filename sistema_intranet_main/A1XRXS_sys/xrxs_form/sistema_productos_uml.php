...
if( ! defined('XMBCXRXSKGC')){
    die('No tienes acceso a esta carpeta o archivo (Access Code 1009-150).');
}
/*******************************************************************************************************************/
...
/*******************************************************************************************************************/
		case 'insert':
			...
			if(isset($Nombre)){
				$ndata_1 = db_select_nrows (false, 'Nombre', 'sistema_productos_uml', '', "Nombre='".$Nombre."'", $dbConn, $_SESSION['usuario']['basic_data']['Nombre'], $original, $form_trabajo);
			}
			...
			if(empty($error)){
				...
				$ultimo_id = db_insert_data (false, $SIS_columns, $SIS_data, 'sistema_productos_uml', $dbConn, $_SESSION['usuario']['basic_data']['Nombre'], $original, $form_trabajo);
...
/*******************************************************************************************************************/
		case 'update':
			...
			if(isset($Nombre, $idUml)){
				$ndata_1 = db_select_nrows (false, 'Nombre', 'sistema_productos_uml', '', "Nombre='".$Nombre."' AND idUml!='".$idUml."'", $dbConn, $_SESSION['usuario']['basic_data']['Nombre'], $original, $form_trabajo);
			}
			//generacion de errores
			if($ndata_1 > 0) {$error['ndata_1'] = 'error/El Nombre ingresado ya existe en el sistema';}
			/*******************************************************************/
			...
			if(empty($error)){
				...
				$resultado = db_update_data (false, $SIS_data, 'sistema_productos_uml', 'idUml = "'.$idUml.'"', $dbConn, $_SESSION['usuario']['basic_data']['Nombre'], $original, $form_trabajo);
				//Si ejecuto correctamente la consulta
				if($resultado==true){
					//redirijo
...
/*******************************************************************************************************************/
		case 'del':
			...
			if($errorn==0){
				//se borran los datos
				$resultado = db_delete_data (false, 'sistema_productos_uml', 'idUml = "'.$indice.'"', $dbConn, $_SESSION['usuario']['basic_data']['Nombre'], $original, $form_trabajo);
				//Si ejecuto correctamente la consulta
				if($resultado==true){
