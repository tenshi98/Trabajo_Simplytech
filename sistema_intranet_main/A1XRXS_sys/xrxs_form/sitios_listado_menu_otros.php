...
/*******************************************************************************************************************/
	...
	foreach ($INT_piezas as $INT_valor) {
		...
		switch ($INT_valor) {
			case 'idMenuOtros':   if(empty($idMenuOtros)){  $error['idMenuOtros']  = 'error/No ha ingresado el id';}break;
...
/*******************************************************************************************************************/
		case 'insert':
			...
			if(isset($idPosicion)&&isset($idSitio)){
				$ndata_1 = db_select_nrows (false, 'idMenuOtros', 'sitios_listado_menu_otros', '', "idPosicion='".$idPosicion."' AND idSitio='".$idSitio."'", $dbConn, $_SESSION['usuario']['basic_data']['Nombre'], $original, $form_trabajo);
			}
			if(isset($Nombre)&&isset($idSitio)){
				$ndata_2 = db_select_nrows (false, 'idMenuOtros', 'sitios_listado_menu_otros', '', "Nombre='".$Nombre."' AND idSitio='".$idSitio."'", $dbConn, $_SESSION['usuario']['basic_data']['Nombre'], $original, $form_trabajo);
			}
...
/*******************************************************************************************************************/
		case 'update':
			...
			$ndata_2 = 0;
			//Se verifica si el dato existe
			if(isset($idPosicion)&&isset($idSitio)&&isset($idMenuOtros)){
				$ndata_1 = db_select_nrows (false, 'idMenuOtros', 'sitios_listado_menu_otros', '', "idPosicion='".$idPosicion."' AND idSitio='".$idSitio."' AND idMenuOtros!='".$idMenuOtros."'", $dbConn, $_SESSION['usuario']['basic_data']['Nombre'], $original, $form_trabajo);
			}
			if(isset($Nombre)&&isset($idSitio)&&isset($idMenuOtros)){
				$ndata_2 = db_select_nrows (false, 'idMenuOtros', 'sitios_listado_menu_otros', '', "Nombre='".$Nombre."' AND idSitio='".$idSitio."' AND idMenuOtros!='".$idMenuOtros."'", $dbConn, $_SESSION['usuario']['basic_data']['Nombre'], $original, $form_trabajo);
			}
			//generacion de errores
			if($ndata_1 > 0) {$error['ndata_1'] = 'error/El Menu que intenta ingresar ya existe en el sistema';}
			if($ndata_2 > 0) {$error['ndata_2'] = 'error/El Menu que intenta ingresar ya existe en el sistema';}
