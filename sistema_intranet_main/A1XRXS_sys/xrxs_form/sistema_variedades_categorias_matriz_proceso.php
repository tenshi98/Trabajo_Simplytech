...
if( ! defined('XMBCXRXSKGC')){
    die('No tienes acceso a esta carpeta o archivo (Access Code 1009-157).');
}
/*******************************************************************************************************************/
...
/*******************************************************************************************************************/
		case 'insert_matriz':
			...
			if(isset($idCategoria)&&isset($idProceso)&&isset($idMatriz)&&isset($idSistema)){
				$ndata_1 = db_select_nrows (false, 'idVarMatriz', 'sistema_variedades_categorias_matriz_proceso', '', "idCategoria='".$idCategoria."' AND idMatriz='".$idMatriz."' AND idProceso='".$idProceso."' AND idSistema='".$idSistema."'", $dbConn, $_SESSION['usuario']['basic_data']['Nombre'], $original, $form_trabajo);
			}
			...
			if(empty($error)){
				...
				$ultimo_id = db_insert_data (false, $SIS_columns, $SIS_data, 'sistema_variedades_categorias_matriz_proceso', $dbConn, $_SESSION['usuario']['basic_data']['Nombre'], $original, $form_trabajo);
...
/*******************************************************************************************************************/
		case 'update_matriz':
			...
			if(isset($idCategoria)&&isset($idProceso)&&isset($idMatriz)&&isset($idSistema)&&isset($idVarMatriz)){
				$ndata_1 = db_select_nrows (false, 'idVarMatriz', 'sistema_variedades_categorias_matriz_proceso', '', "idCategoria='".$idCategoria."' AND idMatriz='".$idMatriz."' AND idProceso='".$idProceso."' AND idSistema='".$idSistema."' AND idVarMatriz!='".$idVarMatriz."'", $dbConn, $_SESSION['usuario']['basic_data']['Nombre'], $original, $form_trabajo);
			}
			//generacion de errores
			if($ndata_1 > 0) {$error['ndata_1'] = 'error/La matriz ya existe en el sistema';}
			/*******************************************************************/
			...
			if(empty($error)){
				...
				$resultado = db_update_data (false, $SIS_data, 'sistema_variedades_categorias_matriz_proceso', 'idVarMatriz = "'.$idVarMatriz.'"', $dbConn, $_SESSION['usuario']['basic_data']['Nombre'], $original, $form_trabajo);
				//Si ejecuto correctamente la consulta
				if($resultado==true){
					//redirijo
...
/*******************************************************************************************************************/
		case 'del_matriz':
			...
			if($errorn==0){
				//se borran los datos
				$resultado = db_delete_data (false, 'sistema_variedades_categorias_matriz_proceso', 'idVarMatriz = "'.$indice.'"', $dbConn, $_SESSION['usuario']['basic_data']['Nombre'], $original, $form_trabajo);
				//Si ejecuto correctamente la consulta
				if($resultado==true){
