...
/*******************************************************************************************************************/		
		case 'insert_matriz':
			...
			if(isset($idCategoria)&&isset($idProceso)&&isset($idMatriz)&&isset($idSistema)){
				$ndata_1 = db_select_nrows ('idVarMatriz', 'sistema_variedades_categorias_matriz_proceso', '', "idCategoria='".$idCategoria."' AND idMatriz='".$idMatriz."' AND idProceso='".$idProceso."' AND idSistema='".$idSistema."'", $dbConn);
			}
...
/*******************************************************************************************************************/		
		case 'update_matriz':	
			...
			if(isset($idCategoria)&&isset($idProceso)&&isset($idMatriz)&&isset($idSistema)&&isset($idVarMatriz)){
				$ndata_1 = db_select_nrows ('idVarMatriz', 'sistema_variedades_categorias_matriz_proceso', '', "idCategoria='".$idCategoria."' AND idMatriz='".$idMatriz."' AND idProceso='".$idProceso."' AND idSistema='".$idSistema."' AND idVarMatriz!='".$idVarMatriz."'", $dbConn);
			}
			//generacion de errores
			if($ndata_1 > 0) {$error['ndata_1'] = 'error/La matriz ya existe en el sistema';}
			/*******************************************************************/
...
/*******************************************************************************************************************/
		case 'del_matriz':	
			...
			$query  = "DELETE FROM `sistema_variedades_categorias_matriz_proceso` WHERE idVarMatriz = {$_GET['del']}";
			//Consulta
			$resultado = mysqli_query ($dbConn, $query);
			//Si ejecuto correctamente la consulta
			if($resultado){
