...
if( ! defined('XMBCXRXSKGC')){
    die('No tienes acceso a esta carpeta o archivo (Access Code 1009-089).');
}
/*******************************************************************************************************************/
...
/*******************************************************************************************************************/
		case 'insert':
			...
			if(empty($error)){
				...
				$SIS_columns = 'idLicitacion, idUsuario, Fecha, Observacion';
				$ultimo_id = db_insert_data (false, $SIS_columns, $SIS_data, 'licitacion_listado_observaciones', $dbConn, $_SESSION['usuario']['basic_data']['Nombre'], $original, $form_trabajo);
...
/*******************************************************************************************************************/
		case 'update':
			...
			if(empty($error)){
				...
				$resultado = db_update_data (false, $SIS_data, 'licitacion_listado_observaciones', 'idObservacion = "'.$idObservacion.'"', $dbConn, $_SESSION['usuario']['basic_data']['Nombre'], $original, $form_trabajo);
				//Si ejecuto correctamente la consulta
				if($resultado==true){
					//redirijo
...
/*******************************************************************************************************************/
		case 'del':
			...
			if($errorn==0){
				//se borran los datos
				$resultado = db_delete_data (false, 'licitacion_listado_observaciones', 'idObservacion = "'.$indice.'"', $dbConn, $_SESSION['usuario']['basic_data']['Nombre'], $original, $form_trabajo);
				//Si ejecuto correctamente la consulta
				if($resultado==true){
