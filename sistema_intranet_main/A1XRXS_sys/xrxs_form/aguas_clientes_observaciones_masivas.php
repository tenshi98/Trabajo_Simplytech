...
/*******************************************************************************************************************/
		case 'send_masivo':
			...
			if(empty($error)){
				...
				$SIS_data = "idSistema=".$_SESSION['usuario']['basic_data']['idSistema'];
				/**********************************************************/
				//Se aplican los filtros
				if(isset($idCliente) && $idCliente!=''){        $SIS_data .= " AND aguas_clientes_listado.idCliente=".$idCliente;}
				if(isset($idTipo) && $idTipo!=''){              $SIS_data .= " AND aguas_clientes_listado.idTipo=".$idTipo;}
				if(isset($idFacturable) && $idFacturable!=''){  $SIS_data .= " AND aguas_clientes_listado.idFacturable=".$idFacturable;}
				if(isset($idSector) && $idSector!=''){          $SIS_data .= " AND aguas_clientes_listado.idSector=".$idSector;}
				/**********************************************************/
				// Se trae un listado con todos los elementos
				$arrUsers = array();
				$arrUsers = db_select_array (false, 'idCliente', 'aguas_clientes_listado', 0, $SIS_data, 0, $dbConn, $_SESSION['usuario']['basic_data']['Nombre'], $original, $form_trabajo);
