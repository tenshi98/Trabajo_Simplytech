...
if( ! defined('XMBCXRXSKGC')){
    die('No tienes acceso a esta carpeta o archivo (Access Code 1009-132).');
}
/*******************************************************************************************************************/
...
/*******************************************************************************************************************/
		case 'insert':
			...
			if(isset($idUsuario)&&isset($idSistema)){
				$ndata_1 = db_select_nrows (false, 'idUsuario', 'sistema_aprobador_oc', '', "idUsuario='".$idUsuario."' AND idSistema='".$idSistema."'", $dbConn, $_SESSION['usuario']['basic_data']['Nombre'], $original, $form_trabajo);
			}
...
/*******************************************************************************************************************/
		case 'update':
			...
			if(isset($idUsuario)&&isset($idSistema)&&isset($idAprobador)){
				$ndata_1 = db_select_nrows (false, 'idUsuario', 'sistema_aprobador_oc', '', "idUsuario='".$idUsuario."' AND idSistema='".$idSistema."' AND idAprobador!='".$idAprobador."'", $dbConn, $_SESSION['usuario']['basic_data']['Nombre'], $original, $form_trabajo);
			}
			//generacion de errores
			if($ndata_1 > 0) {$error['ndata_1'] = 'error/El Usuario ya existe';}
			/*******************************************************************/
...
/*******************************************************************************************************************/
		case 'del':
			...
			if($errorn==0){
				//se borran los datos
				$resultado = db_delete_data (false, 'sistema_aprobador_oc', 'idAprobador = "'.$indice.'"', $dbConn, $_SESSION['usuario']['basic_data']['Nombre'], $original, $form_trabajo);
				//Si ejecuto correctamente la consulta
				if($resultado==true){
