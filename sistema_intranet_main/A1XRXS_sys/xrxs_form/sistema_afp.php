...
/*******************************************************************************************************************/
	...
	foreach ($piezas as $valor) {
		...
		switch ($valor) {
			...
			case 'PorcentajeDependiente':    if(empty($PorcentajeDependiente)){    $error['PorcentajeDependiente']    = 'error/No ha ingresado el porcentaje para trabajadores dependientes';}break;
			case 'PorcentajeIndependiente':  if(empty($PorcentajeIndependiente)){  $error['PorcentajeIndependiente']  = 'error/No ha ingresado el porcentaje para trabajadores independientes';}break;
...
/*******************************************************************************************************************/		
		case 'insert':
			...
			if(isset($Nombre)){
				$ndata_1 = db_select_nrows ('Nombre', 'sistema_afp', '', "Nombre='".$Nombre."'", $dbConn);
			}
...
/*******************************************************************************************************************/		
		case 'update':	
			...
			if(isset($Nombre)&&isset($idAFP)){
				$ndata_1 = db_select_nrows ('Nombre', 'sistema_afp', '', "Nombre='".$Nombre."' AND idAFP!='".$idAFP."'", $dbConn);
			}
			//generacion de errores
			if($ndata_1 > 0) {$error['ndata_1'] = 'error/El nombre ya existe en el sistema';}
			/*******************************************************************/
...
/*******************************************************************************************************************/
		case 'del':	
			...
			$query  = "DELETE FROM `sistema_afp` WHERE idAFP = {$_GET['del']}";
			//Consulta
			$resultado = mysqli_query ($dbConn, $query);
			//Si ejecuto correctamente la consulta
			if($resultado){
