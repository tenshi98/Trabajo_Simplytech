...
/*******************************************************************************************************************/		
		case 'insert':
			...
			if(isset($Creacion_fecha)&&isset($idTrabajador)&&isset($idSistema)){
				$ndata_1 = db_select_nrows ('Creacion_fecha', 'trabajadores_descuentos_anticipos', '', "Creacion_fecha='".$Creacion_fecha."' AND idTrabajador='".$idTrabajador."' AND idSistema='".$idSistema."'", $dbConn);
			}
			...
			if ( empty($error) ) {
				...
				$query  = "INSERT INTO `trabajadores_descuentos_anticipos` (idSistema, idTrabajador, idUsuario,
				fecha_auto, Creacion_fecha, Creacion_Semana, Creacion_mes, Creacion_ano, Monto,
				Observacion, idUso) 
				VALUES ({$a} )";
				//Consulta
...
/*******************************************************************************************************************/		
		case 'update':	
			...
			if(isset($Creacion_fecha)&&isset($idTrabajador)&&isset($idAnticipos)&&isset($idSistema)){
				$ndata_1 = db_select_nrows ('Creacion_fecha', 'trabajadores_descuentos_anticipos', '', "Creacion_fecha='".$Creacion_fecha."' AND idTrabajador='".$idTrabajador."' AND idSistema='".$idSistema."' AND idAnticipos!='".$idAnticipos."'", $dbConn);
			}
			//generacion de errores
			if($ndata_1 > 0) {$error['ndata_1'] = 'error/El anticipo ya existe en el sistema';}
			/*******************************************************************/
			...
			if ( empty($error) ) {
				...
				// inserto los datos de registro en la db
				$query  = "UPDATE `trabajadores_descuentos_anticipos` SET ".$a." WHERE idAnticipos = '$idAnticipos'";
				//Consulta
				$resultado = mysqli_query ($dbConn, $query);
				//Si ejecuto correctamente la consulta
				if($resultado){
...
/*******************************************************************************************************************/
		case 'del':	
			...
			$query  = "DELETE FROM `trabajadores_descuentos_anticipos` WHERE idAnticipos = {$_GET['del']}";
			//Consulta
			$resultado = mysqli_query ($dbConn, $query);
			//Si ejecuto correctamente la consulta
			if($resultado){
