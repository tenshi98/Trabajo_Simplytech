...
/*******************************************************************************************************************/		
		case 'insert':
			...
			if(isset($Nombre)&&isset($idSistema)){
				$ndata_1 = db_select_nrows ('Nombre', 'sistema_cross_analisis_embalaje', '', "Nombre='".$Nombre."' AND idSistema='".$idSistema."'", $dbConn);
			}
...
/*******************************************************************************************************************/		
		case 'update':	
			...
			if(isset($Nombre)&&isset($idSistema)&&isset($idTipo)){
				$ndata_1 = db_select_nrows ('Nombre', 'sistema_cross_analisis_embalaje', '', "Nombre='".$Nombre."' AND idSistema='".$idSistema."' AND idTipo!='".$idTipo."'", $dbConn);
			}
			//generacion de errores
			if($ndata_1 > 0) {$error['ndata_1'] = 'error/El Nombre ingresado ya existe en el sistema';}
			/*******************************************************************/
			...
			if ( empty($error) ) {
				...
				// inserto los datos de registro en la db
				$query  = "UPDATE `sistema_cross_analisis_embalaje` SET ".$a." WHERE idTipo = '$idTipo'";
				//Consulta
				$resultado = mysqli_query ($dbConn, $query);
				//Si ejecuto correctamente la consulta
				if($resultado){
...
/*******************************************************************************************************************/
		case 'del':	
			...
			$query  = "DELETE FROM `sistema_cross_analisis_embalaje` WHERE idTipo = {$_GET['del']}";
			//Consulta
			$resultado = mysqli_query ($dbConn, $query);
			//Si ejecuto correctamente la consulta
			if($resultado){
