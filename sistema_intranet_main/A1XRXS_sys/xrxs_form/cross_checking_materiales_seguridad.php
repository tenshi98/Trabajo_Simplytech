...
/*******************************************************************************************************************/
	...
	foreach ($INT_piezas as $INT_valor) {
		...
		switch ($INT_valor) {
			case 'idMatSeguridad':   if(empty($idMatSeguridad)){    $error['idMatSeguridad']     = 'error/No ha ingresado el id';}break;
...
/*******************************************************************************************************************/		
		case 'insert':
			...
			if(isset($Nombre)){
				$ndata_1 = db_select_nrows (false, 'Nombre', 'cross_checking_materiales_seguridad', '', "Nombre='".$Nombre."'", $dbConn, $_SESSION['usuario']['basic_data']['Nombre'], $original, $form_trabajo);
			}
...
/*******************************************************************************************************************/		
		case 'update':	
			...
			$ndata_1 = 0;
			//Se verifica si el dato existe
			if(isset($Nombre)&&isset($idMatSeguridad)){
				$ndata_1 = db_select_nrows (false, 'Nombre', 'cross_checking_materiales_seguridad', '', "Nombre='".$Nombre."' AND idMatSeguridad!='".$idMatSeguridad."'", $dbConn, $_SESSION['usuario']['basic_data']['Nombre'], $original, $form_trabajo);
			}
			//generacion de errores
			if($ndata_1 > 0) {$error['ndata_1'] = 'error/El Nombre ya existe en el sistema';}
			/*******************************************************************/
			...
			if ( empty($error) ) {
				...
				// inserto los datos de registro en la db
				$query  = "UPDATE `cross_checking_materiales_seguridad` SET ".$a." WHERE idMatSeguridad = '$idMatSeguridad'";
				//Consulta
				$resultado = mysqli_query ($dbConn, $query);
				//Si ejecuto correctamente la consulta
				if($resultado){
...
/*******************************************************************************************************************/
		case 'del':	
			...
			if($errorn==0){
				//se borran los datos
				$resultado = db_delete_data (false, 'cross_checking_materiales_seguridad', 'idMatSeguridad = "'.$indice.'"', $dbConn, $_SESSION['usuario']['basic_data']['Nombre'], $original, $form_trabajo);
				//Si ejecuto correctamente la consulta
				if($resultado==true){
