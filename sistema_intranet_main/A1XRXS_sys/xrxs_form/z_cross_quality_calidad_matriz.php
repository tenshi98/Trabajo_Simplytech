	...
	switch ($form_trabajo) {
		...
		case 'insert_matriz':	
			...
			if(isset($Nombre)&&isset($idSistema)&&isset($idTipo)){
				$ndata_1 = db_select_nrows ('Nombre', 'cross_quality_calidad_matriz', '', "Nombre='".$Nombre."' AND idSistema='".$idSistema."' AND idTipo='".$idTipo."'", $dbConn);
			}
...
/*******************************************************************************************************************/		
		case 'update_matriz':
			...
			if(isset($Nombre)&&isset($idSistema)&&isset($idTipo)&&isset($idMatriz)){
				$ndata_1 = db_select_nrows ('Nombre', 'cross_quality_calidad_matriz', '', "Nombre='".$Nombre."' AND idSistema='".$idSistema."' AND idTipo='".$idTipo."' AND idMatriz!='".$idMatriz."'", $dbConn);
			}
			//generacion de errores
			if($ndata_1 > 0) {$error['ndata_1'] = 'error/El Tipo de Planilla ya existe en el sistema';}
			/*******************************************************************/
...
/*******************************************************************************************************************/
		case 'del_matriz':	
			...
			$query  = "DELETE FROM `cross_quality_calidad_matriz` WHERE idMatriz = {$_GET['del']}";
			//Consulta
			$resultado = mysqli_query ($dbConn, $query);
			//Si ejecuto correctamente la consulta
			if(!$resultado){
