...
if(isset($sesion_IP_Client)&&$sesion_IP_Client!=''){
	...
	if($sesion_Activa!=0){
		...
		$rowUserId = db_select_nrows (false, 'idUsuario','usuarios_listado', '', 'usuario = "'.$sesion_usuario.'" AND password = "'.$sesion_password.'"', $dbConn, 'Ninguno', basename($_SERVER["REQUEST_URI"], ".php"), 'Ninguno');
	...
	//Si no hay sesion activa	
		...
		//se consulta la base de datos
		$rowUserIP = db_select_nrows (false, 'idIpUsuario','usuarios_listado_ip', '', 'IP_Client = "'.$sesion_IP_Client.'"', $dbConn, 'Ninguno', basename($_SERVER["REQUEST_URI"], ".php"), 'Ninguno');
