...
/*******************************************************************************************************************/		
		case 'insert':
			...
			if(isset($Creacion_fecha)&&isset($idTrabajador)&&isset($idSistema)){
				$ndata_1 = db_select_nrows ('Creacion_fecha', 'trabajadores_horas_extras_facturacion_turnos', '', "nSem='".fecha2NSemana($Creacion_fecha)."' AND Creacion_ano='".fecha2Ano($Creacion_fecha)."' AND idTrabajador='".$idTrabajador."' AND idSistema='".$idSistema."'", $dbConn);
			}
...
/*******************************************************************************************************************/		
		case 'update':	
			...
			//Se verifica si el dato existe
			if(isset($Creacion_fecha)&&isset($idTrabajador)&&isset($idSistema)){
				$ndata_1 = db_select_nrows ('Creacion_fecha', 'trabajadores_horas_extras_facturacion_turnos', '', "nSem='".fecha2NSemana($Creacion_fecha)."' AND Creacion_ano='".fecha2Ano($Creacion_fecha)."' AND idTrabajador='".$idTrabajador."' AND idSistema='".$idSistema."' AND idServicios!='".$idServicios."'", $dbConn);
			}
			//generacion de errores
			if($ndata_1 > 0) {$error['ndata_1'] = 'error/El turno ya existe en el sistema';}
			/*******************************************************************/
...
/*******************************************************************************************************************/
		case 'del':	
			...
			$query  = "DELETE FROM `trabajadores_horas_extras_facturacion_turnos` WHERE idServicios = {$_GET['del']}";
			//Consulta
			$resultado = mysqli_query ($dbConn, $query);
			//Si ejecuto correctamente la consulta
			if($resultado){
