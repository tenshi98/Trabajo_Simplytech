...
/*******************************************************************************************************************/		
		case 'insert':
			...
			if(isset($Creacion_fecha)&&isset($idTrabajador)&&isset($idSistema)){
				$ndata_1 = db_select_nrows ('Creacion_fecha', 'trabajadores_inasistencias_dias', '', "Creacion_fecha='".$Creacion_fecha."' AND idTrabajador='".$idTrabajador."' AND idSistema='".$idSistema."'", $dbConn);
			}
			...
			if ( empty($error) ) {
				...
				$query  = "INSERT INTO `trabajadores_inasistencias_dias` (idSistema, idTrabajador, idUsuario,
...
/*******************************************************************************************************************/		
		case 'update':	
			...
			if(isset($Creacion_fecha)&&isset($idTrabajador)&&isset($idInasistenciaDia)&&isset($idSistema)){
				$ndata_1 = db_select_nrows ('Creacion_fecha', 'trabajadores_inasistencias_dias', '', "Creacion_fecha='".$Creacion_fecha."' AND idTrabajador='".$idTrabajador."' AND idSistema='".$idSistema."' AND idInasistenciaDia!='".$idInasistenciaDia."'", $dbConn);
			}
			//generacion de errores
			if($ndata_1 > 0) {$error['ndata_1'] = 'error/La inasistencia ya existe en el sistema';}
			/*******************************************************************/
			...
			if ( empty($error) ) {
				//Filtros
				$a = "idInasistenciaDia='".$idInasistenciaDia."'" ;
				...
				// inserto los datos de registro en la db
				$query  = "UPDATE `trabajadores_inasistencias_dias` SET ".$a." WHERE idInasistenciaDia = '$idInasistenciaDia'";
				//Consulta
				$resultado = mysqli_query ($dbConn, $query);
				//Si ejecuto correctamente la consulta
				if($resultado){
...
/*******************************************************************************************************************/
		case 'del':	
			...
			$query  = "DELETE FROM `trabajadores_inasistencias_dias` WHERE idInasistenciaDia = {$_GET['del']}";
			//Consulta
			$resultado = mysqli_query ($dbConn, $query);
			//Si ejecuto correctamente la consulta
			if($resultado){
