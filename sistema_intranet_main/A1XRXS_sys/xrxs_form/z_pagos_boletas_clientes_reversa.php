...
if( ! defined('XMBCXRXSKGC')){
    die('No tienes acceso a esta carpeta o archivo (Access Code 1009-252).');
}
/*******************************************************************************************************************/
		...
		case 'del_pagos':
			...
			if($errorn==0){
				...
				if(empty($error)){
					...
					//se borran los datos
					if(isset($indice2) && $indice2!=''){
						$resultado = db_delete_data (false, 'pagos_boletas_clientes', 'idFacturacion = "'.$indice2.'"', $dbConn, $_SESSION['usuario']['basic_data']['Nombre'], $original, $form_trabajo);
