...
if( ! defined('XMBCXRXSKGC')){
    die('No tienes acceso a esta carpeta o archivo (Access Code 1009-172).');
}
/*******************************************************************************************************************/
				...
				if (in_array($_FILES['NombreArchivo']['type'], $permitidos) && $_FILES['NombreArchivo']['size'] <= $limite_kb * 1024){
					...
					if (!file_exists($ruta)){
						...
						if ($move_result){
							...
							$SIS_columns = 'NombreArchivo, idTelemetria, idUsuario, Fecha_ingreso, Detalle';
							$ultimo_id = db_insert_data (false, $SIS_columns, $SIS_data, 'telemetria_listado_archivos', $dbConn, $_SESSION['usuario']['basic_data']['Nombre'], $original, $form_trabajo);
