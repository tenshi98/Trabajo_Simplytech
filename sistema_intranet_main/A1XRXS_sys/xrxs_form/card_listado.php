...
/*******************************************************************************************************************/		
		case 'insert':
			...
			if(isset($Nombre)){
				$ndata_1 = db_select_nrows ('Nombre', 'card_listado', '', "Nombre='".$Nombre."'", $dbConn);
			}
...
/*******************************************************************************************************************/		
		case 'update':	
			...
			if(isset($Nombre)&&isset($idCard)){
				$ndata_1 = db_select_nrows ('Nombre', 'card_listado', '', "Nombre='".$Nombre."' AND idCard!='".$idCard."'", $dbConn);
			}
			//generacion de errores
			if($ndata_1 > 0) {$error['ndata_1'] = 'error/El Nombre ya existe en el sistema';}
			/*******************************************************************/
...
/*******************************************************************************************************************/
		case 'del_img':	
			...
			$rowdata = db_select_data ('Direccion_img', 'card_listado', '', "idCard = ".$_GET['del_img'], $dbConn);
...
/*******************************************************************************************************************/
		case 'del':	
			...
			// Se obtiene el nombre del logo
			$rowdata = db_select_data ('Direccion_img', 'card_listado', '', "idCard = ".$_GET['del'], $dbConn);

			//se borra el dato de la base de datos
			$query  = "DELETE FROM `card_listado` WHERE idCard = {$_GET['del']}";
			//Consulta
			$resultado = mysqli_query ($dbConn, $query);
			//Si ejecuto correctamente la consulta
			if($resultado){
