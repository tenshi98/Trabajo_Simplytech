...
$original = "trabajadores_inasistencias_horas.php";
$location = $original;
...
if(isset($_GET['Horas']) && $_GET['Horas'] != ''){                    $location .= "&Horas=".$_GET['Horas'];                     $search .= "&Horas=".$_GET['Horas'];}
/********************************************************************/
...
if ( !empty($_POST['submit']) )  { 
	...
	$form_trabajo= 'insert';
	require_once 'A1XRXS_sys/xrxs_form/trabajadores_inasistencias_horas.php';
}
...
if (isset($_GET['created'])) {$error['usuario'] 	  = 'sucess/Atraso Creado correctamente';}
if (isset($_GET['edited']))  {$error['usuario'] 	  = 'sucess/Atraso Modificado correctamente';}
if (isset($_GET['deleted'])) {$error['usuario'] 	  = 'sucess/Atraso borrado correctamente';}
...
$query = "SELECT idTrabajador,Creacion_fecha,Horas,Observacion, idSistema
FROM `trabajadores_inasistencias_horas`
WHERE idInasistenciaHora = {$_GET['id']}";
//Consulta
	...
	<div class="box dark">
		...
		<div id="div-1" class="body">
			<form class="form-horizontal" method="post" id="form1" name="form1" novalidate>
				...
				$Form_Imputs->form_date('Fecha Facturacion','Creacion_fecha', $x2, 2);
				$Form_Imputs->form_input_number('Cantidad Horas', 'Horas', $x3, 2);
				$Form_Imputs->form_textarea('Observaciones','Observacion', $x4, 1, 160);
				
				$Form_Imputs->form_input_disabled('Empresa Relacionada','fake_emp', $_SESSION['usuario']['basic_data']['RazonSocial'], 1);
				$Form_Imputs->form_input_hidden('idSistema', $_SESSION['usuario']['basic_data']['idSistema'], 2);
				$Form_Imputs->form_input_hidden('idInasistenciaHora', $_GET['id'], 2);
	...
	<div class="box dark">
		...
		<div id="div-1" class="body">
			<form class="form-horizontal" method="post" id="form1" name="form1" novalidate>
				...
				if(isset($Horas)) {           $x3  = $Horas;           }else{$x3  = '';}
...
if(isset($_GET['order_by'])&&$_GET['order_by']!=''){
	switch ($_GET['order_by']) {
		case 'fecha_asc':        $order_by = 'ORDER BY trabajadores_inasistencias_horas.Creacion_fecha ASC ';                                                              $bread_order = '<i class="fa fa-sort-alpha-asc" aria-hidden="true"></i> Fecha Facturacion Ascendente';break;
		case 'fecha_desc':       $order_by = 'ORDER BY trabajadores_inasistencias_horas.Creacion_fecha DESC ';                                                             $bread_order = '<i class="fa fa-sort-alpha-desc" aria-hidden="true"></i> Fecha Facturacion Descendente';break;
		...
		case 'horas_asc':        $order_by = 'ORDER BY usuarios_listado.Horas ASC ';                                                                                       $bread_order = '<i class="fa fa-sort-alpha-asc" aria-hidden="true"></i> Horas Ascendente'; break;
		case 'horas_desc':       $order_by = 'ORDER BY usuarios_listado.Horas DESC ';                                                                                      $bread_order = '<i class="fa fa-sort-alpha-desc" aria-hidden="true"></i> Horas Descendente';break;
		
		default: $order_by = 'ORDER BY trabajadores_inasistencias_horas.Creacion_fecha DESC '; $bread_order = '<i class="fa fa-sort-alpha-desc" aria-hidden="true"></i> Fecha Facturacion Descendente';
	}
}else{
	$order_by = 'ORDER BY trabajadores_inasistencias_horas.Creacion_fecha DESC '; $bread_order = '<i class="fa fa-sort-alpha-desc" aria-hidden="true"></i> Fecha Facturacion Descendente';
}
/**********************************************************/
//Variable de busqueda
$z = "WHERE trabajadores_inasistencias_horas.idInasistenciaHora!=0";
//Verifico el tipo de usuario que esta ingresando
$w = "idSistema={$_SESSION['usuario']['basic_data']['idSistema']} AND idEstado=1";
...
if(isset($_GET['idTrabajador']) && $_GET['idTrabajador'] != ''){      $z .= " AND trabajadores_inasistencias_horas.idTrabajador=".$_GET['idTrabajador'];}
if(isset($_GET['idUsuario']) && $_GET['idUsuario'] != ''){            $z .= " AND trabajadores_inasistencias_horas.idUsuario=".$_GET['idUsuario'];}
if(isset($_GET['Creacion_fecha']) && $_GET['Creacion_fecha'] != ''){  $z .= " AND trabajadores_inasistencias_horas.Creacion_fecha='".$_GET['Creacion_fecha']."'";}
if(isset($_GET['Horas']) && $_GET['Horas'] != ''){                    $z .= " AND trabajadores_inasistencias_horas.Horas=".$_GET['Horas'];}
/**********************************************************/
//Realizo una consulta para saber el total de elementos existentes
$query = "SELECT idInasistenciaHora FROM `trabajadores_inasistencias_horas` 
LEFT JOIN `trabajadores_listado` ON trabajadores_listado.idTrabajador  = trabajadores_inasistencias_horas.idTrabajador
LEFT JOIN `usuarios_listado`     ON usuarios_listado.idUsuario         = trabajadores_inasistencias_horas.idUsuario
LEFT JOIN `core_sistemas`        ON core_sistemas.idSistema            = trabajadores_inasistencias_horas.idSistema
".$z;
//Consulta
$resultado = mysqli_query ($dbConn, $query);
//Si ejecuto correctamente la consulta
...
$arrInasHoras = array();
$query = "SELECT 
trabajadores_inasistencias_horas.idInasistenciaHora,
...
trabajadores_inasistencias_horas.Creacion_fecha,
trabajadores_inasistencias_horas.Horas,
core_sistemas.Nombre AS Sistema,
trabajadores_inasistencias_horas.idUso
...
<div class="col-sm-12 breadcrumb-bar">
	...
	<?php if ($rowlevel['level']>=3){?><a href="<?php echo $location; ?>&new=true" class="btn btn-default fright margin_width fmrbtn" ><i class="fa fa-file-o" aria-hidden="true"></i> Crear Atraso</a><?php } ?>
	...
	<div class="well">
		<div class="col-sm-8 fcenter">
			<form class="form-horizontal" id="form1" name="form1" action="<?php echo $location; ?>" novalidate>
				...
				$Form_Imputs->form_input_number('Cantidad Horas', 'Horas', $x3, 1);
...
<div class="col-sm-12">
	<div class="box">
		<header>
			<div class="icons"><i class="fa fa-table"></i></div><h5>Listado de Atrasos</h5>
			<div class="toolbar">
				<?php 
				...
				<thead>
					<tr role="row">
						...
						<th width="120">
							...
							<div class="btn-group pull-right" style="width: 50px;" >
								<a href="<?php echo $location.'&order_by=horas_asc'; ?>" title="Ascendente" class="btn btn-default btn-xs tooltip"><i class="fa fa-sort-alpha-asc"></i></a>
								<a href="<?php echo $location.'&order_by=horas_desc'; ?>" title="Descendente" class="btn btn-default btn-xs tooltip"><i class="fa fa-sort-alpha-desc"></i></a>
							</div>
						</th>
						...
						<td>
							<div class="btn-group" style="width: 70px;" >
								...
								if(isset($plan['idUso'])&&$plan['idUso']==1){ ?>
									<?php if ($rowlevel['level']>=2){?><a href="<?php echo $location.'&id='.$plan['idInasistenciaHora']; ?>" title="Editar Informacion" class="btn btn-success btn-sm tooltip"><i class="fa fa-pencil-square-o"></i></a><?php } ?>
									<?php if ($rowlevel['level']>=4){
										$ubicacion = $location.'&del='.$plan['idInasistenciaHora'];
