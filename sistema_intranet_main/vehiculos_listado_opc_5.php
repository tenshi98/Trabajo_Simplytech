...
$location = $original;
$new_location = "vehiculos_listado_opc_5.php";
...
if (!empty($_POST['submit'])){
	...
	$form_trabajo= 'insert_pasajero';
	require_once 'A1XRXS_sys/xrxs_form/vehiculos_listado.php';
}

//se borra un dato
if (!empty($_GET['del_pasajero'])){
	//se agregan ubicaciones
	$location = $new_location;
	$location.= '&id='.$_GET['id'];
	//Llamamos al formulario
	$form_trabajo= 'del_pasajero';
	require_once 'A1XRXS_sys/xrxs_form/vehiculos_listado.php';
}
/**********************************************************************************************************************************/
...
if (isset($_GET['created'])){ $error['created'] = 'sucess/Pasajeros Creado correctamente';}
if (isset($_GET['edited'])){  $error['edited']  = 'sucess/Pasajeros Modificado correctamente';}
if (isset($_GET['deleted'])){ $error['deleted'] = 'sucess/Pasajeros Borrado correctamente';}
		...
		<div class="body">
			<form class="form-horizontal" method="post" id="form1" name="form1" autocomplete="off" novalidate>
				...
				<?php
					...
					if(isset($idHijos)){    $x1  = $idHijos;  }else{$x1  = '';}
...
<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
	...
	echo widget_title('bg-aqua', 'fa-cog', 100, 'Vehiculo', $vehiculo, 'Editar Pasajeros'); ?>
	<div class="col-xs-12 col-sm-6 col-md-6 col-lg-8">
		<?php if ($rowlevel['level']>=3){ ?><a href="<?php echo $new_location.'&id='.$_GET['id'].'&new=true'; ?>" class="btn btn-default pull-right margin_width" ><i class="fa fa-file-o" aria-hidden="true"></i> Agregar Pasajero</a><?php } ?>
	</div>
</div>
				...
				<li class="dropdown">
					...
					<ul class="dropdown-menu" role="menu">
						...
						if(isset($rowData['idOpciones_5'])&&$rowData['idOpciones_5']==1){ ?>
							<li class="active"><a href="<?php echo 'vehiculos_listado_opc_5.php?pagina='.$_GET['pagina'].'&id='.$_GET['id']?>" ><i class="fa fa-users" aria-hidden="true"></i> Pasajeros</a></li>
						<?php }
        ...
        <div class="table-responsive">
			<table id="dataTable" class="table table-bordered table-condensed table-hover table-striped dataTable">
				...
				<?php foreach ($arrCargas as $carga) { ?>
					<tr class="odd">
						<td><?php echo $carga['HijoNombre'].' '.$carga['HijoApellidoPat'].' '.$carga['HijoApellidoMat']; ?></td>
						<td><?php echo $carga['ApoderadoNombre'].' '.$carga['ApoderadoApellidoPat'].' '.$carga['ApoderadoApellidoMat']; ?></td>
						<td>
							<div class="btn-group" style="width: 35px;" >
								<?php if ($rowlevel['level']>=4){
									$ubicacion = $new_location.'&id='.$_GET['id'].'&del_pasajero='.simpleEncode($carga['idHijos'], fecha_actual());
									$dialogo   = '¿Realmente deseas eliminar el pasajero '.$carga['HijoNombre'].' '.$carga['HijoApellidoPat'].'?'; ?>
