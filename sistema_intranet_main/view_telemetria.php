...
LEFT JOIN `telemetria_listado_sensores_unimed`  ON telemetria_listado_sensores_unimed.idTelemetria   = telemetria_listado_errores.idTelemetria';
...
$SIS_join  = 'LEFT JOIN `usuarios_listado` ON usuarios_listado.idUsuario = telemetria_carga_bam.idUsuario';
$SIS_where = 'telemetria_carga_bam.idTelemetria ='.$X_Puntero;
$SIS_order = 'telemetria_carga_bam.FechaVencimiento DESC LIMIT 20';
$arrCarga = array();
$arrCarga = db_select_array (false, $SIS_query, 'telemetria_carga_bam', $SIS_join, $SIS_where, $SIS_order, $dbConn, $_SESSION['usuario']['basic_data']['Nombre'], basename($_SERVER["REQUEST_URI"], ".php"), 'arrCarga');
...
$SIS_where = 'telemetria_historial_mantencion_equipos.idTelemetria ='.$X_Puntero;
$SIS_order = 'telemetria_historial_mantencion.Fecha DESC LIMIT 20';
$arrMantenciones = array();
$arrMantenciones = db_select_array (false, $SIS_query, 'telemetria_historial_mantencion_equipos', $SIS_join, $SIS_where, $SIS_order, $dbConn, $_SESSION['usuario']['basic_data']['Nombre'], basename($_SERVER["REQUEST_URI"], ".php"), 'arrMantenciones');
					...
					<div class="col-xs-12 col-sm-8 col-md-8 col-lg-8">
						...
						<p class="text-muted">
							...
							<?php if($rowdata['id_Geo']==2){ ?>
								<br/>
								<strong class="color-red-dark">Ubicación</strong><br/>
								<?php if(isset($rowdata['ZonaSinGPS'])&&$rowdata['ZonaSinGPS']!=''){ ?> <strong>Zona de Trabajo : </strong><?php echo $rowdata['ZonaSinGPS']; ?><br/><?php } ?>
								<?php if(isset($rowdata['Direccion'])&&$rowdata['Direccion']!=''){ ?>   <strong>Dirección : </strong><?php echo $rowdata['Direccion'].', '.$rowdata['Comuna'].', '.$rowdata['Ciudad']; ?><br/><?php } ?>
							<?php } ?>
						</p>
							...
							<tbody role="alert" aria-live="polite" aria-relevant="all">
								<?php foreach ($arrMantenciones as $mant) { ?>
									<tr class="odd">
										...
										<td>
											<div class="btn-group" style="width: 35px;" >
												<a href="<?php echo 'view_telemetria_mantencion.php?view='.simpleEncode($mant['idMantencion'], fecha_actual()).'&return='.basename($_SERVER["REQUEST_URI"], ".php"); ?>" title="Ver Información" class="btn btn-primary btn-sm tooltip"><i class="fa fa-list" aria-hidden="true"></i></a>
											</div>
										</td>
									</tr>
								<?php } ?>
