...
function crear_data($INT_cantsens, $INT_filtro, $INT_idTelemetria, $INT_f_inicio, $INT_f_termino, $INT_desde, $INT_hasta, $dbConn ) {
	...
	for ($i = 1; $i <= $INT_cantsens; $i++) {
		...
		if(isset($INT_desde)&&$INT_desde!=''&&isset($INT_hasta)&&$INT_hasta!=''){
			//$consql .= ',MIN(NULLIF(IF(telemetria_listado.SensoresActivo_'.$i.'=1,IF(backup_telemetria_listado_tablarelacionada_'.$INT_idTelemetria.'.Sensor_'.$i.'>='.$INT_desde.',IF(backup_telemetria_listado_tablarelacionada_'.$INT_idTelemetria.'.Sensor_'.$i.'<='.$INT_hasta.',IF(backup_telemetria_listado_tablarelacionada_'.$INT_idTelemetria.'.Sensor_'.$i.'<99900,backup_telemetria_listado_tablarelacionada_'.$INT_idTelemetria.'.Sensor_'.$i.',0),0),0),0),0)) AS MedMin_'.$i;
			//$consql .= ',MAX(NULLIF(IF(telemetria_listado.SensoresActivo_'.$i.'=1,IF(backup_telemetria_listado_tablarelacionada_'.$INT_idTelemetria.'.Sensor_'.$i.'>='.$INT_desde.',IF(backup_telemetria_listado_tablarelacionada_'.$INT_idTelemetria.'.Sensor_'.$i.'<='.$INT_hasta.',IF(backup_telemetria_listado_tablarelacionada_'.$INT_idTelemetria.'.Sensor_'.$i.'<99900,backup_telemetria_listado_tablarelacionada_'.$INT_idTelemetria.'.Sensor_'.$i.',0),0),0),0),0)) AS MedMax_'.$i;
			$consql .= ',AVG(NULLIF(IF(telemetria_listado.SensoresActivo_'.$i.'=1,IF(backup_telemetria_listado_tablarelacionada_'.$INT_idTelemetria.'.Sensor_'.$i.'>='.$INT_desde.',IF(backup_telemetria_listado_tablarelacionada_'.$INT_idTelemetria.'.Sensor_'.$i.'<='.$INT_hasta.',IF(backup_telemetria_listado_tablarelacionada_'.$INT_idTelemetria.'.Sensor_'.$i.'<99900,backup_telemetria_listado_tablarelacionada_'.$INT_idTelemetria.'.Sensor_'.$i.',0),0),0),0),0)) AS MedProm_'.$i;
			//$consql .= ',STDDEV(NULLIF(IF(telemetria_listado.SensoresActivo_'.$i.'=1,IF(backup_telemetria_listado_tablarelacionada_'.$INT_idTelemetria.'.Sensor_'.$i.'>='.$INT_desde.',IF(backup_telemetria_listado_tablarelacionada_'.$INT_idTelemetria.'.Sensor_'.$i.'<='.$INT_hasta.',IF(backup_telemetria_listado_tablarelacionada_'.$INT_idTelemetria.'.Sensor_'.$i.'<99900,backup_telemetria_listado_tablarelacionada_'.$INT_idTelemetria.'.Sensor_'.$i.',0),0),0),0),0)) AS MedDesStan_'.$i;
		//solo desde	
		...
		//ninguno
			...
			//$consql .= ',MIN(NULLIF(IF(telemetria_listado.SensoresActivo_'.$i.'=1,IF(backup_telemetria_listado_tablarelacionada_'.$INT_idTelemetria.'.Sensor_'.$i.'<99900,backup_telemetria_listado_tablarelacionada_'.$INT_idTelemetria.'.Sensor_'.$i.',0),0),0)) AS MedMin_'.$i;
			//$consql .= ',MAX(NULLIF(IF(telemetria_listado.SensoresActivo_'.$i.'=1,IF(backup_telemetria_listado_tablarelacionada_'.$INT_idTelemetria.'.Sensor_'.$i.'<99900,backup_telemetria_listado_tablarelacionada_'.$INT_idTelemetria.'.Sensor_'.$i.',0),0),0)) AS MedMax_'.$i;
			$consql .= ',AVG(NULLIF(IF(telemetria_listado.SensoresActivo_'.$i.'=1,IF(backup_telemetria_listado_tablarelacionada_'.$INT_idTelemetria.'.Sensor_'.$i.'<99900,backup_telemetria_listado_tablarelacionada_'.$INT_idTelemetria.'.Sensor_'.$i.',0),0),0)) AS MedProm_'.$i;
			//$consql .= ',STDDEV(NULLIF(IF(telemetria_listado.SensoresActivo_'.$i.'=1,IF(backup_telemetria_listado_tablarelacionada_'.$INT_idTelemetria.'.Sensor_'.$i.'<99900,backup_telemetria_listado_tablarelacionada_'.$INT_idTelemetria.'.Sensor_'.$i.',0),0),0)) AS MedDesStan_'.$i;
		}
	}
...
$pdf_file       = 'Informe Promedio Camara del equipo '.$rowEquipo['Nombre'].'.pdf';
$OpcDom         = "'A4', 'landscape'";
