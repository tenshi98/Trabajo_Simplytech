...
if(isset($f_inicio)&&$f_inicio!=''&&isset($f_termino)&&$f_termino!=''&&isset($h_inicio)&&$h_inicio!=''&&isset($h_termino)&&$h_termino!=''){
	$SIS_where.= "(telemetria_listado_crossenergy_hora.TimeStamp BETWEEN '".$f_inicio." ".$h_inicio."' AND '".$f_termino." ".$h_termino."')";
}elseif(isset($f_inicio)&&$f_inicio!=''&&isset($f_termino)&&$f_termino!=''){
	$SIS_where.= "(telemetria_listado_crossenergy_hora.FechaSistema BETWEEN '".$f_inicio."' AND '".$f_termino."')";
}
$SIS_where.= " AND telemetria_listado_crossenergy_hora.idTelemetria=".$idTelemetria;
...
if(isset($ndata_1)&&$ndata_1>=10001){
	...
	$arrEquipos = array();
	$arrEquipos = db_select_array (false, $SIS_query, 'telemetria_listado_crossenergy_hora', $SIS_join, $SIS_where, $SIS_order, $dbConn, $_SESSION['usuario']['basic_data']['Nombre'], basename($_SERVER["REQUEST_URI"], ".php"), 'test_logo');
			...
			if($fac['SensoresGrupo_'.$x]==$idGrupo){
				...
				if(isset($fac['SensoresActivo_'.$x])&&$fac['SensoresActivo_'.$x]==1){
					...
					if(isset($fac['SensorValue_'.$x])&&$fac['SensorValue_'.$x]<99900){
						...
						if($count==0){
							$m_table_title  .= '<th style="font-size: 10px;text-align:center;background-color: #c3c3c3;">'.$fac['SensoresNombre_'.$x].'</th>';
						}
					}
				}
			}
	...
	/**********************************************************************************************************************************/
	//Config
	$pdf_titulo     = 'Resumen Hora';
	$pdf_subtitulo  = $_SESSION['usuario']['basic_data']['RazonSocial'];
	$pdf_subtitulo .= '
	Informe grupo '.$rowGrupo['Nombre'].' del equipo '.$rowEquipo['NombreEquipo'].'
	';
	if(isset($f_inicio)&&$f_inicio!=''&&isset($f_termino)&&$f_termino!=''&&isset($h_inicio)&&$h_inicio!=''&&isset($h_termino)&&$h_termino!=''){
	...
	$pdf_file       = 'Informe Resumen Hora grupo '.$rowGrupo['Nombre'].' del equipo '.$rowEquipo['NombreEquipo'].'.pdf';
	$OpcDom         = "'A4', 'landscape'";
