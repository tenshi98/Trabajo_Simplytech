		...
		<header>
			<ul class="nav nav-tabs pull-right">
				...
				<?php if($todos!=0 or $idTipoUsuario==1) { ?>
					<li class="active"><a href="<?php echo 'vehiculos_listado_configuracion.php?pagina='.$_GET['pagina'].'&id='.$_GET['id']?>" >Configuracion</a></li>
				<?php } ?>
				<li class="dropdown">
        ...
        <div class="table-responsive">
			<div class="col-sm-8 fcenter" style="padding-top:40px;">
				<form class="form-horizontal" method="post" id="form1" name="form1" novalidate>		
					...
					if(isset($idOpciones_3)) {    $x3  = $idOpciones_3;    }else{$x3  = $rowdata['idOpciones_3'];}
					if(isset($idOpciones_4)) {    $x4  = $idOpciones_4;    }else{$x4  = $rowdata['idOpciones_4'];}
					if(isset($idOpciones_5)) {    $x5  = $idOpciones_5;    }else{$x5  = $rowdata['idOpciones_5'];}
					...
					if($telemetria!=0 or $idTipoUsuario==1) {
						$Form_Imputs->form_select('Utilizar Telemetria','idOpciones_1', $x1, 2, 'idOpciones', 'Nombre', 'core_sistemas_opciones', 0, '', $dbConn);
					}
					if($bodega!=0 or $idTipoUsuario==1) {
						$Form_Imputs->form_select('Utilizar Bodega','idOpciones_2', $x2, 2, 'idOpciones', 'Nombre', 'core_sistemas_opciones', 0, '', $dbConn);
					}
					if($ruta!=0 or $idTipoUsuario==1) {
						$Form_Imputs->form_select('Utilizar Ruta','idOpciones_3', $x3, 2, 'idOpciones', 'Nombre', 'core_sistemas_opciones', 0, '', $dbConn);
					}
					if($trabajador!=0 or $idTipoUsuario==1) {
						$Form_Imputs->form_select('Utilizar Trabajador','idOpciones_4', $x4, 2, 'idOpciones', 'Nombre', 'core_sistemas_opciones', 0, '', $dbConn);
					}
					if($pasajeros!=0 or $idTipoUsuario==1) {
						$Form_Imputs->form_select('Utilizar Pasajeros','idOpciones_5', $x5, 2, 'idOpciones', 'Nombre', 'core_sistemas_opciones', 0, '', $dbConn);
						$Form_Imputs->form_select('Utilizar APP','idOpciones_6', $x6, 2, 'idOpciones', 'Nombre', 'core_sistemas_opciones', 0, '', $dbConn);
					}
					if($peonetas!=0 or $idTipoUsuario==1) {
						$Form_Imputs->form_select('Utilizar Peonetas','idOpciones_7', $x7, 2, 'idOpciones', 'Nombre', 'core_sistemas_opciones', 0, '', $dbConn);
					}
					if($colegios!=0 or $idTipoUsuario==1) {
						$Form_Imputs->form_select('Utilizar Colegios','idOpciones_8', $x8, 2, 'idOpciones', 'Nombre', 'core_sistemas_opciones', 0, '', $dbConn);
					}
