...
$original = "seg_vecinal_reportes_post_listado.php";
$location = $original;
		...
		<div id="div-1" class="body">
			<form class="form-horizontal" method="post" id="form1" name="form1" novalidate>
				...
				<div class="form-group">
					<input type="submit" class="btn btn-primary fright margin_width fa-input" value="&#xf0c7; Guardar Cambios" name="submit_infraction">
					<a href="<?php echo $ubicacion; ?>" class="btn btn-danger fright margin_width"><i class="fa fa-arrow-left" aria-hidden="true"></i> Cancelar y Volver</a>
				</div>
...
seg_vecinal_reportes_post_listado.Fecha,
seg_vecinal_reportes_post_listado.Hora,
seg_vecinal_reportes_post_listado.Comentario,
seg_vecinal_clientes_listado.Nombre AS Vecino

FROM `seg_vecinal_reportes_post_listado`
LEFT JOIN `seg_vecinal_clientes_listado`    ON seg_vecinal_clientes_listado.idCliente  = seg_vecinal_reportes_post_listado.idCliente
WHERE seg_vecinal_reportes_post_listado.idEventoPeligro = ".$_GET['idEventoPeligro']." 
AND seg_vecinal_reportes_post_listado.idTipo = ".$_GET['idTipofil']."
ORDER BY seg_vecinal_reportes_post_listado.Fecha DESC, seg_vecinal_reportes_post_listado.Hora DESC";
//Consulta
$resultado = mysqli_query ($dbConn, $query);
//Si ejecuto correctamente la consulta
if(!$resultado){
...
<div class="col-sm-12" style="margin-bottom:30px">
	...
	<div class="btn-group fright margin_width">
								...
								<?php foreach ($arrComentarios as $comment) { ?>
									<li class="right clearfix">
										<span class="chat-img pull-right">
											<?php echo '<img alt="User Picture" class="img-circle" src="'.DB_SITE_REPO.'/LIB_assets/img/usr.png">'; ?>
										</span>
...
$SIS_where  = "seg_vecinal_reportes_post_listado.idSistema=".$_SESSION['usuario']['basic_data']['idSistema'];
$SIS_where .= " AND seg_vecinal_reportes_post_listado.idRevisado=1"; //solo los no revisados
/**********************************************************/
//Se aplican los filtros
if(isset($_GET['idCliente']) && $_GET['idCliente'] != ''){      $SIS_where .= " AND seg_vecinal_reportes_post_listado.idCliente='".$_GET['idCliente']."'";}
if(isset($_GET['idTipo']) && $_GET['idTipo'] != ''){            $SIS_where .= " AND seg_vecinal_reportes_post_listado.idTipo='".$_GET['idTipo']."'";}
if(isset($_GET['Fecha']) && $_GET['Fecha'] != ''){              $SIS_where .= " AND seg_vecinal_reportes_post_listado.Fecha='".$_GET['Fecha']."'";}
if(isset($_GET['Hora']) && $_GET['Hora'] != ''){                $SIS_where .= " AND seg_vecinal_reportes_post_listado.Hora='".$_GET['Hora']."'";}
				
/**********************************************************/
//Realizo una consulta para saber el total de elementos existentes
$cuenta_registros = db_select_nrows (false, 'idReportes', 'seg_vecinal_reportes_post_listado', '', $SIS_where, $dbConn, $_SESSION['usuario']['basic_data']['Nombre'], $original, 'cuenta_registros');
//Realizo la operacion para saber la cantidad de paginas que hay
$total_paginas = ceil($cuenta_registros / $cant_reg);	
// Se trae un listado con todos los elementos
$SIS_query = '
seg_vecinal_reportes_post_listado.idEventoPeligro,
seg_vecinal_reportes_post_listado.idTipo,
seg_vecinal_reportes_post_listado.Fecha,
seg_vecinal_reportes_post_listado.Hora,
seg_vecinal_reportes_post_tipos.Nombre AS TipoReporte,
seg_vecinal_clientes_listado.Nombre AS Vecino,
...
$SIS_join  = '
LEFT JOIN `seg_vecinal_reportes_post_tipos` ON seg_vecinal_reportes_post_tipos.idTipo  = seg_vecinal_reportes_post_listado.idTipo
LEFT JOIN `seg_vecinal_clientes_listado`    ON seg_vecinal_clientes_listado.idCliente  = seg_vecinal_reportes_post_listado.idCliente
LEFT JOIN `seg_vecinal_eventos_listado`     ON seg_vecinal_eventos_listado.idEvento    = seg_vecinal_reportes_post_listado.idEventoPeligro
LEFT JOIN `seg_vecinal_eventos_tipos`       ON seg_vecinal_eventos_tipos.idTipo        = seg_vecinal_eventos_listado.idTipo
LEFT JOIN `seg_vecinal_peligros_listado`    ON seg_vecinal_peligros_listado.idPeligro  = seg_vecinal_reportes_post_listado.idEventoPeligro
LEFT JOIN `seg_vecinal_peligros_tipos`      ON seg_vecinal_peligros_tipos.idTipo       = seg_vecinal_peligros_listado.idTipo';
$SIS_order = 'seg_vecinal_reportes_post_listado.Fecha DESC, seg_vecinal_reportes_post_listado.Hora DESC LIMIT '.$comienzo.', '.$cant_reg;
$arrReportes = array();
$arrReportes = db_select_array (false, $SIS_query, 'seg_vecinal_reportes_post_listado', $SIS_join, $SIS_where, $SIS_order, $dbConn, $_SESSION['usuario']['basic_data']['Nombre'], $original, 'arrReportes');
