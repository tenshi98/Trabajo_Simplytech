...
$original = "trabajadores_turnos.php";
$location = $original;
...
if ( !empty($_POST['submit']) )  { 
	...
	$form_trabajo= 'insert';
	require_once 'A1XRXS_sys/xrxs_form/trabajadores_horas_extras_facturacion_turnos.php';
}
...
$query = "SELECT idTrabajador,Creacion_fecha,idTurnos,idSistema
	...
	<div class="box dark">
		...
		<div id="div-1" class="body">
			<form class="form-horizontal" method="post" id="form1" name="form1" novalidate>
				...
				if(isset($idTurnos)) {        $x3  = $idTurnos;        }else{$x3  = $rowdata['idTurnos'];}
				...
				$Form_Imputs->form_select_filter('Turno','idTurnos', $x3, 2, 'idTurnos', 'Nombre', 'core_horas_extras_turnos', 0, '', $dbConn);
				
				$Form_Imputs->form_input_disabled('Empresa Relacionada','fake_emp', $_SESSION['usuario']['basic_data']['RazonSocial'], 1);
				$Form_Imputs->form_input_hidden('idSistema', $_SESSION['usuario']['basic_data']['idSistema'], 2);
				$Form_Imputs->form_input_hidden('idServicios', $_GET['id'], 2);
	...
	<div class="box dark">
		...
		<div id="div-1" class="body">
			<form class="form-horizontal" method="post" id="form1" name="form1" novalidate>
				...
				if(isset($idTurnos)) {        $x3  = $idTurnos;        }else{$x3  = '';}
...
trabajadores_horas_extras_facturacion_turnos.Creacion_fecha,
	...
	<div class="well">
		<div class="col-sm-8 fcenter">
			<form class="form-horizontal" id="form1" name="form1" action="<?php echo $location; ?>" novalidate>
				...
				if(isset($idTrabajador)) {    $x1  = $idTrabajador;    }else{$x1  = '';}
				if(isset($Creacion_fecha)) {  $x2  = $Creacion_fecha;  }else{$x2  = '';}
				if(isset($nSem)) {            $x3  = $nSem;            }else{$x3  = '';}
				if(isset($idTurnos)) {        $x4  = $idTurnos;        }else{$x4  = '';}
				if(isset($idUsuario)) {       $x5  = $idUsuario;       }else{$x5  = '';}
				if(isset($idUso)) {           $x6  = $idUso;           }else{$x6  = '';}
				...
				$Form_Imputs->form_select_n_auto('Semana','nSem', $x3, 1, 1, 57);
				$Form_Imputs->form_select_filter('Turno','idTurnos', $x4, 1, 'idTurnos', 'Nombre', 'core_horas_extras_turnos', 0, '', $dbConn);
				...
				$Form_Imputs->form_select('Utilizable','idUso', $x6, 1, 'idOpciones', 'Nombre', 'core_sistemas_opciones', 0, '', $dbConn);
...
<div class="col-sm-12">
	<div class="box">
		<header>
			<div class="icons"><i class="fa fa-table"></i></div><h5>Listado de Turno</h5>
			<div class="toolbar">
				<?php 
			...
			<table id="dataTable" class="table table-bordered table-condensed table-hover table-striped dataTable">
				<thead>
					<tr role="row">
						...
						<th>
							...
							<div class="btn-group pull-right" style="width: 50px;" >
								<a href="<?php echo $location.'&order_by=turno_asc'; ?>" title="Ascendente" class="btn btn-default btn-xs tooltip"><i class="fa fa-sort-alpha-asc"></i></a>
								<a href="<?php echo $location.'&order_by=turno_desc'; ?>" title="Descendente" class="btn btn-default btn-xs tooltip"><i class="fa fa-sort-alpha-desc"></i></a>
							</div>
						</th>
						<th>
				...
				<?php foreach ($arrInasHoras as $plan) { ?>
					<tr class="odd">
						...
						<td><?php echo $plan['nSem']; ?></td>
						<td><?php echo $plan['Turno']; ?></td>
						...
						<td>
							<div class="btn-group" style="width: 70px;" >
								...
								if(isset($plan['idUso'])&&$plan['idUso']==1){ ?>
									<?php if ($rowlevel['level']>=2){?><a href="<?php echo $location.'&id='.$plan['idServicios']; ?>" title="Editar Informacion" class="btn btn-success btn-sm tooltip"><i class="fa fa-pencil-square-o"></i></a><?php } ?>
									<?php if ($rowlevel['level']>=4){
										$ubicacion = $location.'&del='.$plan['idServicios'];
										$dialogo   = '¿Realmente deseas eliminar el Turno de '.$plan['TrabApellidoPat'].' '.$plan['TrabApellidoMat'].' '.$plan['TrabNombre'].'?';?>
