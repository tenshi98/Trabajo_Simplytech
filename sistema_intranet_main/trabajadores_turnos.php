...
if(!empty($_GET['id'])){
	...
	/*******************************************************/
	// consulto los datos
	$SIS_query = 'idTrabajador,Creacion_fecha,idTurnos,idSistema';
	$SIS_join  = '';
	$SIS_where = 'idServicios = '.$_GET['id'];
	$rowData = db_select_data (false, $SIS_query, 'trabajadores_horas_extras_facturacion_turnos', $SIS_join, $SIS_where, $dbConn, $_SESSION['usuario']['basic_data']['Nombre'], $original, 'rowData');
	/*******************************************************/
							...
							<td>
								<div class="btn-group" style="width: 70px;" >
									...
									if(isset($plan['idUso'])&&$plan['idUso']==1){ ?>
										<?php if ($rowlevel['level']>=2){ ?><a href="<?php echo $location.'&id='.$plan['idServicios']; ?>" title="Editar Información" class="btn btn-success btn-sm tooltip"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></a><?php } ?>
