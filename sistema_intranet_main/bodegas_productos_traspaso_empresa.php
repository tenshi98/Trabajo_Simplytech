...
$original = "bodegas_productos_traspaso_empresa.php";
$location = $original;
...
if ( !empty($_GET['add_obs']) )     {
	//Llamamos al formulario
	$form_trabajo= 'add_obs_traspasoempresa';
	require_once 'A1XRXS_sys/xrxs_form/z_bodega_productos.php';	
}
//se borra un dato
if ( !empty($_GET['del_obs']) )     {
	//Llamamos al formulario
	$form_trabajo= 'del_obs_traspasoempresa';
	require_once 'A1XRXS_sys/xrxs_form/z_bodega_productos.php';	
}
/******************************************/
...
productos_listado.ValorIngreso,
	...
	AND idBodega=".$_SESSION['productos_traspasoempresa_basicos']['idBodegaOrigen']." LIMIT 1) AS ingreso,
				
	(SELECT 
	SUM(Cantidad_eg) AS egreso
	FROM `bodegas_productos_facturacion_existencias`
	WHERE idProducto = productos_listado.idProducto 
	AND idBodega=".$_SESSION['productos_traspasoempresa_basicos']['idBodegaOrigen']." LIMIT 1) AS egreso
FROM `productos_listado`
LEFT JOIN `sistema_productos_uml` ON sistema_productos_uml.idUml = productos_listado.idUml
WHERE productos_listado.idProducto='{$_SESSION['productos_traspasoempresa_productos'][$_GET['editProd']]['idProducto']}'
	...
	<div class="box dark">
		...
		<div id="div-1" class="body">
			<form class="form-horizontal" method="post" id="form1" name="form1" novalidate>
				...
				if(isset($idProducto)) {       $x1  = $idProducto;      }else{$x1  = $_SESSION['productos_traspasoempresa_productos'][$_GET['editProd']]['idProducto'];}
				if(isset($Number)) {           $x2  = $Number;          }else{$x2  = $_SESSION['productos_traspasoempresa_productos'][$_GET['editProd']]['Number'];}
				...
				$Form_Imputs->form_input_disabled('Valor Unitario','Unitario', Cantidades_decimales_justos($_SESSION['productos_traspasoempresa_productos'][$_GET['editProd']]['ValorEgreso']), 1);
				$Form_Imputs->form_input_hidden('ValorEgreso', Cantidades_decimales_justos($_SESSION['productos_traspasoempresa_productos'][$_GET['editProd']]['ValorEgreso']), 2);
...
<script>
	...
	function OnSelectionChange (select) {
		...
		FROM `bodegas_productos_facturacion_existencias`
		WHERE idProducto = productos_listado.idProducto 
		AND idBodega=".$_SESSION['productos_traspasoempresa_basicos']['idBodegaOrigen'].") AS ingreso,
					
		(SELECT 
		SUM(Cantidad_eg) AS egreso
		FROM `bodegas_productos_facturacion_existencias`
		WHERE idProducto = productos_listado.idProducto 
		AND idBodega=".$_SESSION['productos_traspasoempresa_basicos']['idBodegaOrigen'].") AS egreso
	...
	<div class="box dark">
		...
		<div id="div-1" class="body">
			<form class="form-horizontal" method="post" id="form1" name="form1" novalidate>
				...
				<?php 
				//Se verifican si existen los datos
				if(isset($idCentroCosto)) {  $x1  = $idCentroCosto;  }else{$x1  = $_SESSION['productos_traspasoempresa_basicos']['idCentroCosto'];}
				if(isset($idLevel_1)) {      $x2  = $idLevel_1;      }else{$x2  = $_SESSION['productos_traspasoempresa_basicos']['idLevel_1'];}
				if(isset($idLevel_2)) {      $x3  = $idLevel_2;      }else{$x3  = $_SESSION['productos_traspasoempresa_basicos']['idLevel_2'];}
				if(isset($idLevel_3)) {      $x4  = $idLevel_3;      }else{$x4  = $_SESSION['productos_traspasoempresa_basicos']['idLevel_3'];}
				if(isset($idLevel_4)) {      $x5  = $idLevel_4;      }else{$x5  = $_SESSION['productos_traspasoempresa_basicos']['idLevel_4'];}
				if(isset($idLevel_5)) {      $x6  = $idLevel_5;      }else{$x6  = $_SESSION['productos_traspasoempresa_basicos']['idLevel_5'];}
	...
	<div class="box dark">
		...
		<div id="div-1" class="body">
			<form class="form-horizontal" method="post" id="form1" name="form1" novalidate>
				...
				<?php 
				//Se verifican si existen los datos
				if(isset($Creacion_fecha)) {    $x1  = $Creacion_fecha;     }else{$x1  = $_SESSION['productos_traspasoempresa_basicos']['Creacion_fecha'];}
				if(isset($idBodegaOrigen)) {    $x2  = $idBodegaOrigen;     }else{$x2  = $_SESSION['productos_traspasoempresa_basicos']['idBodegaOrigen'];}
				if(isset($idSistemaDestino)) {  $x3  = $idSistemaDestino;   }else{$x3  = $_SESSION['productos_traspasoempresa_basicos']['idSistemaDestino'];}
				if(isset($idBodegaDestino)) {   $x4  = $idBodegaDestino;    }else{$x4  = $_SESSION['productos_traspasoempresa_basicos']['idBodegaDestino'];}
				
				//se dibujan los inputs
				$Form_Imputs = new Form_Inputs();
				$Form_Imputs->form_date('Fecha','Creacion_fecha', $x1, 2);
				$Form_Imputs->form_select_join_filter('Bodega Origen','idBodegaOrigen', $x2, 2, 'idBodega', 'Nombre', 'bodegas_productos_listado', 'usuarios_bodegas_productos', $z, $dbConn);
				$Form_Imputs->form_select_depend1('Empresa Destino','idSistemaDestino', $x3, 2, 'idSistema', 'Nombre', 'core_sistemas', 0, 0,
				...
				$Form_Imputs->form_input_hidden('idTipo', 6, 2);			
...
if($valor_0!=0){
	...
	<div class="col-sm-12" >
		...
		Uno de los productos seleccionados no tiene un precio asignado, 
		vaya a la transaccion <strong>Administrar - Administrar Productos - Tag Datos Comerciales</strong> y 
		realice los cambios
		</div>
	</div>
	<div class="clearfix"></div>
...
<div class="col-sm-12 fcenter">
	...
	<div id="page-wrap">
		<div id="header"> <?php echo $_SESSION['productos_traspasoempresa_basicos']['TipoDocumento']; ?></div>
			...
			<table id="meta" class="fleft otdata">
				<tbody>
					...
					<tr>
						...
						<td><?php echo $_SESSION['productos_traspasoempresa_basicos']['BodegaOrigen']; ?></td>
					</tr>
					<tr>
						<td class="meta-head">Empresa Destino</td>
						<td><?php echo $_SESSION['productos_traspasoempresa_basicos']['SistemaDestino']; ?></td>
					</tr>
					<tr>
						<td class="meta-head">Bodega Destino</td>
						<td><?php echo $_SESSION['productos_traspasoempresa_basicos']['BodegaDestino']; ?></td>
					</tr>
					<tr>
					...
					<tr>
						...
						<td><?php echo $_SESSION['productos_traspasoempresa_basicos']['CentroCosto']; ?></td>
					</tr>
				</tbody>
			</table>
		...
		<table id="items">
			<tbody>
				...
				<tr>
					<?php if(isset($_SESSION['productos_traspasoempresa_basicos']['Observaciones'])&&$_SESSION['productos_traspasoempresa_basicos']['Observaciones']!=''){ ?>
					
						<td colspan="5" class="blank word_break"> 
							<?php echo $_SESSION['productos_traspasoempresa_basicos']['Observaciones'];?>
						</td>
						<td class="blank">
					...
					<?php }else{?>
						<td colspan="5" class="blank"> 
							<?php 
							$non = '';
							if(isset($_SESSION['productos_traspasoempresa_temporal'])&&$_SESSION['productos_traspasoempresa_temporal']!=''){
								$non = $_SESSION['productos_traspasoempresa_temporal'];
							}	
...
if(isset($_GET['order_by'])&&$_GET['order_by']!=''){
	switch ($_GET['order_by']) {
		...
		case 'destino_asc':        $order_by = 'ORDER BY bodega2.Nombre ASC ';                                $bread_order = '<i class="fa fa-sort-alpha-asc" aria-hidden="true"></i> Destino Ascendente';break;
		case 'destino_desc':       $order_by = 'ORDER BY bodega2.Nombre DESC ';                               $bread_order = '<i class="fa fa-sort-alpha-desc" aria-hidden="true"></i> Destino Descendente';break;
...
//Verifico el tipo de usuario que esta ingresando
if($_SESSION['usuario']['basic_data']['idTipoUsuario']==1){
	$w="bodegas_productos_listado.idSistema>=0";
}else{
	$w="bodegas_productos_listado.idSistema={$_SESSION['usuario']['basic_data']['idSistema']} AND usuarios_bodegas_productos.idUsuario = {$_SESSION['usuario']['basic_data']['idUsuario']}";	
}
/**********************************************************/
//Variable con la ubicacion
$z="WHERE bodegas_productos_facturacion.idTipo=6";//Solo traspaso a otra empresa
//Verifico el tipo de usuario que esta ingresando
...
if(isset($_GET['idBodegaDestino']) && $_GET['idBodegaDestino'] != ''){     $z .= " AND bodegas_productos_facturacion.idBodegaDestino=".$_GET['idBodegaDestino'];}
	...
	<div class="well">
		<div class="col-sm-8 fcenter">
			<form class="form-horizontal" id="form1" name="form1" action="<?php echo $location; ?>" novalidate>
				...
				$Form_Imputs->form_select_join_filter('Bodega Origen','idBodegaOrigen', $x2, 1, 'idBodega', 'Nombre', 'bodegas_productos_listado', 'usuarios_bodegas_productos', $w, $dbConn);
				$Form_Imputs->form_select_depend1('Empresa Destino','idSistemaDestino', $x3, 1, 'idSistema', 'Nombre', 'core_sistemas', 0, 0,
								 'Bodega Destino','idBodegaDestino', $x4, 1, 'idBodega', 'Nombre', 'bodegas_productos_listado', 0, 0, 
								 $dbConn, 'form1');
					...
					<tr role="row">
						<th>
							...
							<div class="btn-group pull-right" style="width: 50px;" >
								<a href="<?php echo $location.'&order_by=origen_asc'; ?>" title="Ascendente" class="btn btn-default btn-xs tooltip"><i class="fa fa-sort-alpha-asc"></i></a>
								<a href="<?php echo $location.'&order_by=origen_desc'; ?>" title="Descendente" class="btn btn-default btn-xs tooltip"><i class="fa fa-sort-alpha-desc"></i></a>
							</div>
						</th>
						<th>
							<div class="pull-left">Destino</div>
							<div class="btn-group pull-right" style="width: 50px;" >
								<a href="<?php echo $location.'&order_by=destino_asc'; ?>" title="Ascendente" class="btn btn-default btn-xs tooltip"><i class="fa fa-sort-alpha-asc"></i></a>
								<a href="<?php echo $location.'&order_by=destino_desc'; ?>" title="Descendente" class="btn btn-default btn-xs tooltip"><i class="fa fa-sort-alpha-desc"></i></a>
							</div>
						</th>
						<th>
