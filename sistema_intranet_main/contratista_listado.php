	...
	<div class="col-md-6 col-sm-6 col-xs-12" style="padding-left: 0px;">
		<div class="info-box bg-aqua">
			...
			<div class="info-box-content">
				<span class="info-box-text">Contratista</span>
	...
	<div class="box">
		<header>
			<ul class="nav nav-tabs pull-right">
				<li class="active"><a href="<?php echo 'contratista_listado.php?pagina='.$_GET['pagina'].'&id='.$_GET['id']?>" >Resumen</a></li>
				<li class=""><a href="<?php echo 'contratista_listado_datos.php?pagina='.$_GET['pagina'].'&id='.$_GET['id']?>" >Datos Basicos</a></li>
				<li class=""><a href="<?php echo 'contratista_listado_datos_contacto.php?pagina='.$_GET['pagina'].'&id='.$_GET['id']?>" >Datos Contacto</a></li>
				<li class="dropdown">
					<a href="#" data-toggle="dropdown">Ver mas <span class="caret"></span></a>
					<ul class="dropdown-menu" role="menu">
						<li class=""><a href="<?php echo 'contratista_listado_datos_facturacion.php?pagina='.$_GET['pagina'].'&id='.$_GET['id']?>" >Datos Facturacion</a></li>
						<li class=""><a href="<?php echo 'contratista_listado_estado.php?pagina='.$_GET['pagina'].'&id='.$_GET['id']?>" >Estado</a></li>
						<li class=""><a href="<?php echo 'contratista_listado_observaciones.php?pagina='.$_GET['pagina'].'&id='.$_GET['id']?>" >Observaciones</a></li>
					</ul>
                </li>           
			</ul>	
		</header>
	...
	<div class="box dark">	
		...
		<div id="div-1" class="body" style="min-height:500px;">	
			<form class="form-horizontal" method="post" id="form1" name="form1" novalidate>
				...
				$Form_Imputs->form_select('Tipo de Contratista','idTipo', $x1, 2, 'idTipo', 'Nombre', 'contratista_tipos', 0, '', $dbConn);
				...
				$Form_Imputs->form_select_country('Pais','idPais', $x5, 2, $dbConn);
...
if(isset($_GET['order_by'])&&$_GET['order_by']!=''){
	switch ($_GET['order_by']) {
		case 'rut_asc':      $order_by = 'ORDER BY contratista_listado.Rut ASC ';        $bread_order = '<i class="fa fa-sort-alpha-asc" aria-hidden="true"></i> Rut Ascendente'; break;
		case 'rut_desc':     $order_by = 'ORDER BY contratista_listado.Rut DESC ';       $bread_order = '<i class="fa fa-sort-alpha-desc" aria-hidden="true"></i> Rut Descendente';break;
		case 'nombre_asc':   $order_by = 'ORDER BY contratista_listado.Nombre ASC ';     $bread_order = '<i class="fa fa-sort-alpha-asc" aria-hidden="true"></i> Nombre Ascendente';break;
		case 'nombre_desc':  $order_by = 'ORDER BY contratista_listado.Nombre DESC ';    $bread_order = '<i class="fa fa-sort-alpha-desc" aria-hidden="true"></i> Nombre Descendente';break;
		...
		default: $order_by = 'ORDER BY contratista_listado.Nombre ASC '; $bread_order = '<i class="fa fa-sort-alpha-asc" aria-hidden="true"></i> Nombre Ascendente';
	}
}else{
	$order_by = 'ORDER BY contratista_listado.Nombre ASC '; $bread_order = '<i class="fa fa-sort-alpha-asc" aria-hidden="true"></i> Nombre Ascendente';
}
/**********************************************************/
//Variable de busqueda
$z = "WHERE contratista_listado.idContratista!=0";
//verifico que sea un administrador
$z.=" AND contratista_listado.idSistema={$_SESSION['usuario']['basic_data']['idSistema']}";	

/**********************************************************/
//Se aplican los filtros
if(isset($_GET['idTipo']) && $_GET['idTipo'] != ''){           $z .= " AND contratista_listado.idTipo=".$_GET['idTipo'];}
if(isset($_GET['Nombre']) && $_GET['Nombre'] != ''){           $z .= " AND contratista_listado.Nombre LIKE '%".$_GET['Nombre']."%'";}
if(isset($_GET['Rut']) && $_GET['Rut'] != ''){                 $z .= " AND contratista_listado.Rut LIKE '%".$_GET['Rut']."%'";}
if(isset($_GET['fNacimiento']) && $_GET['fNacimiento'] != ''){ $z .= " AND contratista_listado.fNacimiento='".$_GET['fNacimiento']."'";}
if(isset($_GET['idPais']) && $_GET['idPais'] != ''){           $z .= " AND contratista_listado.idPais=".$_GET['idPais'];}
if(isset($_GET['idCiudad']) && $_GET['idCiudad'] != ''){       $z .= " AND contratista_listado.idCiudad=".$_GET['idCiudad'];}
if(isset($_GET['idComuna']) && $_GET['idComuna'] != ''){       $z .= " AND contratista_listado.idComuna=".$_GET['idComuna'];}
if(isset($_GET['Direccion']) && $_GET['Direccion'] != ''){     $z .= " AND contratista_listado.Direccion LIKE '%".$_GET['Direccion']."%'";}
if(isset($_GET['Giro']) && $_GET['Giro'] != ''){               $z .= " AND contratista_listado.Giro LIKE '%".$_GET['Giro']."%'";}
/**********************************************************/
//Realizo una consulta para saber el total de elementos existentes
$query = "SELECT contratista_listado.idContratista FROM `contratista_listado` ".$z;
//Consulta
$resultado = mysqli_query ($dbConn, $query);
//Si ejecuto correctamente la consulta
if(!$resultado){
	...
	<div class="well">
		<div class="col-sm-8 fcenter">
			<form class="form-horizontal" id="form1" name="form1" action="<?php echo $location; ?>" novalidate>
				...
				//se dibujan los inputs
				$Form_Imputs = new Form_Inputs();
				$Form_Imputs->form_select('Tipo de Contratista','idTipo', $x1, 1, 'idTipo', 'Nombre', 'contratista_tipos', 0, '', $dbConn);
				...
				$Form_Imputs->form_select_depend1('Ciudad','idCiudad', $x6, 1, 'idCiudad', 'Nombre', 'core_ubicacion_ciudad', 0, 0,
										'Comuna','idComuna', $x7, 1, 'idComuna', 'Nombre', 'core_ubicacion_comunas', 0, 0, 
										 $dbConn, 'form1');
				$Form_Imputs->form_input_icon( 'Direccion', 'Direccion', $x8, 1,'fa fa-map'); 
				$Form_Imputs->form_input_icon( 'Giro de la empresa', 'Giro', $x10, 1,'fa fa-industry');
...
<div class="col-sm-12">
	<div class="box">	
		<header>		
			<div class="icons"><i class="fa fa-table"></i></div><h5>Listado de Contratistas</h5>	
			<div class="toolbar">
				<?php 
					...
					<tr class="odd">		
						...
						<td>
							<div class="btn-group" style="width: 105px;" >
								<?php if ($rowlevel['level']>=1){?><a href="<?php echo 'view_contratista.php?view='.$usuarios['idContratista']; ?>" title="Ver Informacion" class="iframe btn btn-primary btn-sm tooltip"><i class="fa fa-list"></i></a><?php } ?>
								<?php if ($rowlevel['level']>=2){?><a href="<?php echo $location.'&id='.$usuarios['idContratista']; ?>" title="Editar Informacion" class="btn btn-success btn-sm tooltip"><i class="fa fa-pencil-square-o"></i></a><?php } ?>
								<?php if ($rowlevel['level']>=4){
									$ubicacion = $location.'&del='.$usuarios['idContratista'];
