...
$original = "productores_listado.php";
$location = $original;
...
if (!empty($_POST['submit'])){
	...
	$form_trabajo= 'insert';
	require_once 'A1XRXS_sys/xrxs_form/productores_listado.php';
}
//se borra un dato
if (!empty($_GET['del'])){
	//Llamamos al formulario
...
if (isset($_GET['created'])){ $error['created'] = 'sucess/Productor Creado correctamente';}
if (isset($_GET['edited'])){  $error['edited']  = 'sucess/Productor Modificado correctamente';}
if (isset($_GET['deleted'])){ $error['deleted'] = 'sucess/Productor Borrado correctamente';}
...
if(!empty($_GET['id'])){
	...
	productores_listado.Nombre,
	productores_listado.Codigo,
	productores_listado.Rut,
	productores_listado.RazonSocial,
	productores_listado.fNacimiento,
	productores_listado.Direccion,
	productores_listado.Fono1,
	productores_listado.Fono2,
	productores_listado.Fax,
	...
	productores_tipos.Nombre AS tipoCliente,
	core_rubros.Nombre AS Rubro';
	$SIS_join  = '
	LEFT JOIN `core_estados`              ON core_estados.idEstado                    = productores_listado.idEstado
	LEFT JOIN `core_ubicacion_ciudad`     ON core_ubicacion_ciudad.idCiudad           = productores_listado.idCiudad
	LEFT JOIN `core_ubicacion_comunas`    ON core_ubicacion_comunas.idComuna          = productores_listado.idComuna
	...
	$SIS_where = 'productores_listado.idProductor = '.$_GET['id'];
		...
		<div class="box">
			<header>
				<ul class="nav nav-tabs pull-right">
					...
					<li class=""><a href="<?php echo 'productores_listado_datos.php?pagina='.$_GET['pagina'].'&id='.$_GET['id']?>" ><i class="fa fa-list-alt" aria-hidden="true"></i> Datos Básicos</a></li>
...
<?php //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	if(isset($_GET['order_by'])&&$_GET['order_by']!=''){
		switch ($_GET['order_by']) {
			case 'rut_asc':       $order_by = 'productores_listado.Rut ASC ';       $bread_order = '<i class="fa fa-sort-alpha-asc" aria-hidden="true"></i> Rut Ascendente'; break;
			case 'rut_desc':      $order_by = 'productores_listado.Rut DESC ';      $bread_order = '<i class="fa fa-sort-alpha-desc" aria-hidden="true"></i> Rut Descendente';break;
			case 'nombre_asc':    $order_by = 'productores_listado.Nombre ASC ';    $bread_order = '<i class="fa fa-sort-alpha-asc" aria-hidden="true"></i> Nombre Ascendente';break;
			case 'nombre_desc':   $order_by = 'productores_listado.Nombre DESC ';   $bread_order = '<i class="fa fa-sort-alpha-desc" aria-hidden="true"></i> Nombre Descendente';break;
			case 'estado_asc':    $order_by = 'productores_listado.idEstado ASC ';  $bread_order = '<i class="fa fa-sort-alpha-asc" aria-hidden="true"></i> Estado Ascendente';break;
			case 'estado_desc':   $order_by = 'productores_listado.idEstado DESC '; $bread_order = '<i class="fa fa-sort-alpha-desc" aria-hidden="true"></i> Estado Descendente';break;

			default: $order_by = 'productores_listado.Nombre ASC '; $bread_order = '<i class="fa fa-sort-alpha-asc" aria-hidden="true"></i> Nombre Ascendente';
		}
	}else{
		$order_by = 'productores_listado.Nombre ASC '; $bread_order = '<i class="fa fa-sort-alpha-asc" aria-hidden="true"></i> Nombre Ascendente';
	}
	/**********************************************************/
	//Variable de busqueda
	$SIS_where = "productores_listado.idProductor!=0";
	//verifico que sea un administrador
	$SIS_where.= " AND productores_listado.idSistema=".$_SESSION['usuario']['basic_data']['idSistema'];
	/**********************************************************/
	//Se aplican los filtros
	if(isset($_GET['idTipo']) && $_GET['idTipo']!=''){             $SIS_where .= " AND productores_listado.idTipo=".$_GET['idTipo'];}
	if(isset($_GET['Codigo']) && $_GET['Codigo']!=''){             $SIS_where .= " AND productores_listado.Codigo LIKE '%".EstandarizarInput($_GET['Codigo'])."%'";}
	if(isset($_GET['Nombre']) && $_GET['Nombre']!=''){             $SIS_where .= " AND productores_listado.Nombre LIKE '%".EstandarizarInput($_GET['Nombre'])."%'";}
	if(isset($_GET['Rut']) && $_GET['Rut']!=''){                   $SIS_where .= " AND productores_listado.Rut LIKE '%".EstandarizarInput($_GET['Rut'])."%'";}
	if(isset($_GET['fNacimiento']) && $_GET['fNacimiento']!=''){   $SIS_where .= " AND productores_listado.fNacimiento='".$_GET['fNacimiento']."'";}
	if(isset($_GET['idCiudad']) && $_GET['idCiudad']!=''){         $SIS_where .= " AND productores_listado.idCiudad=".$_GET['idCiudad'];}
	if(isset($_GET['idComuna']) && $_GET['idComuna']!=''){         $SIS_where .= " AND productores_listado.idComuna=".$_GET['idComuna'];}
	if(isset($_GET['Direccion']) && $_GET['Direccion']!=''){       $SIS_where .= " AND productores_listado.Direccion LIKE '%".EstandarizarInput($_GET['Direccion'])."%'";}
	if(isset($_GET['Giro']) && $_GET['Giro']!=''){                 $SIS_where .= " AND productores_listado.Giro LIKE '%".EstandarizarInput($_GET['Giro'])."%'";}

	/**********************************************************/
	//Realizo una consulta para saber el total de elementos existentes
	$cuenta_registros = db_select_nrows (false, 'idProductor', 'productores_listado', '', $SIS_where, $dbConn, $_SESSION['usuario']['basic_data']['Nombre'], $original, 'cuenta_registros');
	//Realizo la operacion para saber la cantidad de paginas que hay
	$total_paginas = ceil($cuenta_registros / $cant_reg);
	// Se trae un listado con todos los elementos
	$SIS_query = '
	productores_listado.idProductor,
	productores_listado.Rut,
	productores_listado.Nombre,
	productores_listado.Codigo,
	core_estados.Nombre AS estado,
	core_sistemas.Nombre AS sistema,
	productores_listado.idEstado';
	$SIS_join  = '
	LEFT JOIN `core_estados`   ON core_estados.idEstado       = productores_listado.idEstado
	LEFT JOIN `core_sistemas`  ON core_sistemas.idSistema     = productores_listado.idSistema';
	$SIS_order = $order_by.' LIMIT '.$comienzo.', '.$cant_reg;
	$arrUsers = array();
	$arrUsers = db_select_array (false, $SIS_query, 'productores_listado', $SIS_join, $SIS_where, $SIS_order, $dbConn, $_SESSION['usuario']['basic_data']['Nombre'], $original, 'arrUsers');
	...
	<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 breadcrumb-bar">
		...
		<?php if ($rowlevel['level']>=3){ ?><a href="<?php echo $location; ?>&new=true" class="btn btn-default pull-right margin_width fmrbtn" ><i class="fa fa-file-o" aria-hidden="true"></i> Crear Productor</a><?php } ?>
				...
				<table id="dataTable" class="table table-bordered table-condensed table-hover table-striped dataTable">
					...
					<tbody role="alert" aria-live="polite" aria-relevant="all">
						...
						<tr class="odd">
							...
							<td><?php echo $usuarios['Codigo'].' - '.$usuarios['Nombre']; ?></td>
							...
							<td>
								<div class="btn-group" style="width: 105px;" >
									<?php if ($rowlevel['level']>=1){ ?><a href="<?php echo 'view_productor.php?view='.simpleEncode($usuarios['idProductor'], fecha_actual()); ?>" title="Ver Información" class="iframe btn btn-primary btn-sm tooltip"><i class="fa fa-list" aria-hidden="true"></i></a><?php } ?>
									<?php if ($rowlevel['level']>=2){ ?><a href="<?php echo $location.'&id='.$usuarios['idProductor']; ?>" title="Editar Información" class="btn btn-success btn-sm tooltip"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></a><?php } ?>
