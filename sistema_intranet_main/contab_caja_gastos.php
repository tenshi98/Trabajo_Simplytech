...
if (isset($_GET['created'])) {$error['usuario'] 	  = 'sucess/Rendicion Realizado correctamente';}
if (isset($_GET['edited']))  {$error['usuario'] 	  = 'sucess/Rendicion Modificado correctamente';}
if (isset($_GET['deleted'])) {$error['usuario'] 	  = 'sucess/Rendicion borrado correctamente';}
	...
	<div class="box dark">
		...
		<div id="div-1" class="body">
			<form class="form-horizontal" method="post" id="form1" name="form1" novalidate>
				...
				$Form_Inputs->form_select_filter('Documento de Pago','idDocPago', $x2, 2, 'idDocPago', 'Nombre', 'sistema_documentos_pago', 0, '', $dbConn);
				$Form_Inputs->form_input_number('N° Documento de Pago', 'N_Doc', $x3, 1);
				$Form_Inputs->form_input_number('Valor Total Neto', 'Valor', $x4, 2);
				$Form_Inputs->form_select_depend5('Centro de Costo', 'idCentroCosto',  $x5,  2,  'idCentroCosto',  'Nombre',  'centrocosto_listado',  $w,   0,
												  'Nivel 1', 'idLevel_1',  $x6,  1,  'idLevel_1',  'Nombre',  'centrocosto_listado_level_1',  0,   0, 
												  'Nivel 2', 'idLevel_2',  $x7,  1,  'idLevel_2',  'Nombre',  'centrocosto_listado_level_2',  0,   0,
												  'Nivel 3', 'idLevel_3',  $x8,  1,  'idLevel_3',  'Nombre',  'centrocosto_listado_level_3',  0,   0,
	...
	<div class="box dark">
		...
		<div id="div-1" class="body">
			<form class="form-horizontal" method="post" id="form1" name="form1" novalidate>
				...
				$Form_Inputs->form_select_filter('Documento de Pago','idDocPago', $x2, 2, 'idDocPago', 'Nombre', 'sistema_documentos_pago', 0, '', $dbConn);
				$Form_Inputs->form_input_number('N° Documento de Pago', 'N_Doc', $x3, 1);
				$Form_Inputs->form_input_number('Monto', 'Valor', $x4, 2);
				$Form_Inputs->form_select_depend5('Centro de Costo', 'idCentroCosto',  $x5,  2,  'idCentroCosto',  'Nombre',  'centrocosto_listado',  $w,   0,
												  'Nivel 1', 'idLevel_1',  $x6,  1,  'idLevel_1',  'Nombre',  'centrocosto_listado_level_1',  0,   0, 
												  'Nivel 2', 'idLevel_2',  $x7,  1,  'idLevel_2',  'Nombre',  'centrocosto_listado_level_2',  0,   0,
												  'Nivel 3', 'idLevel_3',  $x8,  1,  'idLevel_3',  'Nombre',  'centrocosto_listado_level_3',  0,   0,
...
<div class="col-sm-12">
			...
			<table id="meta" class="fleft otdata">
				<tbody>
					...
					<tr>
						...
						<td><?php echo $_SESSION['contab_caja_gastos_basicos']['Trabajador']; ?></td>
					</tr>
					<tr>
						<td class="meta-head">Rut</td>
						<td><?php echo $_SESSION['contab_caja_gastos_basicos']['Rut']; ?></td>
					</tr>
					<tr>
						<td class="meta-head">Cargo</td>
						<td><?php echo $_SESSION['contab_caja_gastos_basicos']['Cargo']; ?></td>
					</tr>
					<tr>
		...
		<table id="items">
			<tbody>
				...
				<tr class="item-row fact_tittle">
					...
					<td>Valor</td>
					<td>
						<a href="<?php echo $location.'&addMonto=true' ?>" title="Agregar Rendicion" class="btn btn-xs btn-primary tooltip" style="position: initial;"><i class="fa fa-plus" aria-hidden="true"></i> Agregar Rendicion</a>
					</td>
				</tr>
				<?php 
				$vtotal = 0;
						...
						<tr class="item-row linea_punteada">
							...
							<td>
								<div class="btn-group" style="width: 70px;" >
									<a href="<?php echo $location.'&editMonto='.$producto['bvar']; ?>" title="Editar Rendicion" class="btn btn-success btn-sm tooltip"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></a>
									<?php 
									$ubicacion = $location.'&del_monto='.$producto['bvar'];
									$dialogo   = '¿Realmente deseas eliminar el Rendicion '.str_replace('"','',$producto['DocPago']).'?';?>
				...
				echo '<tr id="hiderow"><td colspan="5"><a name="Ancla_obs"></a></td></tr>';?>
				

				
				<tr class="invoice-total" bgcolor="#f1f1f1">
					<td colspan="3" align="right"> <strong>Total</strong></td>    
					<td align="right"><?php echo Valores($vtotal, 0);$_SESSION['contab_caja_gastos_basicos']['Valor'] = $vtotal;?></td>
					<td></td>
				</tr>
    ...
    <div class="row">
		<div class="col-xs-12">
			...
			<p class="text-muted well well-sm no-shadow" ><?php echo $_SESSION['contab_caja_gastos_basicos']['Observaciones'];?></p>
		</div>
	</div>
...
if(isset($_GET['order_by'])&&$_GET['order_by']!=''){
	switch ($_GET['order_by']) {
		case 'fecha_asc':       $order_by = 'ORDER BY contab_caja_gastos.Creacion_fecha ASC ';                               $bread_order = '<i class="fa fa-sort-alpha-asc" aria-hidden="true"></i> Fecha Ascendente';break;
		case 'fecha_desc':      $order_by = 'ORDER BY contab_caja_gastos.Creacion_fecha DESC ';                              $bread_order = '<i class="fa fa-sort-alpha-desc" aria-hidden="true"></i> Fecha Descendente';break;
		case 'monto_asc':       $order_by = 'ORDER BY contab_caja_gastos.Valor ASC ';                                        $bread_order = '<i class="fa fa-sort-alpha-asc" aria-hidden="true"></i> Monto Ascendente';break;
		case 'monto_desc':      $order_by = 'ORDER BY contab_caja_gastos.Valor DESC ';                                       $bread_order = '<i class="fa fa-sort-alpha-desc" aria-hidden="true"></i> Monto Descendente';break;
		case 'trabajador_asc':  $order_by = 'ORDER BY trabajadores_listado.ApellidoPat ASC, trabajadores_listado.Nombre ASC ';   $bread_order = '<i class="fa fa-sort-alpha-asc" aria-hidden="true"></i> Trabajador Ascendente';break;
		case 'trabajador_desc': $order_by = 'ORDER BY trabajadores_listado.ApellidoPat DESC, trabajadores_listado.Nombre ASC ';  $bread_order = '<i class="fa fa-sort-alpha-desc" aria-hidden="true"></i> Trabajador Descendente';break;
		
		default: $order_by = 'ORDER BY contab_caja_gastos.Creacion_fecha DESC '; $bread_order = '<i class="fa fa-sort-alpha-desc" aria-hidden="true"></i> Fecha Descendente';
	}
}else{
	$order_by = 'ORDER BY contab_caja_gastos.Creacion_fecha DESC '; $bread_order = '<i class="fa fa-sort-alpha-desc" aria-hidden="true"></i> Fecha Descendente';
}
/**********************************************************/
...
$z="WHERE contab_caja_gastos.idFacturacion!=0";
//Verifico el tipo de usuario que esta ingresando
$z.=" AND contab_caja_gastos.idSistema=".$_SESSION['usuario']['basic_data']['idSistema'];	

/**********************************************************/
//Se aplican los filtros
if(isset($_GET['Creacion_fecha']) && $_GET['Creacion_fecha'] != ''){  $z .= " AND contab_caja_gastos.Creacion_fecha='".$_GET['Creacion_fecha']."'";}
if(isset($_GET['idTrabajador']) && $_GET['idTrabajador'] != ''){      $z .= " AND contab_caja_gastos.idTrabajador=".$_GET['idTrabajador'];}
/**********************************************************/
//Realizo una consulta para saber el total de elementos existentes
$query = "SELECT idFacturacion FROM `contab_caja_gastos` ".$z;
//Consulta
					...
					<tr class="odd">
						...
						<td>
							<div class="btn-group" style="width: 35px;" >
								<?php if ($rowlevel['level']>=1){?><a href="<?php echo 'view_mov_contab_caja_gastos.php?view='.simpleEncode($tipo['idFacturacion'], fecha_actual()); ?>" title="Ver Informacion" class="iframe btn btn-primary btn-sm tooltip"><i class="fa fa-list" aria-hidden="true"></i></a><?php } ?>
							</div>
						</td>
					</tr>
					<?php } ?>                    
