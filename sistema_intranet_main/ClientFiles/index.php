...
//reasigno la carpeta principal
if(isset($_SESSION['Rut'])&&$_SESSION['Rut']!=''){
	$options['general']['root_dir'] = $options['general']['root_dir'].'Cliente_'.$_SESSION['Rut'].'/';
}
...
switch ($options['bootstrap']['icons']) {
    ...
    case "glyphicons":
        ...
        $icons['search'] = "          <i class=\"fa fa-search form-control-feedback\" aria-hidden=\"true\" style=\"top: 9px;\"></i> " . PHP_EOL;
...
foreach($dir_name as $dir => $name) :
    if(($name != ' ') && ($name != '') && ($name != '.') && ($name != '/')):
        ...
        $breadcrumbs .= "      <li class=\"btn btn-default\"><a href=\"".htmlentities($absolute_path.$parent, ENT_QUOTES, 'utf-8')."\">".$name."</a></li>" . PHP_EOL;
    endif;
endforeach;
$breadcrumbs = $breadcrumbs."    </ul>" . PHP_EOL;
...
if ($options['general']['enable_search'] == true) {
    ...
    $search  = "    <div class=\"pull-right\">" . PHP_EOL;
    $search .= "      <div class=\"col-xs-12 col-sm-12 col-md-12\">" . PHP_EOL;
    ...
    $search .= "            <input type=\"text\" class=\"form-control$input_size\" id=\"search\" placeholder=\"Buscar\"$autofocus>" . PHP_EOL;
    $search .= $icons['search'];
...
$table_header .= "            <th class=\"".$column_name." text-".$left."\" data-sort=\"string\">"._('Nombre')."</th>" . PHP_EOL;
...
if ($table_options['size']) {
    ...
    $table_header .= _('Tamaño')."</th>" . PHP_EOL;
}
...
if ($table_options['age']) {
    ...
    $table_header .= _('Ultima Modificacion')."</th>" . PHP_EOL;
}
