...
$original = "bodegas_productos_traspaso.php";
$location = $original;
...
if ( !empty($_POST['submit_modCentroCosto']) )  { 
	//Llamamos al formulario
	$form_trabajo= 'modCentroCosto_traspaso';
	require_once 'A1XRXS_sys/xrxs_form/z_bodega_productos.php';
}
/******************************************/
//formulario para crear
...
productos_listado.ValorIngreso,
	...
	AND idBodega=".$_SESSION['productos_traspaso_basicos']['idBodegaOrigen']." LIMIT 1) AS ingreso,
				
	(SELECT 
	SUM(Cantidad_eg) AS egreso
	FROM `bodegas_productos_facturacion_existencias`
	WHERE idProducto = productos_listado.idProducto 
	AND idBodega=".$_SESSION['productos_traspaso_basicos']['idBodegaOrigen']." LIMIT 1) AS egreso
FROM `productos_listado`
LEFT JOIN `sistema_productos_uml` ON sistema_productos_uml.idUml = productos_listado.idUml
WHERE productos_listado.idProducto='{$_SESSION['productos_traspaso_productos'][$_GET['editProd']]['idProducto']}'
	...
	<div class="box dark">
		...
		<div id="div-1" class="body">
			<form class="form-horizontal" method="post" id="form1" name="form1" novalidate>
				...
				if(isset($idProducto)) {       $x1  = $idProducto;      }else{$x1  = $_SESSION['productos_traspaso_productos'][$_GET['editProd']]['idProducto'];}
				if(isset($Number)) {           $x2  = $Number;          }else{$x2  = $_SESSION['productos_traspaso_productos'][$_GET['editProd']]['Number'];}
				...
				$Form_Imputs->form_input_disabled('Valor Unitario','Unitario', Cantidades_decimales_justos($_SESSION['productos_traspaso_productos'][$_GET['editProd']]['ValorEgreso']), 1);
				$Form_Imputs->form_input_hidden('ValorEgreso', Cantidades_decimales_justos($_SESSION['productos_traspaso_productos'][$_GET['editProd']]['ValorEgreso']), 2);
...
<script>
	...
	function OnSelectionChange (select) {
		...
		FROM `bodegas_productos_facturacion_existencias`
		WHERE idProducto = productos_listado.idProducto 
		AND idBodega=".$_SESSION['productos_traspaso_basicos']['idBodegaOrigen'].") AS ingreso,
					
		(SELECT 
		SUM(Cantidad_eg) AS egreso
		FROM `bodegas_productos_facturacion_existencias`
		WHERE idProducto = productos_listado.idProducto 
		AND idBodega=".$_SESSION['productos_traspaso_basicos']['idBodegaOrigen'].") AS egreso
	...
	<div class="box dark">
		...
		<div id="div-1" class="body">
			<form class="form-horizontal" method="post" id="form1" name="form1" novalidate>
				...
				<?php 
				//Se verifican si existen los datos
				if(isset($idCentroCosto)) {  $x1  = $idCentroCosto;  }else{$x1  = $_SESSION['productos_traspaso_basicos']['idCentroCosto'];}
				if(isset($idLevel_1)) {      $x2  = $idLevel_1;      }else{$x2  = $_SESSION['productos_traspaso_basicos']['idLevel_1'];}
				if(isset($idLevel_2)) {      $x3  = $idLevel_2;      }else{$x3  = $_SESSION['productos_traspaso_basicos']['idLevel_2'];}
				if(isset($idLevel_3)) {      $x4  = $idLevel_3;      }else{$x4  = $_SESSION['productos_traspaso_basicos']['idLevel_3'];}
				if(isset($idLevel_4)) {      $x5  = $idLevel_4;      }else{$x5  = $_SESSION['productos_traspaso_basicos']['idLevel_4'];}
				if(isset($idLevel_5)) {      $x6  = $idLevel_5;      }else{$x6  = $_SESSION['productos_traspaso_basicos']['idLevel_5'];}
	...
	<div class="box dark">
		...
		<div id="div-1" class="body">
			<form class="form-horizontal" method="post" id="form1" name="form1" novalidate>
				...
				<?php 
				//Se verifican si existen los datos
				if(isset($Creacion_fecha)) {   $x1  = $Creacion_fecha;    }else{$x1  = $_SESSION['productos_traspaso_basicos']['Creacion_fecha'];}
				if(isset($idBodegaOrigen)) {   $x2  = $idBodegaOrigen;    }else{$x2  = $_SESSION['productos_traspaso_basicos']['idBodegaOrigen'];}
				if(isset($idBodegaDestino)) {  $x3  = $idBodegaDestino;   }else{$x3  = $_SESSION['productos_traspaso_basicos']['idBodegaDestino'];}
				
				//se dibujan los inputs
				$Form_Imputs = new Form_Inputs();
				$Form_Imputs->form_date('Fecha','Creacion_fecha', $x1, 2);
				$Form_Imputs->form_select_join_filter('Bodega Origen','idBodegaOrigen', $x2, 2, 'idBodega', 'Nombre', 'bodegas_productos_listado', 'usuarios_bodegas_productos', $z1, $dbConn);
				$Form_Imputs->form_select('Bodega Destino','idBodegaDestino', $x3, 2, 'idBodega', 'Nombre', 'bodegas_productos_listado', $z2, '', $dbConn);
...
<div class="col-sm-12 fcenter">
	...
	<div id="page-wrap">
		<div id="header"> <?php echo $_SESSION['productos_traspaso_basicos']['TipoDocumento']; ?></div>
			...
			<table id="meta" class="fleft otdata">
				<tbody>
					...
					<tr>
						...
						<td><?php echo $_SESSION['productos_traspaso_basicos']['BodegaOrigen']; ?></td>
					</tr>
					<tr>
						<td class="meta-head">Bodega Destino</td>
						<td><?php echo $_SESSION['productos_traspaso_basicos']['BodegaDestino']; ?></td>
					</tr>
					<tr>
					...
					<tr>
						...
						<td><?php echo $_SESSION['productos_traspaso_basicos']['CentroCosto']; ?></td>
					</tr>
				</tbody>
			</table>
		...
		<table id="items">
			<tbody>
				...
				<tr>
					<?php if(isset($_SESSION['productos_traspaso_basicos']['Observaciones'])&&$_SESSION['productos_traspaso_basicos']['Observaciones']!=''){ ?>
					
						<td colspan="5" class="blank word_break"> 
							<?php echo $_SESSION['productos_traspaso_basicos']['Observaciones'];?>
						</td>
						<td class="blank">
					...
					<?php }else{?>
						<td colspan="5" class="blank"> 
							<?php 
							$non = '';
							if(isset($_SESSION['productos_traspaso_temporal'])&&$_SESSION['productos_traspaso_temporal']!=''){
								$non = $_SESSION['productos_traspaso_temporal'];
							}	
	...
	<div class="box dark">
		...
		<div id="div-1" class="body">
			<form class="form-horizontal" method="post" id="form1" name="form1" novalidate>
				...
				if(isset($idBodegaDestino)) {  $x3  = $idBodegaDestino;   }else{$x3  = '';}
...
if(isset($_GET['order_by'])&&$_GET['order_by']!=''){
	switch ($_GET['order_by']) {
		case 'bodega_ori_asc':     $order_by = 'ORDER BY bodega1.Nombre ASC ';                                $bread_order = '<i class="fa fa-sort-alpha-asc" aria-hidden="true"></i> Bodega Origen Ascendente'; break;
		case 'bodega_ori_desc':    $order_by = 'ORDER BY bodega1.Nombre DESC ';                               $bread_order = '<i class="fa fa-sort-alpha-desc" aria-hidden="true"></i> Bodega Origen Descendente';break;
		case 'bodega_dest_asc':    $order_by = 'ORDER BY bodega2.Nombre ASC ';                                $bread_order = '<i class="fa fa-sort-alpha-asc" aria-hidden="true"></i> Bodega Destino Ascendente';break;
		case 'bodega_dest_desc':   $order_by = 'ORDER BY bodega2.Nombre DESC ';                               $bread_order = '<i class="fa fa-sort-alpha-desc" aria-hidden="true"></i> Bodega Destino Descendente';break;
...
$z="WHERE bodegas_productos_facturacion.idTipo=4";//Solo traspaso
//Verifico el tipo de usuario que esta ingresando
	...
	<div class="well">
		<div class="col-sm-8 fcenter">
			<form class="form-horizontal" id="form1" name="form1" action="<?php echo $location; ?>" novalidate>
				...
				$Form_Imputs->form_select_join_filter('Bodega Origen','idBodegaOrigen', $x2, 1, 'idBodega', 'Nombre', 'bodegas_productos_listado', 'usuarios_bodegas_productos', $z1, $dbConn);
				$Form_Imputs->form_select('Bodega Destino','idBodegaDestino', $x3, 1, 'idBodega', 'Nombre', 'bodegas_productos_listado', $z2, '', $dbConn);
				$Form_Imputs->form_textarea('Observaciones','Observaciones', $x4, 1, 160);
					...
					<tr role="row">
						<th>
							...
							<div class="btn-group pull-right" style="width: 50px;" >
								<a href="<?php echo $location.'&order_by=bodega_ori_asc'; ?>" title="Ascendente" class="btn btn-default btn-xs tooltip"><i class="fa fa-sort-alpha-asc"></i></a>
								<a href="<?php echo $location.'&order_by=bodega_ori_desc'; ?>" title="Descendente" class="btn btn-default btn-xs tooltip"><i class="fa fa-sort-alpha-desc"></i></a>
							</div>
						</th>
						<th>
							<div class="pull-left">Bodega Destino</div>
							<div class="btn-group pull-right" style="width: 50px;" >
								<a href="<?php echo $location.'&order_by=bodega_dest_asc'; ?>" title="Ascendente" class="btn btn-default btn-xs tooltip"><i class="fa fa-sort-alpha-asc"></i></a>
								<a href="<?php echo $location.'&order_by=bodega_dest_desc'; ?>" title="Descendente" class="btn btn-default btn-xs tooltip"><i class="fa fa-sort-alpha-desc"></i></a>
							</div>
						</th>
						<th>
