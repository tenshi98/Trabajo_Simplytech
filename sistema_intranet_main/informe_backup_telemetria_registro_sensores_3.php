...
$original = "informe_backup_telemetria_registro_sensores_3.php";
$location = $original;
...
if(isset($_GET['idTelemetria'])&&$_GET['idTelemetria']!=''){
	...
	$rowData = db_select_data (false, $SIS_query, 'backup_telemetria_listado_tablarelacionada_'.$_GET['idTelemetria'], $SIS_join, $SIS_where, $dbConn, $_SESSION['usuario']['basic_data']['Nombre'], $original, 'rowData');
	...
	for ($i = 1; $i <= $total_files; $i++) {
		...
		$Alert_Text .= '<a target="new" href="informe_backup_telemetria_registro_sensores_3_to_excel.php?bla=bla'.$datosx.'" class="btn btn-sm btn-metis-2 pull-right "><i class="fa fa-file-excel-o" aria-hidden="true"></i> Exportar a Excel</a>';
...
//Si no se slecciono se traen todos los equipos a los cuales tiene permiso
	...
	foreach ($arrEquipos as $equipo) {
		...
		$rowData = db_select_data (false, $SIS_query, 'backup_telemetria_listado_tablarelacionada_'.$equipo['idTelemetria'], '', $SIS_where, $dbConn, $_SESSION['usuario']['basic_data']['Nombre'], $original, 'rowData');
