...
$location = $original;
$new_location = "sistema_listado_datos_variedades_especies.php";
...
if ( !empty($_GET['sis_especie_add']) )  { 
	//Nueva ubicacion
	$location = $new_location;
	$location.='&id='.$_GET['id'];
	//Llamamos al formulario
	$form_trabajo= 'sis_especie_add';
	require_once 'A1XRXS_sys/xrxs_form/core_sistemas.php';
}
//se borra un dato
if ( !empty($_GET['sis_especie_del']) )     {
	//nueva ubicacion
	$location = $new_location;
	$location.='&id='.$_GET['id'];
	//Llamamos al formulario
	$form_trabajo= 'sis_especie_del';
	require_once 'A1XRXS_sys/xrxs_form/core_sistemas.php';	
}
/**********************************************************************************************************************************/
...
$arrProductos = array();
$query = "SELECT 
sistema_variedades_categorias.idCategoria,
sistema_variedades_categorias.Nombre,
(SELECT COUNT(idSisProd) FROM core_sistemas_variedades_categorias WHERE idCategoria = sistema_variedades_categorias.idCategoria AND idSistema = {$_GET['id']} LIMIT 1) AS contar,
(SELECT idSisProd FROM core_sistemas_variedades_categorias WHERE idCategoria = sistema_variedades_categorias.idCategoria AND idSistema = {$_GET['id']} LIMIT 1) AS idpermiso
	...
	<div class="col-md-6 col-sm-6 col-xs-12" style="padding-left: 0px;">
		<div class="info-box bg-aqua">
			...
			<div class="info-box-content">
				...
				<span class="progress-description">Editar Especies Usadas</span>
			</div>
		</div>
	</div>
</div>
	...
	<div class="box">
		<header>
			<ul class="nav nav-tabs pull-right">
				<li class=""><a href="<?php echo 'sistema_listado.php?pagina='.$_GET['pagina'].'&id='.$_GET['id']?>" >Resumen</a></li>
				...
				<li class="dropdown">
					...
					<ul class="dropdown-menu" role="menu">
						...
						<?php if(isset($Count_Variedades)&&$Count_Variedades!=0){?>
							<li class="active"><a href="<?php echo 'sistema_listado_datos_variedades_especies.php?pagina='.$_GET['pagina'].'&id='.$_GET['id'];?>" >Especies</a></li>
        ...
        <div class="table-responsive">
			<table id="dataTable" class="table table-bordered table-condensed table-hover table-striped dataTable">
				...
				<tbody role="alert" aria-live="polite" aria-relevant="all" id="TableFiltered">
					...
					<tr class="odd">
						<td><?php echo '<strong>Especie: </strong>'.$productos['Nombre']; ?></td>
						<td>
							<div class="btn-group" style="width: 100px;" id="toggle_event_editing">	
								<?php if ( isset($productos['contar'])&&$productos['contar']!='0' ) {?>    
									<a title="Quitar Permiso" class="btn btn-sm btn-default unlocked_inactive tooltip" href="<?php echo $new_location.'&id='.$_GET['id'].'&sis_especie_del='.$productos['idpermiso']; ?>">OFF</a>
								...
								<?php } else {?>
									...
									<a title="Dar Permiso" class="btn btn-sm btn-default unlocked_inactive tooltip" href="<?php echo $new_location.'&id='.$_GET['id'].'&sis_especie_add='.$productos['idCategoria']; ?>">ON</a>
								<?php }?>    
							</div> 
						</td>
					</tr>
