...
$location = $original;
$new_location = "productores_listado_observaciones.php";
...
if (!empty($_POST['submit'])){
	...
	$form_trabajo= 'insert';
	require_once 'A1XRXS_sys/xrxs_form/productores_observaciones.php';
}
...
FROM `productores_observaciones`
...
// consulto los datos
$query = "SELECT 
productores_listado.Nombre AS nombre_cliente,
usuarios_listado.Nombre AS nombre_usuario,
productores_observaciones.Fecha,
productores_observaciones.Observacion
FROM `productores_observaciones`
LEFT JOIN `productores_listado`   ON productores_listado.idProductor     = productores_observaciones.idProductor
LEFT JOIN `usuarios_listado`   ON usuarios_listado.idUsuario     = productores_observaciones.idUsuario
WHERE productores_observaciones.idObservacion = ".$_GET['view']."
ORDER BY productores_observaciones.idObservacion ASC ";
//Consulta
$resultado = mysqli_query ($dbConn, $query);
//Si ejecuto correctamente la consulta
if(!$resultado){
...
$query = "SELECT 
productores_observaciones.idObservacion,
productores_listado.Nombre AS nombre_cliente,
usuarios_listado.Nombre AS nombre_usuario,
productores_observaciones.Fecha,
productores_observaciones.Observacion
...
LEFT JOIN `productores_listado`   ON productores_listado.idProductor     = productores_observaciones.idProductor
LEFT JOIN `usuarios_listado`   ON usuarios_listado.idUsuario     = productores_observaciones.idUsuario
WHERE productores_observaciones.idProductor = ".$_GET['id']."
ORDER BY productores_observaciones.idObservacion ASC ";
//Consulta
$resultado = mysqli_query ($dbConn, $query);
//Si ejecuto correctamente la consulta
			...
			<ul class="nav nav-tabs pull-right">
				...
				<li class="dropdown">
					...
					<ul class="dropdown-menu" role="menu">
						...
						<li class="active"><a href="<?php echo 'productores_listado_observaciones.php?pagina='.$_GET['pagina'].'&id='.$_GET['id']?>" ><i class="fa fa-tasks" aria-hidden="true"></i> Observaciones</a></li>
					</ul>
                </li>
			</ul>
		</header>
