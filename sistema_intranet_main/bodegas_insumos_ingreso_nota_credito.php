...
$original = "bodegas_insumos_ingreso_nota_credito.php";
$location = $original;
...
if ( !empty($_POST['submit_prod']) )  { 
	//Llamamos al formulario
	$form_trabajo= 'new_prod_ing_nc';
	require_once 'A1XRXS_sys/xrxs_form/z_bodega_insumos.php';
}
//formulario para crear
if ( !empty($_POST['submit_edit_prod']) )  { 
	//Llamamos al formulario
	$form_trabajo= 'edit_prod_ing_nc';
	require_once 'A1XRXS_sys/xrxs_form/z_bodega_insumos.php';
}
//se borra un dato
if ( !empty($_GET['del_prod']) )     {
	//Llamamos al formulario
	$form_trabajo= 'del_prod_ing_nc';
	require_once 'A1XRXS_sys/xrxs_form/z_bodega_insumos.php';	
}
/**********************************************/
//formulario para crear
if ( !empty($_POST['submit_otros']) )  { 
	//Llamamos al formulario
	$form_trabajo= 'new_otros_ing_nc';
	require_once 'A1XRXS_sys/xrxs_form/z_bodega_insumos.php';
}
//formulario para crear
if ( !empty($_POST['submit_edit_otros']) )  { 
	//Llamamos al formulario
	$form_trabajo= 'edit_otros_ing_nc';
	require_once 'A1XRXS_sys/xrxs_form/z_bodega_insumos.php';
}
//se borra un dato
if ( !empty($_GET['del_otros']) )     {
	//Llamamos al formulario
	$form_trabajo= 'del_otros_ing_nc';
	require_once 'A1XRXS_sys/xrxs_form/z_bodega_insumos.php';	
}
/**********************************************/
//se borra un dato
if ( !empty($_GET['add_obs']) )     {
	//Llamamos al formulario
	$form_trabajo= 'add_obs_ing_nc';
	require_once 'A1XRXS_sys/xrxs_form/z_bodega_insumos.php';	
}
//se borra un dato
if ( !empty($_GET['del_obs']) )     {
	//Llamamos al formulario
	$form_trabajo= 'del_obs_ing_nc';
	require_once 'A1XRXS_sys/xrxs_form/z_bodega_insumos.php';	
}
/**********************************************/
//formulario para crear
if ( !empty($_POST['submit_impuesto']) )  { 
	//Llamamos al formulario
	$form_trabajo= 'new_impuesto_ing_nc';
	require_once 'A1XRXS_sys/xrxs_form/z_bodega_insumos.php';
}
//se borra un dato
if ( !empty($_GET['del_impuesto']) )     {
	//Llamamos al formulario
	$form_trabajo= 'del_impuesto_ing_nc';
	require_once 'A1XRXS_sys/xrxs_form/z_bodega_insumos.php';	
}
/**********************************************/
//formulario para crear
if ( !empty($_POST['submit_file']) )  { 
	//Llamamos al formulario
	$form_trabajo= 'new_file_ing_nc';
	require_once 'A1XRXS_sys/xrxs_form/z_bodega_insumos.php';
}
//se borra un dato
if ( !empty($_GET['del_file']) )     {
	//Llamamos al formulario
	$form_trabajo= 'del_file_ing_nc';
	require_once 'A1XRXS_sys/xrxs_form/z_bodega_insumos.php';	
}
/**********************************************/
if ( !empty($_GET['ing_bodega_nc']) )     {
	//Llamamos al formulario
	$form_trabajo= 'ing_bodega_nc';
	require_once 'A1XRXS_sys/xrxs_form/z_bodega_insumos.php';	
}
/**********************************************************************************************************************************/
...
WHERE insumos_listado.idProducto='{$_SESSION['insumos_ing_nc_productos'][$_GET['editProd']]['idProducto']}'
	...
	<div class="box dark">
		...
		<div id="div-1" class="body">
			<form class="form-horizontal" method="post" id="form1" name="form1" novalidate>
				...
				if(isset($idProducto)) {       $x1  = $idProducto;      }else{$x1  = $_SESSION['insumos_ing_nc_productos'][$_GET['editProd']]['idProducto'];}
				if(isset($Cantidad_eg)) {      $x2  = $Cantidad_eg;     }else{$x2  = $_SESSION['insumos_ing_nc_productos'][$_GET['editProd']]['Cantidad_eg'];}
				if(isset($ValorTotal)) {       $x3  = $ValorTotal;      }else{$x3  = $_SESSION['insumos_ing_nc_productos'][$_GET['editProd']]['ValorTotal'];}
				
				//se dibujan los inputs
				$Form_Imputs = new Form_Inputs();
				$Form_Imputs->form_select_filter('Insumo','idProducto', $x1, 2, 'idProducto', 'Nombre', 'insumos_listado', $zx2, '', $dbConn);
				$Form_Imputs->form_input_number('Cantidad', 'Cantidad_eg', $x2, 2);
				...
				$Form_Imputs->form_input_disabled('Valor Unitario Neto','Unitario', Cantidades_decimales_justos($_SESSION['insumos_ing_nc_productos'][$_GET['editProd']]['ValorIngreso']), 1);
				$Form_Imputs->form_input_number('Valor Total Neto', 'ValorTotal', $x3, 2);
				$Form_Imputs->form_input_hidden('ValorIngreso', Cantidades_decimales_justos($_SESSION['insumos_ing_nc_productos'][$_GET['editProd']]['ValorIngreso']), 2);
...
<div style="display: none;"> 
	<div id="clone_producto" class="prod_container"> 	
		...
		<div class="col-sm-2 nopadding">
			<div class="form-group">
				<?php $Form_Imputs->input_number('Cantidad','Cantidad_eg[]', '', 2);?>
			</div>
		</div>
		<div class="col-sm-2 nopadding">
			<div class="form-group">
		...
		<div class="col-sm-2 nopadding">
			<div class="form-group">
				<?php $Form_Imputs->input_disabled('text', 'Proveedor Actual', 'escribeme2', 0, 1);?>
			</div>
		</div>
		<div class="col-sm-1 nopadding">
			<div class="form-group">
				<?php $Form_Imputs->input_disabled('text', 'V Neto', 'escribeme3', 0, 1);?>
			</div>
		</div>
...
<script>
	...
	function OnSelectionChange (select) {
		...
		foreach ($arrTipo as $tipo) {
			echo 'var id_prov_'.$tipo['idProducto'].'= "'.$tipo['Proveedor'].'";';
	...
	<div class="box dark">
		...
		<div id="div-1" class="body">
			<form class="form-horizontal" method="post" id="form1" name="form1" novalidate>
				...
				<?php 
				//Se verifican si existen los datos
				if(isset($Nombre)) {  $x1  = $Nombre;  }else{$x1  = $_SESSION['insumos_ing_nc_otros'][$_GET['editOtros']]['Nombre'];}
				if(isset($vTotal)) {  $x2  = $vTotal;  }else{$x2  = Cantidades_decimales_justos($_SESSION['insumos_ing_nc_otros'][$_GET['editOtros']]['vTotal']);}
				...
				$Form_Imputs->form_input_hidden('oldidProducto', $_SESSION['insumos_ing_nc_otros'][$_GET['editOtros']]['idOtros'], 2);
	...
	<div class="box dark">
		...
		<div id="div-1" class="body">
			<form class="form-horizontal" method="post" id="form1" name="form1" novalidate>
				...
				<?php 
				//Se verifican si existen los datos
				if(isset($idCentroCosto)) {  $x1  = $idCentroCosto;  }else{$x1  = $_SESSION['insumos_ing_nc_basicos']['idCentroCosto'];}
				if(isset($idLevel_1)) {      $x2  = $idLevel_1;      }else{$x2  = $_SESSION['insumos_ing_nc_basicos']['idLevel_1'];}
				if(isset($idLevel_2)) {      $x3  = $idLevel_2;      }else{$x3  = $_SESSION['insumos_ing_nc_basicos']['idLevel_2'];}
				if(isset($idLevel_3)) {      $x4  = $idLevel_3;      }else{$x4  = $_SESSION['insumos_ing_nc_basicos']['idLevel_3'];}
				if(isset($idLevel_4)) {      $x5  = $idLevel_4;      }else{$x5  = $_SESSION['insumos_ing_nc_basicos']['idLevel_4'];}
				if(isset($idLevel_5)) {      $x6  = $idLevel_5;      }else{$x6  = $_SESSION['insumos_ing_nc_basicos']['idLevel_5'];}
	...
	<div class="box dark">
		...
		<div id="div-1" class="body">
			<form class="form-horizontal" method="post" id="form1" name="form1" novalidate>
				...
				<?php 
				//Se verifican si existen los datos
				if(isset($idProveedor)) {        $x1  = $idProveedor;      }else{$x1  = $_SESSION['insumos_ing_nc_basicos']['idProveedor'];}
				if(isset($N_Doc)) {              $x2  = $N_Doc;            }else{$x2  = $_SESSION['insumos_ing_nc_basicos']['N_Doc'];}
				if(isset($Creacion_fecha)) {     $x3  = $Creacion_fecha;   }else{$x3  = $_SESSION['insumos_ing_nc_basicos']['Creacion_fecha'];}
				if(isset($idBodega)) {           $x4  = $idBodega;         }else{$x4  = $_SESSION['insumos_ing_nc_basicos']['idBodega'];}
				if(isset($Observaciones)) {      $x5  = $Observaciones;    }else{$x5  = $_SESSION['insumos_ing_nc_basicos']['Observaciones'];}
				...
				$Form_Imputs->form_select_join_filter('Bodega origen','idBodega', $x4, 2, 'idBodega', 'Nombre', 'bodegas_insumos_listado', 'usuarios_bodegas_insumos', $z, $dbConn);
				...
				$Form_Imputs->form_input_hidden('idTipo', 11, 2);			
...
<div class="col-sm-12 fcenter" style="margin-bottom:30px">
	...
	<?php 
	$ubicacion = $location.'&view=true&ing_bodega_nc=true';
...
<div class="col-sm-12 fcenter">
	...
	<div id="page-wrap">
		<div id="header"> <?php echo $_SESSION['insumos_ing_nc_basicos']['TipoDocumento']; ?></div>
			...
			<table id="meta" class="fleft otdata">
				<tbody>
					...
					<tr>
						...
						<td><?php echo $_SESSION['insumos_ing_nc_basicos']['Proveedor']; ?></td>
					</tr>
					<tr>
						<td class="meta-head">Documento</td>
						<td><?php echo $_SESSION['insumos_ing_nc_basicos']['Documento'].' N°'.$_SESSION['insumos_ing_nc_basicos']['N_Doc']?></td>
					</tr>
					<tr>
						<td class="meta-head">Bodega</td>
						<td><?php echo $_SESSION['insumos_ing_nc_basicos']['Bodega']; ?></td>
					</tr>
					<tr>
					...
					<tr>
						...
						<td><?php echo $_SESSION['insumos_ing_nc_basicos']['CentroCosto']; ?></td>
					</tr>
				</tbody>
			</table>
		...
		<table id="items">
			<tbody>
						...
						<tr class="item-row linea_punteada">
							...
							<td>
								<div class="btn-group" style="width: 70px;" >
									<a href="<?php echo $location.'&editProd='.$producto['idProducto']; ?>" title="Editar Insumo" class="btn btn-success btn-sm tooltip"><i class="fa fa-pencil-square-o"></i></a>
									<?php 
				...
				<?php  //Guardo el neto
				$_SESSION['insumos_ing_nc_basicos']['valor_neto_fact'] = $vtotal_neto;
				?>
					...
					$_SESSION['insumos_ing_nc_basicos']['valor_neto_imp'] = $vtotal_neto;
					?>
					<tr class="invoice-total" bgcolor="#f1f1f1">
					...
					if (isset($_SESSION['insumos_ing_nc_impuestos'])){
						...
						$tempa = $vtotal_neto;
						//recorro el lsiatdo entregado por la base de datos
						foreach ($_SESSION['insumos_ing_nc_impuestos'] as $key => $producto){
							...
							$_SESSION['insumos_ing_nc_impuestos'][$producto['idImpuesto']]['valor'] = $vtotal_IVA;
					...
					//guardo el total
					$_SESSION['insumos_ing_nc_basicos']['valor_total_fact'] = $vtotal_neto;
				...
				<tr>
					<?php if(isset($_SESSION['insumos_ing_nc_basicos']['Observaciones'])&&$_SESSION['insumos_ing_nc_basicos']['Observaciones']!=''){ ?>
					
						<td colspan="5" class="blank word_break"> 
							<?php echo $_SESSION['insumos_ing_nc_basicos']['Observaciones'];?>
						</td>
						<td class="blank">
							<div class="btn-group" style="width: 35px;" >
								<?php 
					...
					<?php }else{?>
						<td colspan="5" class="blank"> 
							<?php 
							$non = '';
							if(isset($_SESSION['insumos_ing_nc_temporal'])&&$_SESSION['insumos_ing_nc_temporal']!=''){
								$non = $_SESSION['insumos_ing_nc_temporal'];
							}	
...
if(isset($_GET['order_by'])&&$_GET['order_by']!=''){
	switch ($_GET['order_by']) {
		case 'cliente_asc':  $order_by = 'ORDER BY proveedor_listado.Nombre ASC ';                      $bread_order = '<i class="fa fa-sort-alpha-asc" aria-hidden="true"></i> Proveedor Ascendente'; break;
		case 'cliente_desc': $order_by = 'ORDER BY proveedor_listado.Nombre DESC ';                     $bread_order = '<i class="fa fa-sort-alpha-desc" aria-hidden="true"></i> Proveedor Descendente';break;
...
$z="WHERE bodegas_insumos_facturacion.idTipo=11";//Solo egresos
//Verifico el tipo de usuario que esta ingresando
...
if(isset($_GET['Devolucion_fecha']) && $_GET['Devolucion_fecha'] != ''){  $z .= " AND bodegas_insumos_facturacion.Devolucion_fecha='".$_GET['Devolucion_fecha']."'";}
if(isset($_GET['idTrabajador']) && $_GET['idTrabajador'] != ''){          $z .= " AND bodegas_insumos_facturacion.idTrabajador=".$_GET['idTrabajador'];}
if(isset($_GET['idBodega']) && $_GET['idBodega'] != ''){                  $z .= " AND bodegas_insumos_facturacion.idBodega=".$_GET['idBodega'];}
...
<div class="col-sm-12 breadcrumb-bar">
	...
	<?php if ($rowlevel['level']>=3){ ?>
		<?php if (isset($_SESSION['insumos_ing_nc_basicos']['idProveedor'])&&$_SESSION['insumos_ing_nc_basicos']['idProveedor']!=''){?>
	...
	<div class="well">
		<div class="col-sm-8 fcenter">
			<form class="form-horizontal" id="form1" name="form1" action="<?php echo $location; ?>" novalidate>
				...
				$Form_Imputs->form_select_join_filter('Bodega origen','idBodega', $x4, 1, 'idBodega', 'Nombre', 'bodegas_insumos_listado', 'usuarios_bodegas_insumos', $v, $dbConn);
				$Form_Imputs->form_textarea('Observaciones','Observaciones', $x5, 1, 160);
