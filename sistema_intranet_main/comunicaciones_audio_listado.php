...
if (!empty($_POST['submit'])){
	...
	$form_trabajo= 'insert';
	require_once 'A1XRXS_sys/xrxs_form/comunicaciones_audio_listado.php';
}
//se borra un dato
if (!empty($_GET['del'])){
	//Llamamos al formulario
...
if(!empty($_GET['id'])){
	...
	comunicaciones_audio_listado.HoraInicio,
	comunicaciones_audio_listado.HoraTermino,
	comunicaciones_audio_listado.idEstado,
	...
	$SIS_join  = '
	LEFT JOIN `core_sistemas`               ON core_sistemas.idSistema            = comunicaciones_audio_listado.idSistema
	LEFT JOIN `core_estados`                ON core_estados.idEstado              = comunicaciones_audio_listado.idEstado
	LEFT JOIN `core_tipo_videoconferencia`  ON core_tipo_videoconferencia.idTipo  = comunicaciones_audio_listado.idTipo
	LEFT JOIN `usuarios_listado`            ON usuarios_listado.idUsuario         = comunicaciones_audio_listado.idUsuario';
	$SIS_where = 'comunicaciones_audio_listado.idAudio ='.$_GET['id'];
	...
	$SIS_join  = 'LEFT JOIN `usuarios_listado`   ON usuarios_listado.idUsuario = comunicaciones_audio_listado_usuarios.idUsuario';
	$SIS_where = 'comunicaciones_audio_listado_usuarios.idAudio ='.$_GET['id'];
	$SIS_order = 'usuarios_listado.Nombre ASC';
	$arrUsuarios = array();
	$arrUsuarios = db_select_array (false, $SIS_query, 'comunicaciones_audio_listado_usuarios', $SIS_join, $SIS_where, $SIS_order, $dbConn, $_SESSION['usuario']['basic_data']['Nombre'], $original, 'arrUsuarios');

	?>

	<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
		<?php echo widget_title('bg-aqua', 'fa-cog', 100, 'AudioConferencia', $rowData['Nombre'], 'Resumen'); ?>
	</div>
		...
		<div class="box">
			<header>
				<ul class="nav nav-tabs pull-right">
					<li class="active"><a href="<?php echo 'comunicaciones_audio_listado.php?pagina='.$_GET['pagina'].'&id='.$_GET['id']?>" ><i class="fa fa-bars" aria-hidden="true"></i> Resumen</a></li>
					<li class=""><a href="<?php echo 'comunicaciones_audio_listado_datos.php?pagina='.$_GET['pagina'].'&id='.$_GET['id']?>" ><i class="fa fa-list-alt" aria-hidden="true"></i> Datos Básicos</a></li>
					<li class=""><a href="<?php echo 'comunicaciones_audio_listado_usuarios.php?pagina='.$_GET['pagina'].'&id='.$_GET['id']?>" >Usuarios</a></li>         
				</ul>
			</header>
...
<?php //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	$SIS_where = "comunicaciones_audio_listado.idEstado!=0";
	//Verifico el tipo de usuario que esta ingresando
	$SIS_where.=" AND comunicaciones_audio_listado.idSistema=".$_SESSION['usuario']['basic_data']['idSistema'];
	/**********************************************************/
	//Se aplican los filtros
	if(isset($_GET['Nombre']) && $_GET['Nombre']!=''){        $SIS_where .= " AND comunicaciones_audio_listado.Nombre LIKE '%".EstandarizarInput($_GET['Nombre'])."%'";}
	if(isset($_GET['Fecha']) && $_GET['Fecha']!=''){          $SIS_where .= " AND comunicaciones_audio_listado.Fecha='".$_GET['Fecha']."'";}
	if(isset($_GET['idEstado']) && $_GET['idEstado']!=''){    $SIS_where .= " AND comunicaciones_audio_listado.idEstado='".$_GET['idEstado']."'";}
	if(isset($_GET['idTipo']) && $_GET['idTipo']!=''){        $SIS_where .= " AND comunicaciones_audio_listado.idTipo='".$_GET['idTipo']."'";}
	if(isset($_GET['idUsuario']) && $_GET['idUsuario']!=''){  $SIS_where .= " AND comunicaciones_audio_listado.idUsuario='".$_GET['idUsuario']."'";}
	/*******************************************************/
	// consulto los datos
	$SIS_query = '
	...
	comunicaciones_audio_listado.HoraTermino,
	comunicaciones_audio_listado.idEstado,
	comunicaciones_audio_listado.idTipo,
	comunicaciones_audio_listado.idUsuario,
	comunicaciones_audio_listado_usuarios.idUsuario AS idInvitado,
	...
	LEFT JOIN `core_tipo_videoconferencia`              ON core_tipo_videoconferencia.idTipo              = comunicaciones_audio_listado.idTipo
	LEFT JOIN `usuarios_listado`                        ON usuarios_listado.idUsuario                     = comunicaciones_audio_listado.idUsuario
	LEFT JOIN `comunicaciones_audio_listado_usuarios`   ON comunicaciones_audio_listado_usuarios.idAudio  = comunicaciones_audio_listado.idAudio
	LEFT JOIN `usuarios_listado`   users                ON users.idUsuario                                = comunicaciones_audio_listado_usuarios.idUsuario';
	$SIS_order = 'comunicaciones_audio_listado.idEstado ASC, comunicaciones_audio_listado.Fecha ASC';
	$arrAudioConferencia = array();
	$arrAudioConferencia = db_select_array (false, $SIS_query, 'comunicaciones_audio_listado', $SIS_join, $SIS_where, $SIS_order, $dbConn, $_SESSION['usuario']['basic_data']['Nombre'], $original, 'arrAudioConferencia');
	...
	<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
		<div class="box">
			<header>
				<div class="icons"><i class="fa fa-table" aria-hidden="true"></i></div><h5>Listado de AudioConferencias</h5>
			</header>
							...
							<td>
								<div class="btn-group" style="width: 105px;" >
									...
									<?php if(($Conferencia[0]['idUsuario']==$_SESSION['usuario']['basic_data']['idUsuario']) OR ($_SESSION['usuario']['basic_data']['idTipoUsuario']==1)){ ?>
										<?php if ($rowlevel['level']>=2){ ?><a href="<?php echo $location.'&id='.$AudioConferencia; ?>" title="Editar Información" class="btn btn-success btn-sm tooltip"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></a><?php } ?>
