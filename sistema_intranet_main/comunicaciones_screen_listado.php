...
if (!empty($_POST['submit'])){
	...
	$form_trabajo= 'insert';
	require_once 'A1XRXS_sys/xrxs_form/comunicaciones_screen_listado.php';
}
//se borra un dato
if (!empty($_GET['del'])){
	//Llamamos al formulario
...
if(!empty($_GET['id'])){
	...
	// consulto los datos
	$SIS_query = '
	comunicaciones_screen_listado.Nombre,
	comunicaciones_screen_listado.Fecha,
	comunicaciones_screen_listado.HoraInicio,
	comunicaciones_screen_listado.HoraTermino,
	comunicaciones_screen_listado.idEstado,
	...
	$SIS_join  = '
	LEFT JOIN `core_sistemas`               ON core_sistemas.idSistema            = comunicaciones_screen_listado.idSistema
	LEFT JOIN `core_estados`                ON core_estados.idEstado              = comunicaciones_screen_listado.idEstado
	LEFT JOIN `core_tipo_videoconferencia`  ON core_tipo_videoconferencia.idTipo  = comunicaciones_screen_listado.idTipo
	LEFT JOIN `usuarios_listado`            ON usuarios_listado.idUsuario         = comunicaciones_screen_listado.idUsuario';
	$SIS_where = 'comunicaciones_screen_listado.idScreen ='.$_GET['id'];
	...
	$SIS_join  = 'LEFT JOIN `usuarios_listado`   ON usuarios_listado.idUsuario = comunicaciones_screen_listado_usuarios.idUsuario';
	$SIS_where = 'comunicaciones_screen_listado_usuarios.idScreen ='.$_GET['id'];
	$SIS_order = 'usuarios_listado.Nombre ASC';
	$arrUsuarios = array();
	$arrUsuarios = db_select_array (false, $SIS_query, 'comunicaciones_screen_listado_usuarios', $SIS_join, $SIS_where, $SIS_order, $dbConn, $_SESSION['usuario']['basic_data']['Nombre'], $original, 'arrUsuarios');
...
<?php //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	$SIS_where = "comunicaciones_screen_listado.idEstado!=0";
	//Verifico el tipo de usuario que esta ingresando
	$SIS_where.=" AND comunicaciones_screen_listado.idSistema=".$_SESSION['usuario']['basic_data']['idSistema'];
	/**********************************************************/
	//Se aplican los filtros
	if(isset($_GET['Nombre']) && $_GET['Nombre']!=''){        $SIS_where .= " AND comunicaciones_screen_listado.Nombre LIKE '%".EstandarizarInput($_GET['Nombre'])."%'";}
	if(isset($_GET['Fecha']) && $_GET['Fecha']!=''){          $SIS_where .= " AND comunicaciones_screen_listado.Fecha='".$_GET['Fecha']."'";}
	if(isset($_GET['idEstado']) && $_GET['idEstado']!=''){    $SIS_where .= " AND comunicaciones_screen_listado.idEstado='".$_GET['idEstado']."'";}
	if(isset($_GET['idTipo']) && $_GET['idTipo']!=''){        $SIS_where .= " AND comunicaciones_screen_listado.idTipo='".$_GET['idTipo']."'";}
	if(isset($_GET['idUsuario']) && $_GET['idUsuario']!=''){  $SIS_where .= " AND comunicaciones_screen_listado.idUsuario='".$_GET['idUsuario']."'";}
	/*******************************************************/
	// consulto los datos
	$SIS_query = '
	comunicaciones_screen_listado.idScreen,
	comunicaciones_screen_listado.Nombre,
	comunicaciones_screen_listado.Fecha,
	comunicaciones_screen_listado.HoraInicio,
	comunicaciones_screen_listado.HoraTermino,
	comunicaciones_screen_listado.idEstado,
	comunicaciones_screen_listado.idTipo,
	comunicaciones_screen_listado.idUsuario,
	comunicaciones_screen_listado_usuarios.idUsuario AS idInvitado,
	...
	LEFT JOIN `core_tipo_videoconferencia`               ON core_tipo_videoconferencia.idTipo                = comunicaciones_screen_listado.idTipo
	LEFT JOIN `usuarios_listado`                         ON usuarios_listado.idUsuario                       = comunicaciones_screen_listado.idUsuario
	LEFT JOIN `comunicaciones_screen_listado_usuarios`   ON comunicaciones_screen_listado_usuarios.idScreen  = comunicaciones_screen_listado.idScreen
	LEFT JOIN `usuarios_listado`   users                 ON users.idUsuario                                  = comunicaciones_screen_listado_usuarios.idUsuario';
	$SIS_order = 'comunicaciones_screen_listado.idEstado ASC, comunicaciones_screen_listado.Fecha ASC';
	$arrScreenSharing = array();
	$arrScreenSharing = db_select_array (false, $SIS_query, 'comunicaciones_screen_listado', $SIS_join, $SIS_where, $SIS_order, $dbConn, $_SESSION['usuario']['basic_data']['Nombre'], $original, 'arrScreenSharing');
	...
	<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
		<div class="box">
			<header>
				<div class="icons"><i class="fa fa-table" aria-hidden="true"></i></div><h5>Listado de Screen Sharing</h5>
			</header>
			<div class="table-responsive">
				<table id="dataTable" class="table table-bordered table-condensed table-hover table-striped dataTable">
					...
					<?php
					filtrar($arrScreenSharing, 'idScreen');
					foreach($arrScreenSharing as $ScreenSharing=>$Conferencia){ ?>
						<tr class="odd">
							...
							<td>
								<div class="btn-group" style="width: 105px;" >
									...
									if(isset($Conferencia[0]['idEstado'])&&$Conferencia[0]['idEstado']==1){
										...
										if(($Asiste!=0) OR ($Conferencia[0]['idUsuario']==$_SESSION['usuario']['basic_data']['idUsuario']) OR ($_SESSION['usuario']['basic_data']['idTipoUsuario']==1)){
											...
											if(isset($Conferencia[0]['idTipo'])&&$Conferencia[0]['idTipo']==1){
												if ($rowlevel['level']>=1){ ?><a href="<?php echo 'comunicaciones_screen_room.php?view='.$ScreenSharing; ?>" title="Entrar en el Screen Sharing" class="btn btn-primary btn-sm tooltip"><i class="fa fa-list" aria-hidden="true"></i></a><?php }
											//si es por dia fijo
									...
									<?php if(($Conferencia[0]['idUsuario']==$_SESSION['usuario']['basic_data']['idUsuario']) OR ($_SESSION['usuario']['basic_data']['idTipoUsuario']==1)){ ?>
										<?php if ($rowlevel['level']>=2){ ?><a href="<?php echo $location.'&id='.$ScreenSharing; ?>" title="Editar Información" class="btn btn-success btn-sm tooltip"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></a><?php } ?>
