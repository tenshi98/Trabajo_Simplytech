...
$location = $original;
$new_location = "usuarios_listado_cajas.php";
...
if ( !empty($_GET['caja_add']) )  { 
	//nuevas ubicaciones
	$location = $new_location;
	$location.='&id='.$_GET['id'];
	//Llamamos al formulario
	$form_trabajo= 'caja_add';
	require_once 'A1XRXS_sys/xrxs_form/usuarios_listado.php';
}
//se borra un dato
if ( !empty($_GET['caja_del']) )     {
	//nuevas ubicaciones
	$location = $new_location;
	$location.='&id='.$_GET['id'];
	//Llamamos al formulario
	$form_trabajo= 'caja_del';
	require_once 'A1XRXS_sys/xrxs_form/usuarios_listado.php';	
}
//formulario para crear
if ( !empty($_GET['prm_add_all']) )  { 
	...
	$location.='&id='.$_GET['id'];
	//Llamamos al formulario
	$form_trabajo= 'prm_add_all_cajas';
	require_once 'A1XRXS_sys/xrxs_form/usuarios_listado.php';
}
//se borra un dato
if ( !empty($_GET['prm_del_all']) )     {
	...
	$location.='&id='.$_GET['id'];
	//Llamamos al formulario
	$form_trabajo= 'prm_del_all_cajas';
	require_once 'A1XRXS_sys/xrxs_form/usuarios_listado.php';	
}
/**********************************************************************************************************************************/
...
(SELECT COUNT(idCajaPermiso) FROM usuarios_cajas_chicas WHERE idCajaChica = caja_chica_listado.idCajaChica AND idUsuario = {$_GET['id']} LIMIT 1) AS contar,
(SELECT idCajaPermiso FROM usuarios_cajas_chicas WHERE idCajaChica = caja_chica_listado.idCajaChica AND idUsuario = {$_GET['id']} LIMIT 1) AS idpermiso
FROM `caja_chica_listado`
LEFT JOIN `core_sistemas`   ON core_sistemas.idSistema   = caja_chica_listado.idSistema
WHERE caja_chica_listado.idSistema = ".$_SESSION['usuario']['basic_data']['idSistema']."
	...
	<div class="col-md-6 col-sm-6 col-xs-12" style="padding-left: 0px;">
		<div class="info-box bg-aqua">
			...
			<div class="info-box-content">
				...
				<span class="progress-description">Editar Permisos de acceso a Caja chica</span>
			</div>
		</div>
	</div>
</div>
				...
				<li class="dropdown">
					...
					<ul class="dropdown-menu" role="menu">
						...
						<?php if($x_permisos_4 > 0){ ?>
							<li class="active"><a href="<?php echo 'usuarios_listado_cajas.php?pagina='.$_GET['pagina'].'&id='.$_GET['id']?>" >Caja Chica</a></li>
						<?php } ?>
        ...
        <div class="table-responsive">
			...
			<table id="dataTable" class="table table-bordered table-condensed table-hover table-striped dataTable">
				<thead>
					...
					<tr role="row">
						<th <?php if($_SESSION['usuario']['basic_data']['idTipoUsuario']==1){echo 'colspan="3"';}else{echo 'colspan="2"';} ?>><input class="form-control" id="InputTableFilter" type="text" placeholder="Filtrar.."></th>
					</tr>
				</thead>
				...
				<tbody role="alert" aria-live="polite" aria-relevant="all" id="TableFiltered">
					...
					<?php foreach ($arrCajas as $cajas) { ?>
						<tr class="odd">
							<td><?php echo '<strong>Caja: </strong>'.$cajas['Nombre']; ?></td>
							<?php if($_SESSION['usuario']['basic_data']['idTipoUsuario']==1){ ?><td><?php echo $cajas['RazonSocial']; ?></td><?php } ?>
							<td>
								<div class="btn-group" style="width: 100px;" id="toggle_event_editing">	
									<?php if ( $cajas['contar']=='1' ) {?>    
										<a title="Quitar Permiso" class="btn btn-sm btn-default unlocked_inactive tooltip" href="<?php echo $new_location.'&id='.$_GET['id'].'&caja_del='.$cajas['idpermiso']; ?>">OFF</a>
									...
									<?php } else {?>
										...
										<a title="Dar Permiso" class="btn btn-sm btn-default unlocked_inactive tooltip" href="<?php echo $new_location.'&id='.$_GET['id'].'&caja_add='.$cajas['idCajaChica']; ?>">ON</a>
									<?php }?>    
								</div> 
							</td>
						</tr>
