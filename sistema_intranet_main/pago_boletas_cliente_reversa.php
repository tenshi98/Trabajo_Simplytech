...
$original = "pago_boletas_cliente_reversa.php";
$location = $original;
...
if (!empty($_GET['del_idPago'])){
	...
	require_once 'A1XRXS_sys/xrxs_form/z_pagos_boletas_clientes_reversa.php';
}
/**********************************************************************************************************************************/
...
if(!empty($_GET['submit_filter'])){
	...
	// consulto los datos
	$SIS_query = '
	pagos_boletas_clientes.idPago,
	clientes_listado.Nombre AS Cliente,
	sistema_documentos_pago.Nombre AS DocumentoPago,
	pagos_boletas_clientes.N_DocPago AS DocumentoPagoNumero,
	pagos_boletas_clientes.montoPactado AS DocumentoMontoPactado,
	pagos_boletas_clientes.MontoPagado AS DocumentoMontoPagado,
	pagos_boletas_clientes.idFacturacion';
	$SIS_join  = '
	LEFT JOIN `clientes_listado`          ON clientes_listado.idCliente          = pagos_boletas_clientes.idCliente
	LEFT JOIN `sistema_documentos_pago`   ON sistema_documentos_pago.idDocPago   = pagos_boletas_clientes.idDocPago';
	$SIS_where = 'pagos_boletas_clientes.idPago!=0';
	if(isset($_GET['idDocPago'])&&$_GET['idDocPago']!=''){	$SIS_where .= ' AND pagos_boletas_clientes.idDocPago='.$_GET['idDocPago'];}
	if(isset($_GET['N_DocPago'])&&$_GET['N_DocPago']!=''){	$SIS_where .= ' AND pagos_boletas_clientes.N_DocPago='.$_GET['N_DocPago'];}
	if(isset($_GET['idCliente'])&&$_GET['idCliente']!=''){	$SIS_where .= ' AND pagos_boletas_clientes.idCliente='.$_GET['idCliente'];}
	$SIS_order = 0;
	$arrBoletas = array();
	$arrBoletas = db_select_array (false, $SIS_query, 'pagos_boletas_clientes', $SIS_join, $SIS_where, $SIS_order, $dbConn, $_SESSION['usuario']['basic_data']['Nombre'], $original, 'arrBoletas');
		...
		<div class="box">
			...
			<div class="body">
				<form class="form-horizontal" id="form1" name="form1" action="<?php echo $location; ?>" autocomplete="off" novalidate>
					...
					$Form_Inputs->form_select_filter('Cliente','idCliente', $x3, 1, 'idCliente', 'Nombre', 'clientes_listado', $w, '', $dbConn);
