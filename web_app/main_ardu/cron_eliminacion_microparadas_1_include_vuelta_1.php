...
foreach ($arrErrores as $errn) {
	...
	//Si es la segunda o mas ejecuto
		...
		if($idTelemetria==$errn['idTelemetria']){
			/********************************************************************************/
			//Si el dato anterior era una activacion y el actual una desactivacion
			if($ValorAntiguo==1&&valores_truncados($errn['Valor'])==0){
		...
		//Si no es la misma grua
		}else{
			//El valor actual, el primer registro, es una desactivacion
			if(valores_truncados($errn['Valor'])==0){
				$query  = "UPDATE `telemetria_listado_historial_activaciones` SET idEstado=2 WHERE idH_Activacion = '".$errn['idH_Activacion']."'";
				$resultado = mysqli_query ($dbConn, $query);

			}
			//el ultimo valor de la grua anterior es una activacion
			if($ValorAntiguo==1){
				$query  = "UPDATE `telemetria_listado_historial_activaciones` SET idEstado=2 WHERE idH_Activacion = '".$idAntiguo."'";
				$resultado = mysqli_query ($dbConn, $query);
