...
/*******************************************************************************************************************/		
		case 'insert':
			...
			if(isset($Nombre)){
				$sql_usuario = mysqli_query("SELECT Nombre FROM documentacion_categorias WHERE Nombre='".$Nombre."' AND idSistema='".$idSistema."' "); 
			...
			if ( empty($error) ) {
				...
				$query  = "INSERT INTO `documentacion_categorias` (Nombre, idSistema) VALUES ({$a} )";
...
/*******************************************************************************************************************/		
		case 'update':	
			...
			if ( empty($error) ) {
				...
				// inserto los datos de registro en la db
				$query  = "UPDATE `documentacion_categorias` SET ".$a." WHERE idCategoria = '$idCategoria'";
				$result = mysqli_query($dbConn, $query);
...
/*******************************************************************************************************************/
		case 'del':	
			...
			$query  = "DELETE FROM `documentacion_categorias` WHERE idCategoria = {$_GET['del']}";
			$result = mysqli_query($dbConn, $query);	
