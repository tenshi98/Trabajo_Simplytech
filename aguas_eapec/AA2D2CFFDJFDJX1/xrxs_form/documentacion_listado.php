...
/*******************************************************************************************************************/		
		case 'insert':
			...
			if(isset($Nombre)){
				$sql_usuario = mysqli_query("SELECT Nombre FROM documentacion_listado WHERE idSistema='".$idSistema."' AND idCategoria='".$idCategoria."' AND Nombre='".$Nombre."' "); 
				$n1 = mysqli_num_rows($sql_usuario);
			} else {$n1=0;}
			if($n1 > 0) {$error['Nombre'] = 'error/El Nombre del archivo de documentacion ya existe en el sistema';}
			...
			if ( empty($error) ) {
				...
				if(isset($_FILES["imgLogo"])){
						...
						if (in_array($_FILES['imgLogo']['type'], $permitidos) && $_FILES['imgLogo']['size'] <= $limite_kb * 1024){
							...
							if (!file_exists($ruta)){
								...
								if ($resultado){
									...
									$query  = "INSERT INTO `documentacion_listado` (idSistema, idCategoria, Version, Nombre, Direccion_img) VALUES ({$a} )";
					...
					// inserto los datos de registro en la db
					$query  = "INSERT INTO `documentacion_listado` (idSistema, idCategoria, Version, Nombre) VALUES ({$a} )";
					$result = mysqli_query($dbConn, $query);
						...
						if (in_array($_FILES['imgLogo']['type'], $permitidos) && $_FILES['imgLogo']['size'] <= $limite_kb * 1024){
							...
							if (!file_exists($ruta)){
								...
								if ($resultado){
									...
									//Filtros
									$a = "idDocumentacion='".$idDocumentacion."'" ;
									...
									// inserto los datos de registro en la db
									$query  = "UPDATE `documentacion_listado` SET ".$a." WHERE idDocumentacion = '$idDocumentacion'";
									$result = mysqli_query($dbConn, $query);
...
/*******************************************************************************************************************/
		case 'del_file':	
			...
			if(unlink('upload/'.$archivo)&&isset($rowdata['Direccion_img'])&&$rowdata['Direccion_img']!=''){	
				...
				$query  = "UPDATE `documentacion_listado` SET Direccion_img='' WHERE idDocumentacion = '{$_GET['del_file']}'";
				$result = mysqli_query($dbConn, $query);
				//Redirijo			
...
/*******************************************************************************************************************/
		case 'del':	
			...
			$query = "SELECT Direccion_img
			FROM `documentacion_listado`
			WHERE idDocumentacion = {$_GET['del']}";
			$resultado = mysqli_query ($dbConn, $query);
			$rowdata = mysqli_fetch_assoc ($resultado);
			...
			if(unlink('upload/'.$archivo)&&isset($rowdata['Direccion_img'])&&$rowdata['Direccion_img']!=''){	
				...
				//se borran los datos
				$query  = "DELETE FROM `documentacion_listado` WHERE idDocumentacion = {$_GET['del']}";
				$result = mysqli_query($dbConn, $query);	
