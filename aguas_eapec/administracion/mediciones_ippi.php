...
if ( !empty($_POST['submit']) )  { 
	...
	$form_trabajo= 'insert';
	require_once '../AA2D2CFFDJFDJX1/xrxs_form/mediciones_ippi.php';
}
//formulario para editar
if ( !empty($_POST['submit_edit']) )  { 
	//Llamamos al formulario
	$form_obligatorios = 'idIPPI,Ano,idMes,Valor,idSistema';
	$form_trabajo= 'update';
	require_once '../AA2D2CFFDJFDJX1/xrxs_form/mediciones_ippi.php';
}
//se borra un dato
	...
	<div class="box dark">
		...
		<form class="form-horizontal" method="post" name="form1">
			...
			<div class="form-group">
            	<input type="hidden" name="idIPPI" value="<?php echo $_GET['id']; ?>" >
...
if($arrUsuario['tipo']=='SuperAdmin'){
	$z="WHERE mediciones_ippi.idSistema>=0";	
}else{
	$z="WHERE mediciones_ippi.idSistema={$arrUsuario['idSistema']}";	
}
//Realizo una consulta para saber el total de elementos existentes
$query = "SELECT idIPPI FROM `mediciones_ippi` ".$z."";
...
$arrContactos = array();
$query = "SELECT 
mediciones_ippi.idIPPI,
mediciones_ippi.Ano,
mediciones_ippi.Valor,
core_sistemas.Nombre AS Sistema,
mnt_meses.Nombre AS Mes
FROM `mediciones_ippi`
LEFT JOIN `core_sistemas`   ON core_sistemas.idSistema   = mediciones_ippi.idSistema
LEFT JOIN `mnt_meses`       ON mnt_meses.idMes           = mediciones_ippi.idMes
".$z."
ORDER BY mediciones_ippi.Ano DESC, mediciones_ippi.idMes DESC
...
<?php if ($rowlevel['level']>=3){?><a href="<?php echo $location; ?>&new=true" class="btn btn-default fright margin_width fmrbtn" >Crear Nuevo IPPI</a><?php } ?>
</div>
...
<div class="col-lg-12">
	<div class="box">
		<header>
			<div class="icons"><i class="fa fa-table"></i></div><h5>Listado de IPPI</h5>
			<div class="toolbar">
				<?php 
					...
					<tr class="odd">
						...
						<td>
							<div class="btn-group widthtd120" >
								<?php if ($rowlevel['level']>=2){?><a href="<?php echo $location.'&id='.$cont['idIPPI']; ?>" data-placement="bottom" title="Editar Informacion" data-toggle="tooltip" class="btn btn-primary btn-sm info-tooltip"><i class="fa fa-pencil-square-o"></i></a><?php } ?>
								<?php if ($rowlevel['level']>=4){
									$ubicacion = $location.'&del='.$cont['idIPPI'];
									$dialogo   = '¿Realmente deseas eliminar el IPPI del mes de '.$cont['Mes'].'?';?>
