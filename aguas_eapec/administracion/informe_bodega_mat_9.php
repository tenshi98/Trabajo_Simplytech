...
$original = "informe_bodega_mat_9.php";
$location = $original;
...
if($arrUsuario['tipo']=='SuperAdmin'){
	...
	$w = "bodegas_facturacion.idSistema={$arrUsuario['idSistema']}";	
}
	...
	<div class="box">
		<header>
			...
			<div class="toolbar">
				...
				$zz .= '&idBodegaDestino='.$_GET['idBodegaDestino'];
				$zz .= '&idSistemaDestino='.$_GET['idSistemaDestino'];
				$zz .= '&f_inicio='.$_GET['f_inicio'];
				$zz .= '&f_termino='.$_GET['f_termino'];
				?>
				<a target="new" href="informe_bodega_mat_9_to_excel.php<?php echo $zz ?>" class="btn btn-sm btn-metis-2"><i class="fa fa-file-excel-o"></i> Exportar a Excel</a>
				<a target="new" href="informe_bodega_mat_9_to_pdf.php<?php echo $zz ?>" class="btn btn-sm btn-metis-3"><i class="fa fa-file-pdf-o"></i> Exportar a PDF</a>
				<a target="new" href="informe_bodega_mat_9_to_print.php<?php echo $zz ?>" class="btn btn-sm btn-metis-5"><i class="fa fa-print"></i> Imprimir</a>
			</div>
		</header>
			...
			<table id="dataTable" class="table table-bordered table-condensed table-hover table-striped dataTable">
				...
				<?php foreach ($arrProductos as $productos) { ?>
					<tr class="odd">
						...
						<td><?php echo Cantidades_decimales_justos($productos['Engreso']).' '.$productos['Uml']; ?></td>
						<td><?php echo Valores_sd($productos['ValorIngreso']); ?></td>
						<td><?php echo Valores_sd($productos['Engreso']*$productos['ValorIngreso']); ?></td>
...
<div class="col-lg-6 fcenter">
	<div class="box dark">
		...
		<form class="form-horizontal" method="post" name="form1">
			...
			echo form_select('Bodega Origen','idBodegaOrigen', $x1, 2, 'idBodega', 'Nombre', 'bodegas_listado', $z, $dbConn);
			echo form_select_depend1('Empresa Destino','idSistemaDestino', $x2, 2, 'idSistema', 'Nombre', 'core_sistemas', 0,
							 ...
							 $dbConn, 'form1');
			echo form_date('Fecha Inicio','f_inicio', $x4, 2);
			echo form_date('Fecha Termino','f_termino', $x5, 2);
