...
if ( !empty($_POST['submit']) )  { 
	...
	$form_trabajo= 'insert';
	require_once '../AA2D2CFFDJFDJX1/xrxs_form/z_clientes_eventos.php';
}
...
$query = "SELECT idSistema, idCliente, idTipo, FechaEjecucion, Fecha, Observacion, Archivo, NSello
	...
	<div class="box dark">	
		...
		<div id="div-1" class="body">	
			<form class="form-horizontal" method="post" name="form1" enctype="multipart/form-data">
				...
				if(isset($idSistema)) {        $x7  = $idSistema;         }else{$x7  = $rowdata['idSistema'];}
				...
				echo form_select('Tipo de Evento','idTipo', $x2, 2, 'idTipo', 'Nombre', 'clientes_eventos_tipos', 0, $dbConn);
				echo form_date('Fecha Ejecucion','FechaEjecucion', $x3, 2);
				echo form_date('Fecha Facturacion (9 del mes)','Fecha', $x4, 2);
				echo form_input_icon('text','Numero de Sello','NSello', $x5, 1, 'fa fa-barcode');
				echo form_textarea('Observacion', 'Observacion', $x6, 2);
				
				//si el archivo existe la muestro con las opciones de borrarlo
				if(isset($rowdata['Archivo'])&&$rowdata['Archivo']!=''){?>
			
				<div class="col-lg-10 fcenter">
					<h3>Nombre Archivo</h3>
					<p><?php echo $rowdata['Archivo']; ?></p>
				</div>
				...
				<div class="form-group">
					<input type="hidden" name="idEventos" value="<?php echo $_GET['id']; ?>" >		
...
if($arrUsuario['tipo']=='SuperAdmin'){
	$z="WHERE clientes_eventos.idSistema>=0";	
}else{
	$z="WHERE clientes_eventos.idSistema={$arrUsuario['idSistema']}";	
}
...
$query = "SELECT clientes_eventos.idEventos FROM `clientes_eventos` LEFT JOIN `clientes_listado`   ON clientes_listado.idCliente   = clientes_eventos.idCliente  ".$z." ";
...
".$z."
ORDER BY clientes_eventos.FechaEjecucion DESC
...
<?php if ($rowlevel['level']>=3){?><a href="<?php echo $location; ?>&new=true" class="btn btn-default fright margin_width fmrbtn" >Crear Nuevo Evento</a><?php }?>
</div>
		...
		<div class="table-responsive">
			<table id="dataTable" class="table table-bordered table-condensed table-hover table-striped dataTable">
				...
				<tbody role="alert" aria-live="polite" aria-relevant="all">
					...
					<tr class="odd">		
						<td><?php echo $usuarios['FechaEjecucion']; ?></td>
						...
						<td>
							<div class="btn-group widthtd120" >
								<?php if ($rowlevel['level']>=1){?><a href="<?php echo 'view_cliente_eventos.php?view='.$usuarios['idEventos']; ?>" data-placement="bottom" title="Ver Informacion" data-toggle="tooltip" class="btn btn-primary btn-sm info-tooltip"><i class="fa fa-list"></i></a><?php } ?>
								<?php if ($rowlevel['level']>=2){?><a href="<?php echo $location.'&id='.$usuarios['idEventos']; ?>" data-placement="bottom" title="Editar Informacion" data-toggle="tooltip" class="btn btn-primary btn-sm info-tooltip"><i class="fa fa-pencil-square-o"></i></a><?php } ?>
								<?php if ($rowlevel['level']>=4){
									$ubicacion = $location.'&del='.$usuarios['idEventos'];
