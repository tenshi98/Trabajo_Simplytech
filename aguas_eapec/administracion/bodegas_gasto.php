...
if ( !empty($_POST['submit']) )  { 
	...
	$form_obligatorios = 'idBodega,idSistema,idUsuario,Creacion_fecha,idTipo';
...
WHERE productos_listado.idProducto='{$_SESSION['gasto_productos'][$_GET['editProd']]['idProducto']}'";
	...
	<div class="box dark">
		...
		<div id="div-1" class="body">
			<form class="form-horizontal" method="post" name="form1">
				...
				if(isset($idProducto)) {       $x1  = $idProducto;      }else{$x1  = $_SESSION['gasto_productos'][$_GET['editProd']]['idProducto'];}
				if(isset($Number)) {           $x2  = $Number;          }else{$x2  = $_SESSION['gasto_productos'][$_GET['editProd']]['Number'];}
...
<div class="col-lg-6 fcenter">
	<div class="box dark">
		...
		<form class="form-horizontal" method="post" name="form1">
            ...
            <?php 
			//Se verifican si existen los datos
			if(isset($idBodega)) {         $x1  = $idBodega;       }else{$x1  = $_SESSION['gasto_basicos']['idBodega'];}
			if(isset($idSistema)) {        $x2  = $idSistema;      }else{$x2  = $_SESSION['gasto_basicos']['idSistema'];}

			//se dibujan los inputs
			echo form_select_bodega('Bodega Origen','idBodega', $x1, 2, 'idBodega', 'Nombre', 'bodegas_listado', 'usuarios_bodegas_productos', $z, $dbConn);
			
			if($arrUsuario['tipo']=='SuperAdmin'){
			echo form_select('Sistema','idSistema', $x2, 2, 'idSistema', 'Nombre', 'core_sistemas', 0, $dbConn);
			}else{
			...
			echo '<input type="hidden" name="idTipo"   value="3">';			
			?> 
...
WHERE idTipo = {$_SESSION['gasto_basicos']['idTipo']}";
$resultado = mysqli_query ($dbConn, $query);
...
WHERE idBodega = {$_SESSION['gasto_basicos']['idBodega']}";
$resultado = mysqli_query ($dbConn, $query);
        ...
        <table id="meta" class="otdata2">
            <tbody>
                <tr>
                    ...
                    <td><?php echo Fecha_estandar($_SESSION['gasto_basicos']['Creacion_fecha'])?></td>
                </tr>
            </tbody>
        </table>
    </div>
        ...
        <tbody>
            ...
            <tr>
				<td colspan="6" class="blank word_break"> 
					<?php echo $_SESSION['gasto_basicos']['Observaciones'];?>
				</td>	
            </tr>
            <tr>
...
$dialogo   = '¿Realmente desea realizar este gasto de bodega, una vez realizada no podra realizar cambios?';?>
<a onClick="dialogBox('<?php echo $ubicacion ?>', '<?php echo $dialogo ?>')" class="btn btn-danger fright margin_width" data-original-title="" title="">Realizar gasto</a>	
 ...
 <div class="col-lg-6 fcenter">
	<div class="box dark">
		...
		<form class="form-horizontal" method="post" name="form1">
			...
			//se dibujan los inputs
			echo form_select_bodega('Bodega Origen','idBodega', $x1, 2, 'idBodega', 'Nombre', 'bodegas_listado', 'usuarios_bodegas_productos', $z, $dbConn);
			echo form_textarea('Observaciones','Observaciones', $x2, 2);
...
$z="WHERE bodegas_facturacion.idTipo=3";//Solo gastos
//Verifico el tipo de usuario que esta ingresando
if($arrUsuario['tipo']=='SuperAdmin'){
...
<?php if ($rowlevel['level']>=3){
	if (isset($_SESSION['gasto_basicos']['idBodega'])&&$_SESSION['gasto_basicos']['idBodega']!=''){?>
		<a href="<?php echo $location; ?>&view=true" class="btn btn-default fright margin_width" >Continuar Gasto</a>
<?php }else{?>
		<a href="<?php echo $location; ?>&new=true" class="btn btn-default fright margin_width" >Crear Nuevo Gasto</a>
<?php }
 }?>
</div>
<div class="clearfix"></div>                       
